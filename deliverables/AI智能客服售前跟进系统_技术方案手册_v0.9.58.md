...
当前唯一架构口径继承 `0.9.57` 全部业务边界。`0.9.58` 只清理重复耗时统计：后端 `process_stage_runs` 是唯一运营级权威；Worker 本地 telemetry 只是有界的断网缓冲、4xx 隔离和后端权威快照证据缓存，禁止参与业务决策。C3/C4 标准发送耗时固定由 Worker 从启动 Sidecar 前计时至 Sidecar 返回或异常；Sidecar 内部耗时只作诊断。本地实现已统一为 `contract_revision=0.9.58`；规范化 SHA 已由正式生成器实算为 `47185e439b6fced86dad8d8d607efc1efa4764775f0a19d5e629cb945bca2a10`。独立 OmniAuto 来源已形成真实提交 `2a4a7ed9e459421c53865b0bbc9eeb9d88b3dd09` 并完成来源登记；车金提交、推送、标签和安装包尚未形成。
...
6. 对外 HTTP 接口的唯一正式名称固定为“接口编号 + HTTP 方法 + 从 `/api` 开始的
   ...
   同义名称。
7. 灰度版本使用唯一 `0.9.x` 序列：`0.9.0` 至 `0.9.57` 已冻结，当前目标候选为 `0.9.58`；
   ...
   灰度发布版本；本地实现已登记 `contract_revision=0.9.58`，规范化 SHA 为 `47185e439b6fced86dad8d8d607efc1efa4764775f0a19d5e629cb945bca2a10`；独立 OmniAuto 来源提交为 `2a4a7ed9e459421c53865b0bbc9eeb9d88b3dd09`，车金候选提交、标签和安装包仍须按发布流程真实形成。
...
8. 灰度稳定分支固定为 `codex/gray-release-0.9.x`，只接收当前灰度版本的缺陷修复、合同同步、
   测试和发布治理；唯一经确认提前进入灰度的优化是 15.6 耗时观测清理，目标为 `0.9.58`，
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `contract_version` | integer | 是 | 协议结构代号固定为 `3`，不是发布版本；目标灰度版本由 `contract_revision=0.9.57` 表达，并同时校验最终重新生成的 `contract_sha256 / observation_schema_version`。 |
...
### 15.6 灰度 `0.9.58` 重复耗时统计清理实施合同

本节是灰度 `0.9.58`“清理新旧两套重复耗时统计”的完整开发口径。该项只收口观测数据，
不优化业务耗时，不调整 C0—C4 步骤，不删除任何安全校验，也不改变任务、会话、消息、
媒体、Brain、Handoff、飞书和发送的执行次数。`0.9.x`、`1.0.x` 已发布产物及其历史证据
保持不可变；实际开发必须使用新的精确 `0.9.58` 版本，不能回写旧包。
...
| UAT/故障包对旧字段的读取 | 冻结历史包兼容 | 只允许读取已发布历史包；新 `0.9.58` 证据优先读取标准阶段。旧读取兼容不得让新生产路径继续双写。 |
...
除 15.6“重复耗时统计清理”经确认提前进入独立灰度 `0.9.58` 外，其余 P1 和 P2 固定进入
正式上线后的 `1.1.x` 独立优化系列，不再占用 `0.9.x` 灰度修复序列，也不得回写 `1.0.x`
正式稳定系列。`0.9.58` 只允许实施 15.6，不得顺带带入下列其他优化；后续每个实现候选使用
...
2. 耗时观测清理：保留 `0.9.8` 已完成的统一耗时合同、后端阶段表、Worker telemetry SQLite、
   排队/执行耗时分离和查询接口。完成 15.4 的新旧计时并行比对后，只删除重复输出、重复汇总
   和错误的父子阶段相加；底层单调计时和故障诊断耗时继续保留。
3. C4 性能只在取得独立真实召回链路样本后再评估；不得用 C2/C3 样本推算或直接套用优化。
