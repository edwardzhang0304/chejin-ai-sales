...
当前版本继承不可变 `gray-v0.9.46`（`672b63a`），其生产能力仍以 `gray-v0.9.43`（`b6ad192`）为原始稳定底座。历史版本的提交、合同 SHA、发布状态和变更原因只允许记录在《版本更新记录》，不在本文继续保留可能被误读为现行规则的旧实现说明。本文及配套全流程图是 `0.9.47` 唯一有效技术口径。
...
**实现状态：** `0.9.47` 已完成重启恢复与故障状态收口并通过架构复审。已确认物理语音/图片 Journal 只允许修复同一客户、同一 `origin_read_run_id`、同一 `source_message_key`、同一媒体类型且仍处于 `failed + waiting` 的 Ledger；旧失败结果作为审计证据保留，已确认事实绝不覆盖。后端已确认而 Windows 暂时无法删除 Journal 时，保留旧 Flow 并在下一心跳只重试归档，零重复 HTTP、零重复微信操作。永久恢复冲突必须先持久化 `faulted`；状态接口或旧 Flow 结束接口断网、多次心跳及中央暂停函数均不得降级为 `paused`，只有明确且成功的用户“开始接单”可以解除。规范化合同为 `0.9.47 / c3d1b2a8f436182df84d1b692f6be76cb233fb2d7716664a9db0107da8cf0b28`；独立 OmniAuto 来源提交为 `6fc2b12a1c121ec747f9f7ef7123775474dff1d1`，只同步生成合同 Schema。正常文字、语音、图片、混合消息、Brain、媒体编排和后端业务接口未改变；自动化通过不代表 Windows 实机 UAT。
