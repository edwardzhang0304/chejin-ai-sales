...
文档版本：v0.9.85；合同基线：0.9.85；文档修订：r4（2026-09-16，分段灰度集成，未发布）。
...
当前正式发布为 **0.9.85**，客户端包、生产后端合同和四份文档一致。按用户要求从原装 0.9.84 保留数据安装（强升），不开放检查更新。正式成品、暂停／故障安装及公网下载已验收；客户 Windows 实际安装与原生微信加好友仍待验收。此前 0.9.84 的已发布包和回退分支保留。
