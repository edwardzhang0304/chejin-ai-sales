...
文档版本：v0.9.85；合同基线：0.9.85；文档修订：r1（2026-09-16，备注光标正式强升候选，尚未发布）。

## 0.9.85 正式发布准备（2026-09-16）

本轮来源 Worker `62261fd08b8e61399589967ab830e81c267d7d49`、OmniAuto `1406585cbc7a24e941aa825484c421716d4ed903`。备注 OCR 恰为9位且末尾为 ASCII `|` 时去掉光标后走原八位校验；不增加点击、OCR 或重试，不带入其他未审修改。源码独立复审通过，复用73条去重正向证据，0项业务测试重跑；原47项回归中有1项旧分类断言失败，已在原版复现且独立复核，单独保留，不登记为全绿。

用户确认实际 Windows 为0.9.84，已正常退出且无待处理项。按固定SOP执行一次构建、同包暂停／故障保留数据安装、GitHub直接传服务器、配套合同后端与生产/api后台核验。保留全部业务数据、账号、绑定、配置及正常排队任务；本轮沿用免新增备份、免自动回滚授权，失败停止对应阶段并保留证据。Fast UAT不变。Windows实际好友申请和现场业务仍待验收。

以下源码交付和历史发布条目保留当时状态，不代表当前候选已部署。独立复审原始证据保留于本机 `releases/0.9.85/ops/source-review/`；公开交接凭证仅包含文件哈希、测试名称与范围说明，不上传客户原图或聊天内容。
...
当前正式候选为 **0.9.85**，生产仍为 **0.9.84**。本轮仅加入已审备注末尾光标修复；按用户要求从原装 0.9.84 保留数据安装（强升），不开放检查更新。应用、合同和四份文档同步升版；正式成品、保留数据安装及 Windows 现场加好友仍待验收。此前 0.9.84 的已发布包和回退分支保留。
