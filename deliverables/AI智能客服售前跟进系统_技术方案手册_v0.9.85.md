...
文档版本：v0.9.85；合同基线：0.9.85；文档修订：r2（2026-09-16，正式强升发布完成）。
...
账号、知识、销售、Worker绑定／配置、业务数据和4条正常排队任务均保留；Brain／DeepSeek配置指纹一致。本轮未新增备份、未自动回滚、未清库；失败停止对应阶段并保留证据。客户Windows实际安装、原生微信好友申请及现场业务仍待验收。容量前置优化另行修改发布工具，不改这份成品；Fast UAT不变。
...
**构建前服务器容量准备（2026-09-16 用户要求；实现中，验证后登记启用）**
