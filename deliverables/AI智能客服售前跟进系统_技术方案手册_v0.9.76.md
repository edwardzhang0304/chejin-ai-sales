...
0.9.76正式发布准备：业务源码821517d，原装Windows0.9.75为升级起点；复用工程师已通过的57项定向结果及未变基础源码证据，只补版本/合同一致性和真实Windows成品升级验收。尚未确认必须强升；按钮路线必须以原装0.9.75和本次不可变ZIP的paused/faulted及长路径验收为准。用户已确认暂停、正常退出且没有本机待处理项。后端与文档合同同步0.9.76，后台源代码不变时保留已核对生产/api的产物。生产配置、Brain/DeepSeek、账号、绑定、知识库和业务数据保留；沿用免备份及不自动生产回滚授权，失败阻断更新登记并保留现场。当前未构建或切换；Fast UAT不触发。
