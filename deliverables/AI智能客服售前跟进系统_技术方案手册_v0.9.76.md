...
任一不满足时保持 `waiting_for_safe_boundary`，继续原恢复/结算机制，禁止强制终止进程、
...
才允许安装；更新或回滚后必须继续保持 `faulted`，只能由运营人员明确点击“开始接单”解除。
...
当前新旧合同不兼容，详见 15.11。生产后端切换前须在旧后端仍可结算时排空 Worker 的 Ledger、ActionJournal、Outbox、sent_ack 和在途流程，
