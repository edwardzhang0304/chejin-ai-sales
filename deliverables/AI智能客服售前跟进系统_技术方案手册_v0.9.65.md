...
版本：v0.9.65（发布状态登记 v0.2；生产已部署，Windows 实机验收待完成）
...
当前唯一架构口径继承 `0.9.64` 的 C0—C4、头像/多行分组修复和严格单合同。`0.9.65` 只接收
客户端工程师 `aadbea4f9b3d0cecfe132eea402a95773bf763cf` 的租约清理与诊断补充，以及本轮必要的发布版本同步。
...
`0.9.65` 合同为 `6ebe42853790b9911c3f81dbf1b917ddec03b249747bb4c26a447a889a862b5b`，由正式生成器计算。OmniAuto 行为与来源提交
`03ddcd66cf22740c5c30537b0fb9e7873e24b50f` 保持不变，生成 Schema 作为车金合同覆盖层重新生成。
用户确认上一轮 `0.9.63 → 0.9.64` 安装失败并回滚，查询超时与 Windows 子进程退出原因仍【待确认】；
诊断增强不能写成这些原因已修复。本轮正式构建、同版后端/后台切换、更新登记和公网验证已通过；Windows 实际升级与业务 UAT 仍待完成，证据见版本更新记录。
...
6. 对外 HTTP 接口的唯一正式名称固定为“接口编号 + HTTP 方法 + 从 `/api` 开始的
   ...
   同义名称。
7. 灰度版本使用唯一 `0.9.x` 序列：`0.9.0` 至 `0.9.64` 已冻结，当前目标候选为 `0.9.65`；
   ...
   灰度发布版本；当前目标为 `contract_revision=0.9.65`，规范化 SHA 必须在客户端、后端发布清单、
...
8. 灰度稳定分支固定为 `codex/gray-release-0.9.x`，只接收当前灰度版本的缺陷修复、合同同步、
   ...
   `0.9.65` 继承 6.0.3.3.3 的头像检测/多行分组修复，只额外接收已确认任务租约清理、更新诊断及必要测试、发布治理，不得借此带入确认次数优化、AI 回复效果优化、经验池、Prompt 管理或其他下一期功能。每个不可变灰度候选使用精确标签
...
#### 6.0.3.3.3 当前帧真实头像检测与发送后短末行整改（0.9.63 事故补充，0.9.64 实现已复审，0.9.65 继承）
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `contract_version` | integer | 是 | 协议结构代号固定为 `3`，不是发布版本；目标灰度版本由 `contract_revision=0.9.65` 表达，并同时校验最终重新生成的 `contract_sha256 / observation_schema_version`。 |
