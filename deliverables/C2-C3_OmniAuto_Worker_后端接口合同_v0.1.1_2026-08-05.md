...
车金 `3f65660…`（Worker 运行代码父基线 `4352f5e…`）、OmniAuto 整改父基线 `99d0070…`；当前仲裁通用修复已固定并推送为 `75a85ac…`。复审曾确认
...
> （包含 `35b0eee`），并选择性接入 `75a85ac…` 的真实弹窗边界菜单分类、正式失败原因、点击前安全落盘和可靠消息类型优先于结构图片的仲裁能力。车金只保留适配层；
...
| P0 | 历史基线已收口；当前候选待复审 | 历史 `v16.132.0` 固定到 `35b0eee` 且活动选择性集成为空；当前候选基础为 `a563e668…`，活动选择性接入 `75a85ac…`。 |
