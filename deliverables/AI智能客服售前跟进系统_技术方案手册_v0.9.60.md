...
版本：v0.9.60（本地代码瘦身与 Brain 主生成额度修复候选；待架构复审）
...
当前唯一架构口径完整继承已发布 `0.9.59` 的业务能力。`0.9.60` 收口两项已经确认的灰度变更：一是低风险代码瘦身，包括生产压缩 Worker Web 资源、从正式运行包排除测试和仓库资料、删除已证明无调用的兼容代码和重复接口、统一 FastAPI 启停生命周期；二是把普通 Brain、商品快速 Brain、低权限快速 Brain 三条主生成路径的 `max_tokens` 上限统一为 `8192`，避免模型思考消耗生成额度后没有足够空间输出合法结果。该数值只是输出上限，不要求模型必须生成到 8192，也不改变模型、提示词、状态机、超时、重试、回复长度、安全审核、Handoff 或 C0—C4 流程。当前 C2 机器合同 SHA 为 `30cdc4e69e87d3cb628ed334d2e95199837aad526a6298ab3bd5030225f51ea6`；Brain 额度不是 C2 消息合同字段，集成后必须重新运行生成器核对，若 C2 合同内容未变则 SHA 保持相同，不得为体现代码变化而伪造新 SHA。独立 OmniAuto 来源已形成真实提交 `eae057db568b4bb8a57a0d4694aaaadfacef9464`；车金候选提交、推送、标签和打包尚未进行。
...
6. 对外 HTTP 接口的唯一正式名称固定为“接口编号 + HTTP 方法 + 从 `/api` 开始的
   ...
   同义名称。
7. 灰度版本使用唯一 `0.9.x` 序列：`0.9.0` 至 `0.9.59` 已冻结，当前目标候选为 `0.9.60`；
   ...
   灰度发布版本；当前目标为 `contract_revision=0.9.60`，规范化 SHA 必须在客户端更新、后端发布清单、
   Schema 和生成物全部完成后由正式生成器真实计算，不得预填。当前 C2 合同已实算为 `30cdc4e69e87d3cb628ed334d2e95199837aad526a6298ab3bd5030225f51ea6`；15.10 的 Brain 额度不属于 C2 合同字段，集成后仍须重新运行生成器核对，合同内容未变时该 SHA 应保持不变。独立 OmniAuto 来源、车金候选提交、标签和安装包仍须按发布流程真实形成。
...
8. 灰度稳定分支固定为 `codex/gray-release-0.9.x`，只接收当前灰度版本的缺陷修复、合同同步、
   ...
   15.8 客户端手动检查更新，均已在 `0.9.59` 完成并由 `0.9.60` 原样继承。
   `0.9.60` 只额外接收 15.9 代码瘦身和 15.10 Brain 主生成额度修复，不得借此带入其他下一期功能。每个不可变灰度候选使用精确标签
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `contract_version` | integer | 是 | 协议结构代号固定为 `3`，不是发布版本；目标灰度版本由 `contract_revision=0.9.59` 表达，并同时校验最终重新生成的 `contract_sha256 / observation_schema_version`。 |
