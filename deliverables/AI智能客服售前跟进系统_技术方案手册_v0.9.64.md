...
版本：v0.9.64（发布状态登记 v0.1；生产测试灰度已部署，Windows 实机验收待完成）
...
当前唯一架构口径继承 `0.9.63` 的生产地址 `https://jiangsuchejin.com/api`、严格单合同及既有 C0—C4 能力。目标 `0.9.64` 仅修复 Sidecar 同帧头像检测与多行文字分组的必要接线，规则见 6.0.3.3.3；不修改 C2 字段语义、物理发送、媒体编排、Brain、Handoff 或 C0—C4 状态机。发布仍须按停止旧 Worker、结算本地事实、同步替换后端和客户端的顺序执行。

2026-09-04 状态：`0.9.64` 第三轮架构复审及正式构建门禁通过；按用户后续授权，运维已从
`d9132bc` 构建、部署同版生产后端与运营后台，并登记签名客户端到 `gray/windows-x64`。
包内与后端运行合同均为 `0.9.64 / 840a770923bf6ef43c37c33086d817a0ff2d860cbb7c923c3b9c43f0bcf465bc`。
来源、包哈希、备份、隔离恢复、发布验证及回滚入口见版本更新记录；旧 `0.9.63` 标签和包保留。
Windows 实际检查更新、设备绑定/心跳恢复和业务 UAT 仍【待完成】，不能把自动化与部署写成现场验收通过。
...
6. 对外 HTTP 接口的唯一正式名称固定为“接口编号 + HTTP 方法 + 从 `/api` 开始的
   ...
   同义名称。
7. 灰度版本使用唯一 `0.9.x` 序列：`0.9.0` 至 `0.9.63` 已冻结，当前目标候选为 `0.9.64`；
   ...
   灰度发布版本；当前目标为 `contract_revision=0.9.64`，规范化 SHA 必须在客户端、后端发布清单、
...
8. 灰度稳定分支固定为 `codex/gray-release-0.9.x`，只接收当前灰度版本的缺陷修复、合同同步、
   ...
   `0.9.64` 只额外接收 6.0.3.3.3 的头像检测/多行分组缺陷修复及必要测试、发布治理，不得借此带入确认次数优化、AI 回复效果优化、经验池、Prompt 管理或其他下一期功能。每个不可变灰度候选使用精确标签
...
- Worker 不重复做身份或业务决策：只校验 Sidecar 合同和后端授权，串行执行微信动作，维护 ActionJournal/Ledger/Outbox 并上报事实。
...
#### 6.0.3.3.3 当前帧真实头像检测与发送后短末行整改（0.9.63 事故补充，0.9.64 实现候选待复审）
...
2. 左右头像候选列必须相对于当前 `message_viewport_bounds`、有效布局尺度和图中独立头像结构
   确定；不得由每行 OCR 文字的 `left/right` 长短改变候选列，也不得使用某台电脑的固定屏幕坐标。
   候选列不是整个左/右聊天半屏，不得把气泡正文区、标题区、输入区或窗口外框作为头像候选。
...
5. 每个真实头像在当前组件表内只出现一次。每个文字行按既有同行关联规则查询该表；若多个候选
   仍无法唯一关联，须保留歧义，不能按候选顺序随便取一个，也不能把它解释成没有新头像。

##### C. 正常无头像续行与检测失败必须区分

1. 当前头像列完整有效、检测正常完成且该行没有新的头像或未决候选时，才表示“本行无新头像”。
   对普通文字，仍须满足 6.0.3.3.2 全部续行条件，才能继承上一组的唯一真实头像角色；
   三至五行的后续行不必各自有头像，短末行不能单独成为失败原因。
2. 截图不可用、布局/搜索列无效、候选裁断、检测异常或候选归属冲突表示证据无效/不明确，
   不得通过返回普通的 `present=false` 混同于“正常不存在”，并进入无头像续行。
   明确排除的气泡或边框噪声不是未决头像，不得因此把正常续行全部封死。
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `contract_version` | integer | 是 | 协议结构代号固定为 `3`，不是发布版本；目标灰度版本由 `contract_revision=0.9.64` 表达，并同时校验最终重新生成的 `contract_sha256 / observation_schema_version`。 |
