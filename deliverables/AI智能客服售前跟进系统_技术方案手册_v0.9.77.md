...
本节优先于前文把按钮升级列为所有发布必经项、或允许现场反复修工具的执行口径。目标是使用已冻结工具版本和一份发布参数记录完成发布，日常操作不编辑脚本。**当前为已确定的 SOP；现有脚本仍有写死版本/运行号、复测需重建环境及诊断不完整的问题，未通过工具冻结验收，不得宣称稳定 SOP 已自动实现。** 本次只修订文档，不触发 CI、打包或生产切换。
...
目前工作流和恢复检查含本轮固定版本/证据编号，完整参数化、逐场景续跑以及差异诊断尚未全部完成；**没有已验收的通用冻结工具版本**。现有 GitHub 工作流入口不等于上述能力已就绪。本 SOP 不提供假冒可直接执行的“一键命令”；工具维护完成后，才登记唯一入口、固定工具 SHA、参数说明及实测边界，不要求每次发版再开发脚本。
