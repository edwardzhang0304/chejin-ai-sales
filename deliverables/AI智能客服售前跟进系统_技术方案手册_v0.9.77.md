...
工具冻结验收使用合成版本/身份及故障注入检查参数、续跑、证据污染、路由隔离和PowerShell语法；不运行客户端全量业务回归，不发布测试包。Fast UAT工作流及GitHub ZIP→Mac→命令行保持原样。用户已取消0.9.75按钮修复，该起点的失败证据保留，但不再作为后续强升发布必须解决项。工具落地状态须在本轮实现及独立CI完成后如实更新。
...
本地工具检查及四项受影响打包结构检查共 128 项通过，随后补充接收器前置检查；独立 Windows PowerShell/GUI 驱动检查另走 `release-evidence.yml` 的 `tooling`，通过后在版本记录登记运行号。没有运行客户端全量业务回归，没有构建或更改现有正式包。额外查看旧打包测试文件时发现历史 OmniAuto 来源列表断言与已发布源码已有差异，本轮未改来源或业务逻辑，该无关断言不作为工具冻结检查。
