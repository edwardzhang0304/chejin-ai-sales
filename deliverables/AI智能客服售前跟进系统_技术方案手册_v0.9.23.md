...
当前唯一架构口径：服务端负责授权、状态机、事实持久化、Brain/Guard、任务和通知；OmniAuto 负责微信 UI 观察、物理目标判定和动作证据；Worker 只负责调度、合同校验、事务持久化与执行。C2 使用固定八位短码和 private 单聊门禁；文字、语音、图片按同一最终画面顺序入库，一条物理媒体只形成一个业务对象，所有已触发媒体动作必须有限终态。C2 消息身份只允许沿“帧内观察 -> 待处理媒体动作 -> 正式消息或隔离记录”单向推进；只有正式消息可以生成 source key、查询 Ledger/Outbox、上报后端或进入 Brain。Brain 只在最新待回复尾部完整且证据足够时生成回复；当前客户媒体失败、高意向及必须人工批准的业务硬风险进入人工接管。人工接管以未关闭 `HandoffEvent` 为权威事实并投影 `waiting_sales_reply`，服务端按同一事件通过车金统一飞书应用立即且至多通知一次所属销售。微信列表在截图与点击之间重排时，点击后的短码校验仍是硬门禁，但明确点到其他会话时允许丢弃旧坐标并在同一授权内完整重新定位一次。添加朋友流程无论“邀请已发送”还是“已经是好友”，都只对已经证明的添加朋友 HWND 执行一次右上角关窗并验证结果。`0.9.23` 取消 `0.9.22` “每个业务帧重建整套布局”的候选口径，改为启动时基于真实微信客户区截图完成一次全局布局标定，之后 C0—C4 复用该区域地图；会话行、消息、弹窗和表单只使用 `0.9.20` 原业务流程中本来就存在的必要帧定位。目标机器合同 revision 统一升为 `0.9.23`；规范化 SHA 必须在代码、Schema 和样例同步完成后实算，当前不得沿用 `0.9.22` SHA 或伪填新 SHA。
...
6. 对外 HTTP 接口的唯一正式名称固定为“接口编号 + HTTP 方法 + 从 `/api` 开始的
   ...
   同义名称。
7. 灰度版本使用唯一 `0.9.x` 序列：`0.9.0` 至 `0.9.22` 已冻结，当前目标候选为 `0.9.23`；
   ...
   灰度发布版本；当前目标必须将客户端、后端、OmniAuto、生成 Schema、manifest 与打包入口同步为 `contract_revision=0.9.23`，并在实现完成后实算唯一规范化 SHA。真实 Windows 验收、发布来源固定和正式包仍是后续门禁，不得把当前 `0.9.22` 代码或 SHA 伪报为 `0.9.23` 已实现。
...
机器合同 revision `0.9.23` 必须完整包含媒体和读取轮次字段，同时表达读取轮次与传输状态、语音动作身份与业务身份分离、
...
机器合同 revision `0.9.23` 在 Sidecar 请求/返回、ActionJournal、Worker 本地身份预留和最终 V3 evidence 中使用以下唯一媒体字段，禁止新增同义字段；规范化 SHA 必须在最终代码与 Schema 同步后实算：
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `contract_version` | integer | 是 | 协议结构代号固定为 `3`，不是发布版本；当前灰度目标由 `contract_revision=0.9.23` 表达，并同时校验实现完成后实算的规范化 `contract_sha256` 和 `observation_schema_version`。 |
...
### 16.11 0.9.23 启动一次布局标定与坐标地图兼容性整改
...
已批准的 C1 未变化画面证据复用必须保留：只合并两次使用之间没有点击、输入、页面切换或
其他 UI 变化的重复截图/OCR；正常 C1 画面节点仍为主界面、加号菜单、搜索弹窗、
手机号输入后、搜索结果、邀请表单、表单填写后和提交结果共八类。

`0.9.23` 的唯一方法是：启动时用当前微信客户区真实截图建立一份
...
2. 侧栏搜索、“+”号和返回按钮只使用侧栏头部坐标系。“+”号不要求 OCR 直接识别字符；
   应由启动标定已确认的侧栏边界和顶部搜索操作行，将 `0.9.20` 参考点映射到当前操作行。
   点击后仍必须使用 `0.9.20` 原有菜单出现确认，搜索图标不得冒充“+”。
3. 会话列表只复用标定的 `session_list_bounds`；具体客户行的 Y 坐标每轮必须来自当前
   `sessions/visible/search` 业务帧，不得缓存“第几行”。会话排序变化不会使全局标定失效；
   截图后到点击瞬间的重排仍由点击后短码标题复核拦截，并最多重新定位一次。
4. 会话标题、消息气泡、语音、图片和 customer/self 角色只复用标定的标题区/消息视口；
   具体对象坐标必须来自当前原业务帧，不得用启动截图推断消息位置。
5. 输入框、工具栏和发送按钮使用启动标定的输入区坐标系；S0/S1/S2 仍是原流程三个
   独立时间点，不得因复用布局标定而复用消息画面或省略新消息检查。
6. 添加朋友菜单、搜索页、搜索结果和邀请表单在启动时并不存在，不得伪造启动标定。
...
HWND 检查，不增加截图、OCR 或整套布局重算。
8. 前台语义必须继承 `0.9.20` 并按动作类型固定，不得把所有 Sidecar 调用统一改成被动探测：
   `status/capabilities/calibration-status/sessions` 只读探测不得抢前台；
   `add_friend/open-chat/messages/voice-transcribe/recover-render/send` 在执行本次第一个物理 UI 动作前，
   必须将启动时已确认的唯一微信主 HWND 切到前台，并确认当前前台根 HWND 仍是该微信 HWND。
   这里的“切到前台”只允许执行既有有限次数的 `ShowWindow/SetForegroundWindow` 类激活，
   不得调用 `MoveWindow`，不得改变窗口位置或尺寸，不得重新规范化窗口，不得重新标定，
   也不得为此新增截图或 OCR。客户端窗口原本处于前台不是失败场景，业务动作应先把微信切回前台再继续。
...
2. 只有已有 Windows 事件或下一张本来就需要的业务帧确认 HWND、进程、客户区尺寸、DPI
   或微信主外壳结构真实变化时，标定才失效。不得为“看看有没有变”增加点击前截图或 OCR。
   不得修改微信窗口样式、删除边框或安装键鼠钩子强行禁止用户拖动。空闲时发现窗口移动/缩放，
   下一业务动作前恢复默认档位并重新标定；在途事务不得中途拉回窗口，必须先按原事务边界安全结算。
3. 标定失效且尚未执行不可逆 UI 动作时，废弃本次坐标并使用下一张必要主会话帧重新标定；
   已触发媒体或发送动作时，不得重点，继续按 ActionJournal、Outbox 和 sent_ack 结算。
4. 启动标定在零点击下允许重取一帧。只有当当前客户区尺寸与 `0.9.20` 参考帧完全一致，
   且主结构锚点通过时，才允许直接使用参考坐标地图；这是精确匹配，不是任意分辨率的盲点兜底。
5. 仍无法标定时返回唯一技术错误 `WECHAT_UI_STARTUP_CALIBRATION_FAILED`，不点击微信，
...
3. 不保留 `0.9.22` 与 `0.9.23` 两套可运行布局决策、开关或双字段兜底。回滚只能整体回到
   不可变 `0.9.20` 分支/包，不改写业务数据。
4. 机器合同统一升为 `contract_revision=0.9.23`，Sidecar/Worker 至少交换
...
6. 公开生产入口必须证明正常 C1/C2/C3/C4 没有因标定新增业务截图/OCR，没有改变 `0.9.20`
   ...
   还必须分别证明：只读探测不会抢前台；C1/C2/C3 的真实物理动作入口在“客户端窗口当前处于前台”时，
   会先把同一微信主 HWND 激活到前台再执行动作；该激活过程调用 `MoveWindow/窗口规范化/重新标定/新增截图/OCR`
   的次数均为零。测试不得通过把 `foreground_window_matches_target` 永久伪造成 `true` 来绕过这一生产链。
7. 定向测试范围固定为窗口启动标定、C1 加好友、C2 会话定位/消息/媒体、C3 输入发送与
   C4 复用同一标定的直接受影响链路；本阶段不跑无关全量。只在正式打包门禁再跑全量。
8. 至少在两台 Windows 上覆盖 100%/125%/150% DPI、不同分辨率、同一 DPI 档位启动结果稳定不漂移、
   客户区过小时最小调整、会话列表重排、页面切换和窗口真实变化后重新标定。未完成前不得声称已兼容所有分辨率。

`0.9.23` 当前只完成方案与流程图收口。实现完成后必须依次同步 Worker、独立 OmniAuto
固定提交、`.chejin-source.json`、机器合同 revision、生成 Schema、manifest 和版本记录，
并实算新合同 SHA。Windows 实机验收和打包门禁未通过前，不得声称 `0.9.23` 已可交付。
