...
1. **共用测试，不共用临时包**：把 Fast UAT 既有的凭据、Worker/后端结算、恢复、Brain、观测中立性、合同和打包规则检查抽成同一个可调用步骤。两条流程调用同一份定义；Fast UAT 的测试内容、顺序及 ZIP 生成、下载、启动行为保持不变。正式构建独立运行所需检查及 Worker 全量回归，不采用人工填写“通过”来跳过测试。本轮不引入跨提交测试缓存，也不承诺缩短到十分钟。共用步骤入口为 `.github/actions/worker-release-checks/action.yml`；两条工作流只共享检查，不互相调用打包任务。
2. **先验证工具**：构建前执行真实 Qt 鼠标驱动冒烟、真实 SQLite 数据保护正反例、发布脚本回归和共用测试。任一失败即停止，不启动 PyInstaller。复测入口也先自检当前验收工具。新加检查工具必须先通过自身正反例；Windows 未实测不得写为已启用。
...
4. **允许复测的边界**：`retest_candidate` 只读取指定原构建运行中已保存的候选包，完全不调用打包命令。先核对 GitHub 来源、构建任务成功、产物未过期、四件交付文件哈希，再比较构建输入。只允许明确列出的升级验收脚本/对应测试、候选管理脚本/对应测试、验收任务编排及四份文档/规则变化；应用、依赖、合同、打包脚本、构建任务、共用测试或其他未列入例外的文件变化，必须阻断复用、重新构建。候选包丢失/过期或文件损坏同样阻断，不能自动改拿其他包。此复测入口只适用于新流程生成且带 `candidate.json` 的候选，不能把以前没有候选证明的包补标为可复测。
...
6. **自动推进与可观察性**：一次正式触发自动执行自检、必要测试、构建留存、验收和直传；一次复测触发自动执行工具自检、候选核验、验收和直传。`summary` 任务在结束时生成 `formal-stage-timings.json` 和 GitHub 阶段摘要；任务可能并行，不将任务耗时相加当作总等待时间。汇总各任务/步骤耗时、失败阶段、候选运行号、后续应该复测/重建/续传的入口。生产切换及排空条件尚未满足时停在待发布，不扩大自动化到未经确认的业务切换。等待期间由工具等待或状态监视处理，只读取新增失败日志和阶段摘要，不重复读取全量日志、从头分析或报告相同状态。
...
构建前检查：精确已推送提交的独立构建检出必须干净，来源登记完整，合同 revision/SHA 一致，流程 B 的工具自检、共用必要测试及正式门禁必须通过。
...
用户明确批准正式发布且来源已核对后，触发正式门禁；不先触发 Fast UAT ZIP 任务。新编排落地状态以 18.0.2 和版本更新记录为准：
