...
版本：v0.9.70.1（正式包准入准备；仅验收 0.9.69 → 0.9.70，未构建、未部署）

日期：2026-07-21

最后更新：2026-09-08

本次正式发布仅覆盖原始 0.9.69 → 0.9.70，不再要求兼容更早版本，最低 Updater 为 0.9.69。正式构建前补齐流水线目标与服务器验签基线，开放更新前必须使用原始旧客户端“检查更新”入口完成本次签名 ZIP 的完整 Windows 门禁；当前结果待验证。0.9.70 合同及后端两列迁移必须配套切换，不能只登记客户端包。生产配置和数据保持，正式包不经 Mac 中转；具体身份与执行进度见版本更新记录 2.0.70.1。

本轮源码候选：Worker `0.9.70`，C2 合同 `0.9.70 / 9a41f79da6b9dea74d23a2cc0ac5963a259f03285cf42432f6223816eb92c287`；生成 Schema 与双仓来源同步。无效线索联动及重启收尾已通过独立复审，搜索清理维持已通过结论。以下 0.9.69 信息是既有发布状态；0.9.70 尚未打包、部署或开放更新。
