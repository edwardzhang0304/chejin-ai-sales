...
版本：v0.9.73.4（0.9.73正式包与生产后端交付；现场恢复待验收）
...
- 本轮本机验证使用人工状态、真实HTTP/PostgreSQL、Worker生产线程与SQLite及真实浏览器；外部桌面/微信为受控边界。不是Windows正式EXE、高磊原SQLite或实际发送/Flow收尾验收。联合发布需新后端和新客户端，不再将新增恢复能力描述为“只部署后端”。
