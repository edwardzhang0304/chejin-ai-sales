...
版本：v0.9.71.1（0.9.71发布准备；应用与C2合同统一0.9.71，尚未构建或部署）

日期：2026-07-21

最后更新：2026-09-08

本轮发布目标调整为 **0.9.71**；已审功能保持不变，按用户明确要求将应用、安装包、C2 合同与生成 Schema 统一为 **0.9.71**。此前“应用版本未变”仅描述提交时状态，不能用来覆盖既有正式包。发布准备中，详见版本更新记录 2.0.71。


本次文档定义 9.1.1“车辆能源类型”、9.1.2“车系下拉分类”、9.1.3“排量输入字段”、9.1.4“电池包容量”及 9.1.5“驱动方式”。联合复审通过并已完成双仓源码集成，来源记录见版本更新记录2.0.70.16；不代表已进入线上0.9.70，应用版本、C2机器合同及已发布安装包不变。页面统一为先只填写车辆展示名称创建，再进入原详情编辑态补充其他资料。

本次正式发布支持原始 0.9.69 通过设置页“检查更新”升级到 0.9.70，无需强制覆盖安装；最低 Updater 为 0.9.69，更早版本不在范围。正式 Windows 原始 Worker/Updater 的 paused、faulted 两种完整按钮场景及数据保留自动验收均通过；GitHub 已直接将同一签名包交付服务器。生产 Docker 后端、0.9.70 合同及新增两列迁移、运营后台已切换，业务数据和配置保留。新版本已登记，外网完整下载校验通过；用户本机业务验收仍待确认。正式包不经 Mac 中转，Fast UAT 既有流程不变。准确源码、包哈希、验证阶段和限制见版本更新记录 2.0.70.4。

本轮源码候选：Worker `0.9.70`，C2 合同 `0.9.70 / 9a41f79da6b9dea74d23a2cc0ac5963a259f03285cf42432f6223816eb92c287`；生成 Schema 与双仓来源同步。无效线索联动及重启收尾已通过独立复审，搜索清理维持已通过结论。以下 0.9.69 信息是既有发布状态；0.9.70 首轮构建已完成，但完整按钮门禁尚未通过，未部署或开放更新。

适用范围：运营后台、车金后端、Windows Worker、OmniAuto Sidecar、C0—C4、车辆 Product Master、知识管理、人工接管与飞书通知。

当前已发布正式客户端为 `0.9.69`，包含 `709b3239b0072325e880ac4747b405f6f121b974` 及严格旧协议兼容。
用户已反馈 0.9.67 → 0.9.69 按钮升级实机失败；本轮该路径判定不通过，停止推荐，改走保留绑定、配置和业务数据的强制安装升级，具体安装方案及实机结果待验证。运维执行约束见 `../rules/operations.rules.md`。
应用与 Updater 为 0.9.69；本次不改 C2 消息合同，保留 0.9.68 / 23c30e5a5cce3b2e0d5e697fe2733cf670d3c8cde04665c68997e9762977f1a3。
...
**实施状态：按先文档、后代码完成本地实现；38 项发布回归、50 项打包/验收工具测试及 actionlint 工作流静态检查通过。尚未提交、推送，未在 GitHub Windows 验证或启用。** 本节替代流程 B 必须先生成 Fast UAT ZIP 的旧前置条件；历史运行记录原样保留。已发布 0.9.70 不重新打包、不替换、不部署。
...
**执行入口（代码落地并验证后使用）**：新构建使用 `delivery_mode=build_and_stage`；仅验收工具修正使用 `delivery_mode=retest_candidate` 和 `candidate_run_id=<原构建运行号>`；已验收包传输失败使用 `stage_existing` 与 `formal_run_id=<验收通过的运行号>`；`check_staged`/`publish_staged` 延续原生产就绪和授权要求。本轮继续只以原始 0.9.69 为已配置升级起点；新增起点需补真实旧包身份和验收，不凭输入版本号放行。
...
CURRENT_CLIENT_VERSION=<此次实机升级前的精确版本，例如 0.9.67>
