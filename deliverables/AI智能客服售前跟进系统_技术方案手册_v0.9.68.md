...
版本：v0.9.70.15（车辆新增先填名称、详情补资料口径同步；本地实现待复审收口及集成，既有发布状态不变）

日期：2026-07-21

最后更新：2026-09-08

本次文档定义 9.1.1“车辆能源类型”、9.1.2“车系下拉分类”、9.1.3“排量输入字段”、9.1.4“电池包容量”及 9.1.5“驱动方式”。本地实现与主体复审已完成，待 P2 收口复审及双仓集成，不代表已进入线上 0.9.70；不修改应用版本、C2 机器合同、来源 SHA 或已发布安装包。页面统一为先只填写车辆展示名称创建，再进入原详情编辑态补充其他资料。
...
| AI可见车源字段 | 品牌、车系、车型、排量、电池包容量及驱动方式（本地已实现、待集成，见9.1.3—9.1.5）、年份、里程、城市、颜色、燃料、配置摘要、对外可说价格、车辆图片。 |
...
#### 9.1.1 车辆能源类型（2026-09-08 新增，本地已实现，待集成）
...
#### 9.1.2 车系改为固定下拉分类（2026-09-08 新增，本地已实现，待集成）
...
- **Excel**：与9.1.1共用同一 `1.2` 模板（本地已实现，待集成），保留“车系”原列，只更新字段说明和允许值为九项中文；
...
#### 9.1.3 排量输入字段（2026-09-08 新增，本地已实现，待集成）
...
- **Excel**：与9.1.1—9.1.2共用 `1.2` 模板（本地已实现，待集成），在“能源类型”后增加“排量”文本列，示例与页面一致。
...
#### 9.1.4 电池包容量输入字段（2026-09-08 新增，本地已实现，待集成）
...
- **Excel**：与9.1.1—9.1.3共用 `1.2` 模板（本地已实现，待集成），在“排量”后增加“电池包容量（kWh）”列，
...
#### 9.1.5 驱动方式下拉字段（2026-09-08 新增，本地已实现，待集成）
...
- **Excel**：与9.1.1—9.1.4共用 `1.2` 模板（本地已实现，待集成），在“电池包容量（kWh）”后增加“驱动方式”列。
...
本节是后续灰度版本的固定执行口径。每次发布填写当前客户端版本、目标版本、车金源码提交、OmniAuto 来源提交、Fast UAT
运行号和包哈希。默认执行生产备份与回滚准备；用户明确批准的生产备份/回滚例外必须逐次记入版本记录，不能默认为后续版本的永久豁免。
...
构建前检查：精确已推送提交的独立构建检出必须干净，来源登记完整，合同 revision/SHA 一致，Fast UAT 对该提交通过。
