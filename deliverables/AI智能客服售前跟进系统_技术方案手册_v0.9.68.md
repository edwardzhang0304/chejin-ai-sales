...
版本：v0.9.72.5（0.9.72正式包与后端已交付；现场强升恢复待验收）
...
历史源码候选记录：本次补齐 CJNPTTDE 复审缺口：旧错误读取回执须经同一 Flow 的后端只读证明才能修正；自动生成测试先观察真实产物；历史回放包含前序事实与已确认发送。头像排除与新回执透传保留。本轮登记范围已通过独立源码复审；用户指定继续双仓灰度分支，目标应用、机器合同与Schema统一0.9.72。已完成双仓灰度源码提交推送及真实来源登记（见版本记录2.0.72.2）；未发包或部署，正式Windows验收仍待完成；已发布0.9.71及回滚基线保持。
...
除 15.6“重复耗时统计清理”和 15.8“客户端手动检查更新”经确认共同提前进入灰度 `0.9.59` 外，其余 P1 和 P2 固定进入
正式上线后的 `1.1.x` 独立优化系列，不再占用 `0.9.x` 灰度修复序列，也不得回写 `1.0.x`
正式稳定系列。`0.9.59` 只允许实施 15.6 和 15.8，不得顺带带入下列其他优化；后续每个实现候选使用
开发时下一个未占用的精确 `1.1.n`，禁止覆盖已经形成的候选或把多项未经独立验证的优化捆绑发布。

后续 P1 范围固定为：

1. 语音转写性能：只优化 prepare/execute 中等价帧的重复 OCR、右键菜单 ROI 和已满足条件后的
   多余等待；不得减少身份跟踪帧、改变正式 action ID、Journal 终态或歧义失败关闭。
2. 加好友 UI 执行性能：保留 `0.9.22/0.9.24` 已完成的启动标定、坐标地图和无 UI 变化时的
   同帧截图复用，只继续优化搜索框、资料页、邀请表单和确认后页面的 ROI OCR，以及“状态已
   明确出现即可继续”的有上限等待；不得重做 C1 主链，不得删除字段复核、唯一联系人确认、最终确认或页面收尾。
3. Brain 修复链：保留 `0.9.41` 已完成的固定路由、共享总预算和阶段诊断，基于真实批次提高首次输出
   满足现有合同和 Guard 的概率，减少进入修复链的比例；不得删除
...
2. C4 性能只在取得独立真实召回链路样本后再评估；不得用 C2/C3 样本推算或直接套用优化。
