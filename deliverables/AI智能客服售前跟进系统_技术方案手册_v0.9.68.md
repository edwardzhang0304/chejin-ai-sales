...
版本：v0.9.70.3（正式数据门禁修正；仅验收 0.9.69 → 0.9.70，未部署）

日期：2026-07-21

最后更新：2026-09-08

本次正式发布仅覆盖原始 0.9.69 → 0.9.70，最低 Updater 为 0.9.69，更早版本不在范围。第二轮真实按钮流程已返回升级成功，但最终外部数据断言失败，尚未放行。运行控制行在升级时会正常写入；修正后分开验证暂停/流程/升级门状态与必须精确保留的业务配置和历史数据，并保存三个合成数据检查点。原始更新器的全字段冻结基线与验签保持不变。重新通过两种状态的完整 Windows 门禁后，才配套切换 0.9.70 合同及后端两列迁移、登记更新。生产数据和配置保留，正式包不经 Mac 中转；Fast UAT 流程保持不变。具体身份与进度见版本更新记录 2.0.70.3。
