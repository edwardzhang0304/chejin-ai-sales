...
版本：v0.9.69.5（客户端 0.9.69 正式更新已发布；Windows 本机验收待用户完成）
...
用户明确要求现有 0.9.67 从设置页“检查更新”直接进入 0.9.69，保留绑定、配置和本地业务数据；不改为手动安装。
...
该规则已完成源码实现和本机复审，Windows 正式 EXE 仍待验收，不代表已发布 0.9.68 包已修复。不得覆盖现有标签、安装包或预填新来源 SHA。
...
`UPDATE_MANUAL_UPGRADE_REQUIRED`，再由运维说明保留数据安装方案。本次 0.9.67 / 0.9.68 → 0.9.69 使用下述已通过真实 EXE 自动验收的严格兼容入口，正常路径仍为按钮升级；
不得关闭数据检查强行过渡，也不得临时下载未经过发布签名验证的更新器。
不得覆盖已发布 `0.9.68` 的标签/包；后续功能候选版本和真实来源按既有发布治理确定。

**0.9.67 → 0.9.69 严格兼容补充（用户确认按钮升级目标）**：增加旧协议入口，兼容 0.9.67 和 0.9.68；两个版本的交接代码逐字节相同。
...
10. 正式构建必须先验证包 manifest、SHA、数字签名、Updater 版本和 `rollback_safe=true`，再发布后端记录。
   包、数据库发布记录和 Git 标签必须指向本次实际发布的同一个不可变候选，不能复用历史版本号覆盖旧包。
...
Windows 构建环境真实执行，Mac/Linux 的 mock 不能代替。本地与 Windows 门禁全部通过后才能声称自动更新可交付。
...
恢复业务前，客户端和后端必须使用同一个精确版本与合同。

已有 0.9.59 及以上正式客户端使用设置页“检查更新”完成升级；0.9.67 → 0.9.68 属于此路径。
运维仍需构建并发布签名完整 ZIP，客户端自动下载该 ZIP、验签、安装和重启，用户不需要另行下载安装包或覆盖程序目录。
Fast UAT ZIP 不登记到正式更新渠道，也不能替代正式升级包。新电脑和没有更新功能的旧客户端才使用首次人工安装路径。
...
流程 B 完成后仍需执行 Windows 实机验收；实机验收不反向修改流程 A 的结果。
...
### 18.0.1 本次 0.9.69 正式按钮升级

- **目标/环境**：生产 API `https://jiangsuchejin.com/api`；Windows 0.9.67 → 0.9.69。
  GitHub Windows runner 构建，正式产物由 GitHub → 生产隔离目录直传，Mac 不转发 ZIP。
- **检查清单**：固定源码 SHA；源码兼容回归、凭据安全、完整正式构建、原生锁、新旧实际 Updater
  与本次真实 Worker EXE 门禁通过；真实旧包来源/ZIP SHA/Updater SHA 固定，不能用重编译旧更新器代替。
- **执行**：`delivery_mode=build_and_stage`，`current_client_version=0.9.67`；校验产物后，
  确认生产健康、合同 SHA 和 Worker 暂停空闲、用户本机队列排空，再 `check_staged` / `publish_staged`。
  用户只需原客户端点击“检查更新”，不需另行下载或解压。
- **数据/恢复**：保留现有程序恢复能力及全部绑定、配置、业务数据；本次不执行历史清库或日志删除指令。
  失败不得覆盖数据或补写成功；保持暂停/故障并保留诊断，按原程序恢复流程处理。
- **合同与生产**：C2 revision 不随本机升级协议变化；按签名 ZIP 中实际合同 revision/SHA 与生产核对，
  不用应用版本推算合同。无需为本次客户端修复重建后端/运营后台或改动 DeepSeek 配置。
- **验证/观察**：旧版发现 0.9.69、原公钥验签、Range/完整下载校验通过；Windows 自动验收与
  用户本机验收分别记录。观察升级结果、绑定、配置、心跳、C2 受影响业务以及停机失败后的状态。
- **风险**：0.9.67 本身的退出后续写入无法被新包提前修改；严格校验发现差异时安全失败，不保证强行升级成功。
  原始包验收中的旧 Worker 停机由受控进程模拟，真实 Windows “检查更新”界面与微信业务由用户完成最终验收。
...
1. `current_version=${CURRENT_CLIENT_VERSION}`、`platform=windows-x64`、`channel=gray` 返回 `update_available=true` 和目标版本；本次起点为 `0.9.67`，目标为 `0.9.68`；
...
Windows 实机仍必须由验收人执行：在现有 0.9.67 正式客户端设置页点击一次“检查更新”（该点击就是本次安装授权），
确认目标 0.9.68，观察下载、校验、安全边界等待、安装和自动重启。不得另行解压覆盖旧目录或清空绑定和本地业务数据。
