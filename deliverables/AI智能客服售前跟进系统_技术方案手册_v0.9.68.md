...
版本：v0.9.70.2（正式门禁工具修正；仅验收 0.9.69 → 0.9.70，未部署）

日期：2026-07-21

最后更新：2026-09-08

本次正式发布仅覆盖原始 0.9.69 → 0.9.70，不再要求兼容更早版本，最低 Updater 为 0.9.69。正式构建前补齐流水线目标与服务器验签基线，开放更新前必须使用原始旧客户端“检查更新”入口完成本次签名 ZIP 的完整 Windows 门禁；首轮在自动化驱动连接 Qt 时失败，尚未点击按钮；已改用 Qt 页面调试通道并新增构建前驱动检查，完整升级待重跑。0.9.70 合同及后端两列迁移必须配套切换，不能只登记客户端包。生产配置和数据保持，正式包不经 Mac 中转；具体身份与执行进度见版本更新记录 2.0.70.2。
