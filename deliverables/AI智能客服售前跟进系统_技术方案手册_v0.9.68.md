...
版本：v0.9.68.4（文档修订；应用与合同版本仍为 0.9.68；正式交付自动化及更新数据留底整改）
...
当前开发目标为 `0.9.68`，继承已发布 `0.9.67` 与灰度基线 `cef74f3`。
本轮修复 Brain 将指导性知识引用、付款需求询问及多段回复误判为事实承诺的问题；
保留各段事实校验、正式来源成员检查和原安全门禁，不改变 Worker、媒体、发送、重试与结算主流程。
前两轮 P1 已通过独立复审关闭，双仓已提交推送；用户后续授权按正式发布流程从现有 0.9.67 客户端升级到 0.9.68。
当前打包、生产发布及实机验收状态只以版本更新记录为准；本文的流程修订不代表发布或验收已完成。
