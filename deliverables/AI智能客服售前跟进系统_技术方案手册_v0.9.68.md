...
版本：v0.9.69.3（客户端 0.9.69 正式候选；0.9.67 按钮升级兼容；验收状态见版本记录）
...
当前客户端候选为 `0.9.69`，包含 `709b3239b0072325e880ac4747b405f6f121b974` 及严格旧协议兼容。
...
发布前必须用线上旧版本真实包验证进入修复版的路径；不支持新协议时，自动更新应明确返回
`UPDATE_MANUAL_UPGRADE_REQUIRED`，由运维在安全退出后按受支持的保留数据安装方式交付修复版，
