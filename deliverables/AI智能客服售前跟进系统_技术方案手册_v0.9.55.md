...
当前版本完整继承 `0.9.54` 已冻结的 C0—C4、媒体编排、权威历史、Brain 上下文桥、失败终态和隔离 Brain UTF-8 字节通信。本轮只把 GitHub Windows 灰度门禁自身的 Python 控制台输出固定为 UTF-8，使 209 项 Brain 合同报告中的中文可以稳定打印；不得修改 Brain 判断、Provider 路由、历史窗口、状态机、媒体流程或 Handoff。生产能力仍以 `gray-v0.9.43`（`b6ad192`）为原始稳定底座。本文及配套全流程图是 `0.9.55` 唯一有效技术口径；`0.9.54` 文档保留为历史版本，不得反向覆盖本节。
...
**实现状态：** `0.9.53` 的 MessageEvent 不可变快照、Adapter 上下文桥、普通/快速 Brain 同源历史、固定语义指令和 `AI_CONTEXT_BUILD_FAILED` 终态已经通过架构复审；`0.9.54` 已将隔离 Brain 请求/结果固定为 UTF-8 字节协议，并用强制 ASCII 子进程反向测试锁定产品边界。`0.9.55` 只修正 Windows 灰度门禁的报告输出编码，不改变上述生产实现。机器合同与生成 Schema 规范化 SHA 为 `c290881a76f9701fac15221e48898c79101077b20bc07f937d1138a5d334e83f`。独立 OmniAuto 真实来源提交为 `e65e363116791e43e1dd5da5a55c31c060237e09`，`.chejin-source.json` 已完成绑定；车金候选提交、推送、标签、ZIP和部署结论仍须按发布流程真实形成。本轮不涉及 Windows OCR/鼠标，但必须通过 GitHub Windows 灰度门禁；整体版本发布仍保留 Windows UAT 门禁。
...
6. 对外 HTTP 接口的唯一正式名称固定为“接口编号 + HTTP 方法 + 从 `/api` 开始的
   ...
   同义名称。
7. 灰度版本使用唯一 `0.9.x` 序列：`0.9.0` 至 `0.9.54` 已冻结，当前技术方案目标候选为 `0.9.55`；
   ...
   灰度发布版本。`0.9.41` 至 `0.9.54` 已形成不可覆盖的代码、合同或发布证据；当前候选实现 `0.9.55 / c290881a76f9701fac15221e48898c79101077b20bc07f937d1138a5d334e83f` 已完成本地定向验证。客户端、后端、OmniAuto 生成 Schema、manifest 与打包入口已统一升级为 `contract_revision=0.9.55`；OmniAuto 来源提交已真实形成为 `e65e363116791e43e1dd5da5a55c31c060237e09`，车金候选提交、GitHub Windows 门禁、Windows 实机 UAT 和正式包仍待真实形成。
...
本文候选机器合同 revision 为 `0.9.55`，规范化 SHA 为 `c290881a76f9701fac15221e48898c79101077b20bc07f937d1138a5d334e83f`；代码和生成 Schema 已完整继承 0.9.53 的 Brain 上下文桥与 0.9.54 的 Windows 隔离子进程 UTF-8 字节通信，并仅修正 Windows 灰度门禁的报告输出编码。独立 OmniAuto 来源提交已形成，但在车金提交、门禁和 ZIP 真实完成前，不得写成已发布或已通过 Windows UAT：
...
**实现状态补充：** `0.9.43` 仍是生产能力的父基线，`0.9.44` 至 `0.9.54` 均为不可覆盖历史版本。`0.9.55 / c290881a76f9701fac15221e48898c79101077b20bc07f937d1138a5d334e83f` 已形成本地实现并通过受影响定向测试；`.chejin-source.json` 已绑定新的独立 OmniAuto 来源 `e65e363116791e43e1dd5da5a55c31c060237e09`。车金候选提交和发布产物尚未形成。
