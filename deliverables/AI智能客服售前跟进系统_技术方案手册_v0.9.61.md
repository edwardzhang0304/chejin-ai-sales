...
当前唯一架构口径完整继承 `0.9.60` 已集成的代码瘦身和三条 Brain 主生成路径 `max_tokens=8192`。`0.9.61` 只新增运营后台知识管理：运营人员查看知识、新增或编辑并保存草稿、从已保存草稿单独发布、归档、查看发布记录和回滚；发布形成不可变知识快照，只影响发布后新创建的 Brain 批次。Windows Worker、C2 消息合同、微信操作、媒体处理、Handoff 与 C0—C4 状态机均不改变。当前实现已通过架构复审和受影响发布门禁；版本、机器合同和来源记录已按真实代码结果统一生成，提交、推送、Windows 打包和服务端部署按本次正式发布流程执行。
...
6. 对外 HTTP 接口的唯一正式名称固定为“接口编号 + HTTP 方法 + 从 `/api` 开始的
   ...
   同义名称。
7. 灰度版本使用唯一 `0.9.x` 序列：`0.9.0` 至 `0.9.60` 已冻结，当前目标候选为 `0.9.61`；
   ...
   灰度发布版本；当前目标为 `contract_revision=0.9.61`，规范化 SHA 必须在客户端、后端发布清单、
...
8. 灰度稳定分支固定为 `codex/gray-release-0.9.x`，只接收当前灰度版本的缺陷修复、合同同步、
   测试和发布治理；经确认提前进入灰度的优化只有 15.6 耗时观测清理和
   15.8 客户端手动检查更新，均已在 `0.9.59` 完成；15.9 代码瘦身和 15.10 Brain 主生成额度修复已在 `0.9.60` 完成。
   `0.9.61` 只额外接收本章定义的知识管理，不得借此带入 AI 回复效果优化、经验池、Prompt 管理或其他下一期功能。每个不可变灰度候选使用精确标签
...
-> Sidecar取得每个OCR行的显式同行头像证据和弱几何诊断
...
5. 同一截图中，同一头像组件被相邻 OCR 裁剪重复观察时仍是同一个帧内锚点，不得仅因多次检测
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `contract_version` | integer | 是 | 协议结构代号固定为 `3`，不是发布版本；目标灰度版本由 `contract_revision=0.9.61` 表达，并同时校验最终重新生成的 `contract_sha256 / observation_schema_version`。 |
