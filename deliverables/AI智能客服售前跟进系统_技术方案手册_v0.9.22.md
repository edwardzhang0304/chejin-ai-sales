...
OmniAuto 是布局识别和物理坐标的唯一权威。Worker 只负责启动时调用一次窗口规范化、校验布局合同并
串行执行，不得计算侧栏宽度、窗口偏移或点击坐标；后端不接收、不推断也不保存 UI 坐标决策。

#### 16.11.2 窗口规范化门禁

1. Worker 客户端启动并绑定后，必须通过同一个车金 Sidecar 入口对微信主窗口执行一次
   前置检查：确认唯一可见主 HWND、非最小化、非离屏、无阻塞弹窗，并在屏幕工作区能够容纳时
   恢复为非最大化窗口、移动到安全位置、调整到按工作区和当前 DPI 计算出的标准尺寸；随后通过
   被动状态探测确认微信已经登录。只有未找到微信窗口时允许继续等待并重试这一次规范化机会。
2. 每个新的 `add_friend / authorized_read / pre_send_refresh / send` UI Flow 在取得 UI 锁后、
   第一次业务截图前必须以 `window-policy=verify` 复核 HWND、窗口外框、客户区和 DPI；该阶段禁止
   再次移动或缩放微信。所有直接启动 Sidecar、Connector 和 Vision worker 的车金入口必须遵循
   同一策略，禁止某条入口重新规范化、另一条入口只复核。
...
4. 正在执行的 Flow 不得为了“对齐”而在任意步骤中途移动窗口。若动作前发现 HWND、窗口外框、
   ...
   以明确错误零点击停止，已经触发媒体/发送动作时按原 Journal 终态结算。
5. `status/heartbeat` 等被动探测不得移动或调整微信窗口，也不得生成可供后续点击复用的坐标。
6. 启动规范化不得执行完整布局识别，也不得因搜索栏、侧栏、聊天区或输入区暂时无法识别而阻止
   客户端启动。完整截图、OCR、动态布局和点击前安全检查只在真正的业务动作开始后执行。
...
联合确定。相同左侧导航边界后若同时出现侧栏分隔线和更靠右的聊天面板竖线，必须选择第一条
满足完整结构约束的侧栏分隔线；若左侧导航边界本身存在两个近似可信结果，则属于不可消除歧义。
所有必需区域必须非空、不重叠越界、保持正确包含关系并得到唯一结果；多个导航边界候选、
...
加好友入口和 C2/C3 完整布局都必须先取得动态侧栏最上方的唯一“搜索” OCR 锚点。若多行对齐头像
形成的竖边像素强于真实导航分隔线，三条路径必须共用同一规则：用该锚点的当前文字边界排除穿过
或紧贴文字的候选分隔线，再从剩余的当前帧像素候选中选择导航和侧栏边界；不得只修加好友局部
布局、取消搜索锚点，或用固定导航宽度和历史坐标兜底。C2 完整 OCR 有结果但布局仍无效时，必须
返回类型化 `WECHAT_UI_LAYOUT_UNRESOLVED / sessions_layout_unresolved` 以及真实置信度和冲突；不得把
布局失败转换成成功的空会话扫描，也不得继续上报绑定候选。
...
截图与 OCR 的唯一职责划分如下，禁止混用：

1. 只有对当前精确微信 HWND 取得、且能证明真实屏幕原点的“可见窗口截图”可以建立可执行布局；
2. 全窗口 OCR 负责当前帧的结构、标题、消息和基础对象观察，是本帧主观察来源；
3. 增强 OCR 只能裁剪这张同一不可变截图中已经由布局快照确认的 ROI（如侧栏或文字气泡），
   增强后必须按裁剪偏移和缩放比例映射回原图坐标；它只补充文字识别，不得另建布局快照、
   不得改变 capture origin，也不得单独授权点击；
4. `PrintWindow`、桌面全屏、离屏或原点未知截图只可用于诊断。桌面全屏截图不能替代精确 HWND
   截图；一旦取得新截图或发生 UI 动作，旧快照立即失效；
5. S0/S1/S2 仍是三个独立时间点的新帧，任何 ROI 或增强 OCR 结果都不得跨时间点复用。
...
点击点。合成图矩阵只证明结构算法、失败关闭和坐标换算，不得声称已经证明真实微信视觉兼容。
本机自动化至少覆盖 1920x1080/100%、2560x1440/125%、3840x2160/150%，以及窗口被移动、缩放、
...
5. 边界唯一、缺失、多候选、低置信、区域冲突、聊天面板附加竖线、窗口中途变化和截图原点未知
   均覆盖反向测试；
   失败时必须零点击，测试不得只断言错误码。
