...
[下载 Windows 0.9.80 正式包](https://update.jiangsuchejin.com/releases/0.9.80/chejin-worker-v0.9.80-windows-x64.zip) · [SHA256 校验文件](https://update.jiangsuchejin.com/releases/0.9.80/chejin-worker-v0.9.80-windows-x64.sha256.txt)

本次从实际运行的 0.9.78 手动安装 0.9.80（用户选择强升），不使用“检查更新”。旧客户端已正常退出。安装包需完整解压，不能只复制 EXE。

1. 在原来的 Windows 账号下下载正式包，并将整个 ZIP 解压到一个明确命名为 `车金-0.9.80` 的新程序目录。
2. 保留原客户端数据，特别是 `%LOCALAPPDATA%\CheJinWorker`。如果原来设置过 `CHEJIN_WORKER_HOME`，保持其值和目录不变。不要删除数据库、Outbox、绑定、配置或恢复记录。
3. 双击新目录中的 `CheJinWorkerClient.exe`；确认显示 **0.9.80**，原 Worker 和销售绑定正确。不要再打开旧程序或备份目录中的 EXE。
4. 若出现重新绑定页面或原数据消失，先退出，不要重新绑定或清空；核对 Windows 账号及实际数据目录。
5. 若存在旧待恢复消息，先让客户端完成原记录补交和收尾。暂停/故障不会因为换包就自动变成正在接单；确认无待恢复、待上传、未知发送或其他阻塞，并且界面允许开始后，再由高磊明确点击“开始接单”。
6. 现场确认能够领取正常任务，并观察第一次实际微信操作及流程收尾。没有现场证据前，不把自动验收说成客户已经恢复。

Windows 自动验收已通过原装 0.9.78 的暂停安装、故障安装和合成待补交读取恢复；保护原数据、身份和停止接单状态。真实客户已结束 Flow 的具体现场恢复及实际微信发送仍待确认。圈＋落在旧识别区域外仍有已知漏检。

异常处理：保留原数据和错误提示，正常退出后排查。不自动回退程序、不恢复旧数据库，也不要删除恢复状态来绕过阻塞。
