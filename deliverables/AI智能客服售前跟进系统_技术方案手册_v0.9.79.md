...
文档修订：r8（2026-09-14；添加朋友标题误判已通过源码复审，按用户授权沿原灰度分支提交；应用/合同仍为0.9.79，未打包或部署）。
...
文档修订：r3（2026-09-14；圈＋识别及首屏证据源码提交，待后续统一发版；线上仍为 0.9.79）。
...
6. 对外 HTTP 接口的唯一正式名称固定为“接口编号 + HTTP 方法 + 从 `/api` 开始的
   ...
   同义名称。
7. 灰度版本使用唯一 `0.9.x` 序列：`0.9.0` 至 `0.9.67` 已冻结，当前目标候选为 `0.9.68`；
   ...
   灰度发布版本；当前目标为 `contract_revision=0.9.68`，规范化 SHA 必须在客户端、后端发布清单、
...
8. 灰度稳定分支固定为 `codex/gray-release-0.9.x`，只接收当前灰度版本的缺陷修复、合同同步、
   ...
   `0.9.68` 继承 `0.9.67`，只接收本轮 Brain 事实要求缺陷修复、对应测试和版本发布治理，不得带入确认次数优化、AI 回复效果优化或其他下一期功能。每个不可变灰度候选使用精确标签
...
当前实施结论：0.9.74 revision-3及FR映射已通过本次变更范围的源码独立复审；FR-01至FR-07按现有测试与源码核对登记，FR-08明确部分覆盖，Windows现场仍待验收。原revision-2证明重入/冲突出口反例在修复后通过，换回旧方法仍失败；详细证据和集成结果见版本记录2.0.74.5。以上最小规则不变，不增加平台、线程、按钮或1.1.x补发功能。0.9.74仅完成源码集成，本轮未发布；生产0.9.73与其现场安排保持。
...
**当前状态（v0.9.75.4）**：源码/工具凭证门禁代码3e8dff6及工具CI34576557998已验证；正式发布不再默认运行完整源码回归。下一次真实EXE全流程耗时待实测。下面2026-09-08的运行号只保留历史事实，当前步骤以本节v0.9.75.3/4条款为准。
