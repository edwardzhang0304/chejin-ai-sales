...
当前版本继承 `0.9.52` 已冻结的 C0—C4、媒体编排、未读代次、在途 Flow 互斥、故障恢复和发送安全规则，本轮只收口 C3 权威历史到 OmniAuto Brain 的唯一上下文桥、历史语义的唯一判断归属和上下文构建失败终态。生产能力仍以 `gray-v0.9.43`（`b6ad192`）为原始稳定底座。本文及配套全流程图是 `0.9.53` 唯一有效技术口径；`0.9.52` 文档保留为历史版本，不得反向覆盖本节。
...
**实现状态：** `0.9.53` 的 MessageEvent 不可变快照、Adapter 上下文桥、普通/快速 Brain 同源历史、固定语义指令和 `AI_CONTEXT_BUILD_FAILED` 终态已经通过架构复审；越权的车金关键词需求改口解析器已经删除，独立 OmniAuto 原生偏好逻辑保持 `0.9.52` 原实现。机器合同与生成 Schema 规范化 SHA 为 `5b2276513bfeebe759a0d095db3652ee7ba1b5c6b8c11ef0cf7da27a7bf26e4a`。独立 OmniAuto 真实来源提交为 `d1dc417a5a3c051622104485bc3c1bd3dd1840f3`，`.chejin-source.json` 已完成绑定；车金候选提交、推送、标签、ZIP和部署结论仍须按发布流程真实形成。本轮为纯后端数据链和 Brain 输入职责调整，不要求 Windows OCR/鼠标实机证明；整体版本发布仍保留 Windows UAT 门禁。
...
6. 对外 HTTP 接口的唯一正式名称固定为“接口编号 + HTTP 方法 + 从 `/api` 开始的
   ...
   同义名称。
7. 灰度版本使用唯一 `0.9.x` 序列：`0.9.0` 至 `0.9.52` 已冻结，当前技术方案目标候选为 `0.9.53`；
   ...
   灰度发布版本。`0.9.41` 至 `0.9.52` 已形成不可覆盖的代码、合同或发布证据；当前候选实现 `0.9.53 / 5b2276513bfeebe759a0d095db3652ee7ba1b5c6b8c11ef0cf7da27a7bf26e4a` 已通过架构复审。客户端、后端、OmniAuto 生成 Schema、manifest 与打包入口已统一升级为 `contract_revision=0.9.53`；OmniAuto 来源提交已真实形成为 `d1dc417a5a3c051622104485bc3c1bd3dd1840f3`，车金候选提交、GitHub Windows 门禁、Windows 实机 UAT 和正式包仍待真实形成。
...
本文候选机器合同 revision 为 `0.9.53`，规范化 SHA 为 `5b2276513bfeebe759a0d095db3652ee7ba1b5c6b8c11ef0cf7da27a7bf26e4a`；代码和生成 Schema 已按本轮 Brain 上下文桥规则实现、完成定向自动化验证并通过架构复审。独立 OmniAuto 来源提交已形成，但在车金提交、门禁和 ZIP 真实完成前，不得写成已发布或已通过 Windows UAT：
...
**实现状态补充：** `0.9.43` 仍是生产能力的父基线，`0.9.44` 至 `0.9.52` 均为不可覆盖历史版本。`0.9.53 / 5b2276513bfeebe759a0d095db3652ee7ba1b5c6b8c11ef0cf7da27a7bf26e4a` 已形成本地实现并通过受影响定向测试；`.chejin-source.json` 暂时保留 `0.9.52` 的真实 OmniAuto 来源，必须等新的独立提交形成后再更新。车金候选提交和发布产物尚未形成。
