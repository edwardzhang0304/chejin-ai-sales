...
当前版本继承不可变 `gray-v0.9.47`（`a8a75be`）；`0.9.47` 的业务代码和架构复审结论完整保留，本轮仅修复未进入 job 的灰度 CI 配置。其生产能力仍以 `gray-v0.9.43`（`b6ad192`）为原始稳定底座。历史版本的提交、合同 SHA、发布状态和变更原因只允许记录在《版本更新记录》，不在本文继续保留可能被误读为现行规则的旧实现说明。本文及配套全流程图是 `0.9.48` 唯一有效技术口径。
...
**实现状态：** `0.9.48` 完整保留 `0.9.47` 已通过架构复审的重启恢复与故障状态代码，只修正灰度工作流：`CHEJIN_WORKER_HOME` 在具体 Windows 测试 step 中通过 `$env:RUNNER_TEMP` 设置，不再在 GitHub 不允许 `runner` 上下文的 job 级 `env` 中求值。规范化合同为 `0.9.48 / 2cdf88e61310157fc058eeeee4a5300b52c53234a964737945e46ecd6c21f41b`；独立 OmniAuto 来源提交为 `3413c591357f6fd790f53b0f967ff6bc9c6de57f`，只同步生成合同 Schema。业务代码、正常文字/语音/图片/混合消息、Brain、媒体编排和后端业务接口未改变；自动化通过不代表 Windows 实机 UAT。
...
6. 对外 HTTP 接口的唯一正式名称固定为“接口编号 + HTTP 方法 + 从 `/api` 开始的
   ...
   同义名称。
7. 灰度版本使用唯一 `0.9.x` 序列：`0.9.0` 至 `0.9.45` 已冻结，当前目标候选为 `0.9.46`；
   ...
   灰度发布版本。`0.9.41` 至 `0.9.45` 已形成不可覆盖的代码、合同、标签或发布证据；当前目标为 `0.9.46`。客户端、后端、OmniAuto 生成 Schema、manifest 与打包入口最终必须统一使用 `contract_revision=0.9.46`；本地实现已真实计算规范化合同 SHA `5b7cc473f16062dd668a6880379b2375108985abd779f5dcb53deb628da5a371`。架构复审已通过，OmniAuto 来源提交已固定；车金候选提交、GitHub Windows 门禁、Windows 实机 UAT 和正式包仍待完成。
...
目标机器合同 revision 为 `0.9.46`。代码、Worker 本地 Outbox 深度门禁、有限技术失败结算、后端嵌套序列校验及生成 Schema 已按本文统一实现；规范化 SHA 已真实计算为 `5b7cc473f16062dd668a6880379b2375108985abd779f5dcb53deb628da5a371`。来源提交已固定且架构复审通过，但车金提交、打包、部署和 Windows UAT 尚未完成，不得把当前工作树写成已发布或已通过 Windows UAT：
...
**实现状态补充：** `0.9.43` 仍是生产能力的父基线，其规范化合同 SHA 和 OmniAuto 来源提交只用于校验基线真实性。`0.9.44` 关闭跨帧连续性与媒体动作结果绑定问题，`0.9.45` 完成 Windows 旧测试合同和 Pillow 兼容收口；两者均为不可覆盖历史版本。当前 `0.9.46` 只补齐 C2 嵌套序列合同、旧坏 Outbox 有限终态和后端深度校验，新合同 SHA 为 `5b7cc473f16062dd668a6880379b2375108985abd779f5dcb53deb628da5a371`；独立 OmniAuto 来源提交为 `69c69a60b7e08a31f9d773864b14b35faf86754b`，车金提交和发布产物仍须按来源治理顺序真实形成。
