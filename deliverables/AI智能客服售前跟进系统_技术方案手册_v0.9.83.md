...
文档版本：v0.9.83；合同基线：0.9.83；文档修订：r12（2026-09-16，纯数字语音转写归属；待复审，未发布）。

## 当前版本与阅读说明

本分支创建时的发布基线为 **0.9.83**，本轮分段修改尚未发布。灰度已继续演进，正式版本组合与发布状态应在集成时同步，不得用本分支旧口径覆盖灰度记录。
...
本次 r4 在独立开发分支增加 §7.10 长回复分段；源码与机器合同为开发态，尚未升候选版本、提交或部署。其余已发布状态保留原义。
