...
7. 灰度版本使用唯一 `0.9.x` 序列：`0.9.0` 至 `0.9.33` 已冻结，当前目标候选为 `0.9.34`；
   ...
   灰度发布版本；客户端、后端、OmniAuto、生成 Schema、manifest 与打包入口已在本地同步为 `contract_revision=0.9.34`，规范化合同 SHA 为 `151416413b4b846a60c46518345fc04a31517e6f2ab85d07e2ccad14e3ca939f`。当前代码与文档架构复审已通过，尚未形成真实 `0.9.34` 功能提交；Windows UAT、发布来源固定和正式包均未完成，不得伪报为已发布或已交付。
...
复审已通过，尚未形成真实 `0.9.34` 功能提交；`.chejin-source.json` 继续绑定已发布
`0.9.33` 的真实来源，不得预填未生成的新 SHA。下一步必须先形成真实
OmniAuto 功能提交，再更新来源记录并形成车金候选提交。Windows 实机验收和打包
