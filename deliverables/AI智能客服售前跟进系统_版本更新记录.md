...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | `codex/gray-release-0.9.x`；`0.9.31` 功能候选固定于 `3fd52f4e3088acdf5af63eff818fbbfdf9d7aa6e`，最终发布 HEAD 由来源绑定提交及包内 manifest 记录 |
| 本轮整改父基线 | `4ae792b`（`gray-v0.9.15`） |
| 灰度版本 | 目标候选 `0.9.31`；`0.9.0` 至 `0.9.30` 已冻结，`0.9.30` 因旧库恢复无有限出口已否决发布，旧 revision/SHA/标签/ZIP 不得覆盖 |
...
| 当前机器合同 | 已统一为 `contract_revision=0.9.31`，规范化 SHA `74b66af403e007a8739a4b22c18bbfa4d9ba0a38838150a15144e0337a5f7f26`；生成 Schema 已同步 |
| 来源治理 | `0.9.30` 候选提交 `5c94a91cd956f37776be73f872b4c34ccaccd810` 只作审计和回滚基线，禁止作为发布包；`0.9.31` 功能提交已固定为 `3fd52f4e3088acdf5af63eff818fbbfdf9d7aa6e`，`.chejin-source.json` 已绑定该真实提交 |
| PRD | v0.9.10 |
| 技术方案 | v0.9.28 权威方案已补齐 `legacy_media_recovery`；新格式运行时恢复与跨版本历史数据恢复分层，不再把历史字段缺失直接解释为无期限全局暂停 |
| 全流程图 | v0.9.28 源文件已增加旧记录一次分类、可证迁移、客户 handoff、Worker 级待审核事故和自动释放旧 Flow 的完整出口 |
| 发布状态 | `0.9.30` 禁止继续使用。`0.9.31` 功能复审通过，功能提交及 OmniAuto 来源绑定已完成；待发布门禁、推送标签、生成 ZIP 和合同匹配部署。Windows 原故障 SQLite 重启后实际领取新任务仍是现场交付门禁 |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.30` 已冻结，当前目标候选为 `0.9.31`；后续每个内容不同且进入测试的
