...
版本：v0.9.70.4（正式数据门禁修正；仅验收 0.9.69 → 0.9.70，未部署或开放更新）
...
| 项目 | 当前值 |
|---|---|
| 发布分支 / 目标 | `codex/gray-release-0.9.x`；Windows 0.9.70、配套生产 Docker 后端及运营后台 |
| 应用源码 / 登记 | `30473b3423fa16f81771718c4c39b5a16058020d` / `1646a66d4e6b6b2e4b018c55498545dde3a2be02` |
| OmniAuto 来源 | `fd2c9d7700bac78710022b2a0c3579537395fc3e`，用户 fork 已推送 |
| 支持起点 | 原始正式 0.9.69；最低 Updater 0.9.69；更早版本不在本轮兼容范围 |
| 最近正式运行 | `34177422452`，提交 `8e1c16d58bddd2e06de7a429d5798d56270511fb`，同提交 Fast UAT `34176768668` 成功 |
| 构建 / 完整按钮验收 | 构建及原始更新器返回成功；外部数据断言失败，整条正式流水线失败，不能放行 |
| 生产后端 / 合同 | 尚为 `chejin-api:v0.9.68-82b5988`、合同 0.9.68、迁移 0033；目标合同 0.9.70 必须配套切换 |
| 线上更新 | 尚无 0.9.70 登记；生产 0.9.69 查询无新版本 |
| 当前结论 | 修正外部数据检查后重跑完整 Windows 验收；不建议用户点击更新，尚未判断需要强升 |
| 数据 / 配置约束 | 保留业务数据、账号、配置和绑定；沿用免备份及不自动生产回滚授权，不执行历史清理 |
