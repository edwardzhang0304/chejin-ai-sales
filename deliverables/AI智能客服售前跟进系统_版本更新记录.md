...
版本：v0.9.64（运维发布登记 v0.1；生产测试灰度已部署，Windows 实机验收待完成）
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | 原灰度分支 `codex/gray-release-0.9.x`；正式包与部署源码固定为 `d9132bc79d82c4e907f2ba7a6a8a4c0c09d5112e`，不可变标签 `gray-v0.9.64` |
...
| 已发布父基线合同 | `0.9.63 / 18a82d17f94118c56e0f1497df6ff94a828a7db9daa3b9920d1dd9b9e136240e`；保留为回滚基线，未改写旧标签或包 |
...
| 发布状态 | 用户后续明确授权正式构建、生产同版部署与更新登记，再在 Windows 实机测试。后端、后台及 `gray/windows-x64` 更新渠道已发布 `0.9.64`；Windows 实际升级和业务 UAT 待完成，不记为已通过 |

### 2.0.64 2026-09-04：0.9.63 发送后假头像事故——0.9.64 实现候选

#### 运维发布登记 v0.1（2026-09-04，当前）
