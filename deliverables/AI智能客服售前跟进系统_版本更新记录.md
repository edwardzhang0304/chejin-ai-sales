...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.0.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.0.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.0.puml`
...
| 灰度版本 | `0.9.0`；这是 `0.9.x` 灰度系列的首个不可变候选 |
| 灰度分支 | `codex/gray-release-0.9.x`；从完整候选收口后建立，禁止混入下一期功能 |
| 当前机器合同 | `contract_revision=0.9.0`；Sidecar、Worker、后端校验、生成 Schema、样例、manifest 和测试已同步，规范化合同 SHA 为 `eff3a2b2550d92ee48c4bcdb99afae5f0924578549e7cda0afc26ac8097daaa7` |
| 来源治理 | 版本统一后的独立 OmniAuto 本地固定提交为 `156d1a9c4384de90dc226c65b23b1b701e25878a`；内嵌生成 Schema 与该提交一致，`.chejin-source.json` 已同步真实来源哈希，待架构复审后再单独授权推送 |
| PRD | v0.9.0 |
| 技术方案 | v0.9.0 |
| 全流程图 | v0.9.0 源文件 |
| 发布状态 | 统一版本号的代码、合同与 OmniAuto 本地固定提交已完成；车金灰度候选待本轮定向门禁后形成提交并交架构复审。尚未推送、打包或进行 Windows 灰度验证 |
