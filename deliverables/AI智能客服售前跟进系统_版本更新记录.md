...
| 灰度版本 | 目标候选 `0.9.27`；`0.9.0` 至 `0.9.26` 已冻结，旧 revision/SHA/标签/ZIP 不得覆盖 |
...
| 当前机器合同 | `contract_revision=0.9.27`，规范化 SHA `ceac5e078db5516d173f33eb1af3c461b07251e667f3a0be0524268771ed2bd6`；生成 Schema 已同步且 current 检查通过 |
| 来源治理 | `0.9.27` OmniAuto/Worker 功能候选提交 `b12591b54630ed3949a997cfece3f8c6fe164700`；`.chejin-source.json` 已绑定该真实候选，最终集成提交由 Git 标签和包内 manifest 固定 |
| PRD | v0.9.10 |
...
| 发布状态 | `0.9.27` 保留 `0.9.26` 全部功能修复，并关闭 Windows 打包门禁暴露的测试快照绑定缺陷；允许推送并重新生成仅供实机验收的 Fast UAT ZIP。Windows 实际语音转写、正式发布门禁和客户可交付包仍待完成，不得将 Fast UAT ZIP 声称为正式包 |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.26` 已冻结，当前目标候选为 `0.9.27`；后续每个内容不同且进入测试的
