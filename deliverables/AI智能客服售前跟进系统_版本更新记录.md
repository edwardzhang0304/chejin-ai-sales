...
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.34.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.34.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | `codex/gray-release-0.9.x`；`0.9.33` 已打 `gray-v0.9.33` 标签并生成过 ZIP，不得覆盖；`0.9.34` 为当前本地整改候选，尚未形成功能提交 |
| 本轮整改父基线 | `4ae792b`（`gray-v0.9.15`） |
| 灰度版本 | 目标候选 `0.9.34`；`0.9.0` 至 `0.9.33` 均不得被同名覆盖；本轮只修正 Sidecar 历史返回对象泄漏 Worker 专属消息身份字段的合同边界 |
...
| 当前机器合同 | Worker、Sidecar、后端和生成 Schema 已在本地统一为 `contract_revision=0.9.34`，规范化 SHA `151416413b4b846a60c46518345fc04a31517e6f2ab85d07e2ccad14e3ca939f`；禁止与 `0.9.33` 混跑 |
| 来源治理 | `.chejin-source.json` 仍正确绑定已发布 `0.9.33` 功能提交 `61606d645de2c575ad3113de577e0f90d87a41f0`；`0.9.34` 必须先形成真实 OmniAuto 功能提交，再更新来源记录 |
| PRD | v0.9.10 |
| 技术方案 | v0.9.34 是当前唯一技术口径；不改业务流程，补齐四类 Sidecar 公开输出的递归清理与 Worker 拒绝门禁 |
| 全流程图 | v0.9.34 为当前唯一流程图；保持 Sidecar → Worker → 后端原权限和顺序，显式补齐公开输出边界 |
| 发布状态 | `0.9.33` 已有标签和 ZIP，本次修复不得覆盖。`0.9.34` 代码、四入口回归和文档架构复审已通过，尚未提交、推送、标签、打包或 Windows UAT |
...
- 当前本地候选已通过架构复审。下一步先形成真实 OmniAuto 功能提交，再更新 `.chejin-source.json`；不得使用 `0.9.33` 标签或旧 ZIP 验证本次修复。
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.33` 已冻结且不得同名覆盖，当前目标候选为 `0.9.34`；后续每个内容不同且进入测试的
