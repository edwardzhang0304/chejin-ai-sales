...
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.53.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.53.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | 灰度分支 `codex/gray-release-0.9.x` 的隔离工作区；继承 `0.9.52` 已冻结能力，Brain 权威历史桥和上下文失败终态已通过架构复审；越权的车金关键词需求解析器已删除 |
| 本轮整改父基线 | `0.9.52` 已冻结本地能力及真实 OmniAuto 来源 `80b79835f181201e80cb2f58f06df2c515f1dc56`；原始生产能力底座仍为 `b6ad192`（`gray-v0.9.43`） |
| 灰度版本 | 本地候选 `0.9.53`；机器合同和生成 Schema 已同步，独立 OmniAuto 来源提交已形成，车金提交、推送和发布待本次来源治理完成 |
...
| 当前机器合同 | 本地候选 `contract_revision=0.9.53`；规范化 SHA `5b2276513bfeebe759a0d095db3652ee7ba1b5c6b8c11ef0cf7da27a7bf26e4a` |
| 来源治理 | `.chejin-source.json` 暂时保留 `0.9.52` 独立 OmniAuto 真实提交 `80b79835f181201e80cb2f58f06df2c515f1dc56`；必须等本轮形成新的真实 OmniAuto 提交后再更新，禁止预填 |
| PRD | v0.9.50；本轮不改变业务需求，无需为技术互斥修复升版 |
| 技术方案 | v0.9.53 是当前唯一目标口径；MessageEvent 唯一历史、不可变快照、Adapter 唯一桥，客户需求语义统一由 Brain 从完整原话判断 |
| 全流程图 | v0.9.53；同步标明数据库到 Provider 的上下文桥、Brain 语义判断归属和失败终态 |
| 发布状态 | 架构复审已通过；独立 OmniAuto 来源提交已形成，车金集成提交、推送、标签、灰度 ZIP和后端部署待本次发布流程收口 |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.52` 已冻结且不得同名覆盖，当前技术方案目标候选为 `0.9.53`；后续每个内容不同且进入测试的
