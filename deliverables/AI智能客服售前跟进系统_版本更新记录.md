...
版本：v0.9.66.2（推送登记 v0.4；拆分 Fast UAT 本地流程与正式生产流程）
...
| 本轮版本 | `0.9.66`；用户独立复审通过，正式包和同版本后端已发布；Windows 物理操作验收待执行 |
...
| 生产 / Windows 记录 | 后端、后台及灰度更新渠道已登记 `0.9.66`；0.9.63 查询可发现 0.9.66，Windows 待实机升级 |
...
| 当前结论 | 双仓已提交推送，0.9.66 正式包门禁、签名、后端部署和更新登记均通过；PR #16 已关闭且 mergedAt=null，保留贡献说明。Windows 实机验收仍待完成 |
