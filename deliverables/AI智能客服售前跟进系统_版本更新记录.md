...
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.58.md`
3. 本版本更新记录
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | 灰度分支 `codex/gray-release-0.9.x`；基线为 `0.9.57` 功能提交 `f63613a51157c480a442e996ea8b211b3ba484fe`，`0.9.58` 本地实现未提交 |
| 本轮整改父基线 | `0.9.56` 提交 `3da3dc9ebde882d83d043cfa2c58458898981fe7`；原始生产能力底座仍为 `b6ad192`（`gray-v0.9.43`） |
| 灰度版本 | `0.9.58`；架构复审已通过，独立 OmniAuto 来源和来源登记已形成；车金提交、推送、标签和 ZIP 尚未形成 |
| 正式上线版本系列 | `1.0.x`；只接收通过灰度验收的正式上线能力和上线后的稳定性修复，不混入下一期优化 |
| 下一期优化版本系列 | `1.1.x`；承接本记录第 8 节除已提前到灰度 `0.9.58` 的耗时观测清理外的清单，不反向覆盖 `1.0.x` 正式产物 |
| 灰度分支 | `codex/gray-release-0.9.x`；只额外接收已批准提前的 `0.9.58` 耗时观测清理，禁止混入其他下一期功能 |
| 当前机器合同 | `contract_revision=0.9.58`；规范化 SHA `47185e439b6fced86dad8d8d607efc1efa4764775f0a19d5e629cb945bca2a10` |
| 来源治理 | `.chejin-source.json` 已绑定独立 OmniAuto 真实提交 `2a4a7ed9e459421c53865b0bbc9eeb9d88b3dd09`；本次只同步 `c2_contract_0_9_58_generated_schema`，不修改 OmniAuto 生产逻辑 |
| PRD | v0.9.50；本轮不改变业务需求，无需为技术互斥修复升版 |
| 技术方案 | v0.9.58；是当前唯一技术口径；v0.9.57 已恢复为不可变历史文件 |
| 全流程图 | v0.9.57；本轮不修改业务流程，不创建第二张流程图 |
| 发布状态 | 0.9.58 仅为本地候选；未提交、未推送、未打标签、未打包 |

### 2.0.57 2026-09-01：重复耗时统计清理目标候选 0.9.58
...
9. 耗时观测清理：该项已单独提前到灰度 `0.9.58`；完成新旧计时并行比对后，只删除重复输出、重复汇总
   和错误的父子阶段相加，保留底层单调计时和故障诊断数据。不得借此提前带入本清单的其他 `1.1.x` 优化。
