...
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.54.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.54.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | 灰度分支 `codex/gray-release-0.9.x` 的隔离工作区；完整继承 `0.9.53` 权威历史与 Brain 上下文桥，本轮只修 Windows 隔离子进程 UTF-8 JSON 通信 |
| 本轮整改父基线 | `0.9.53` 已冻结代码与标签，真实 OmniAuto 来源为 `d1dc417a5a3c051622104485bc3c1bd3dd1840f3`；原始生产能力底座仍为 `b6ad192`（`gray-v0.9.43`） |
| 灰度版本 | 本地候选 `0.9.54`；机器合同和生成 Schema 已同步，独立 OmniAuto 来源提交已形成，车金提交、推送和发布待本次来源治理完成 |
...
| 当前机器合同 | 本地候选 `contract_revision=0.9.54`；规范化 SHA `841cc44030d00e06bcd97da82ee2e4392ba12fe18f44bd4d0ae40e5f07be1d86` |
| 来源治理 | `.chejin-source.json` 已绑定独立 OmniAuto 真实提交 `b052bfa5a7333ad5651dc2de2e13ab10a74fe961`；两仓生成 Schema 逐字一致 |
| PRD | v0.9.50；本轮不改变业务需求，无需为技术互斥修复升版 |
| 技术方案 | v0.9.54 是当前唯一目标口径；完整继承 0.9.53，只增加 Windows 隔离 Brain UTF-8 管道约束 |
| 全流程图 | v0.9.54；C0—C4 不变，补充隔离子进程 UTF-8 通信边界 |
| 发布状态 | 0.9.53 Windows 灰度门禁发现中文结果输出受默认代码页影响；0.9.54 根因修复与来源提交已形成，车金提交、推送、标签、灰度 ZIP和后端部署待本次发布流程收口 |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.53` 已冻结且不得同名覆盖，当前技术方案目标候选为 `0.9.54`；后续每个内容不同且进入测试的
