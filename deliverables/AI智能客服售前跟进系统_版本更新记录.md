...
版本：v0.9.67.1（正式发布准备；Vision Key 代码复审通过，Windows 实机待验收）
...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.67.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.67.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.67.puml`
...
| 车金集成提交 | `6b4a2c8c236a97a2f47ec2a217bc6f0cd3878c53`（Vision Key 源码，已推送灰度分支） |
| 开发基线 | 已发布 `0.9.66` 与本次读取的灰度 HEAD `315bf41`；不重新修改头像和媒体流程 |
| 本轮版本 | 目标 `0.9.67`；代码复审通过，用户授权提交、推送；正式 EXE 与 Windows UAT 未执行 |
| 合同状态 | `0.9.67 / 49d9425a3166837621ff87bf50418509a518b779b793152b6e0a269a502e0e23` 已实际生成；`0.9.66 / 0f65d3a66514639a1768901f59af18cd35dfb43a7fed75ae211e479ba70ca490` 为上一发布登记 |
| 生产 / Windows 记录 | 已只读核对当前生产为 0.9.66，8 条知识、admin/ops 和绑定保留，Worker paused/idle，无任务租约；0.9.67 正式流水线与切换待执行，Windows 实机待验收 |
| 回滚材料 | 已生成并验证 0.9.65 回滚镜像、静态站点和数据库备份；真实路径保存在受控发布证据中 |
| OmniAuto 来源 | `621ca4795de38a51c579e8a389e2723374dabc23`，基于 b8804f2 仅同步 0.9.67 生成 Schema，已推送用户 fork |
| 正式版本车道 | `1.0.x` 正式上线及稳定性修复；`1.1.x` 下一期优化；本轮不扩范围 |
| 当前结论 | 用户已授权正式 Windows 发包、同版本生产后台/后端及更新渠道发布，并要求保留 Mac 本地安装包；发布执行中，未把 Windows 实机写为通过 |
...
2. 语音转写性能：只减少 prepare/execute 的等价帧重复 OCR、右键菜单 ROI 和已满足条件后的多余等待；
   不减少身份跟踪帧，不改变 action ID、Journal 终态或歧义失败关闭。
3. 加好友 UI 性能：优化搜索框、资料页、邀请表单及确认后页面 ROI OCR 和有上限等待；不删除字段复核、
   唯一联系人确认、最终确认或页面收尾。
4. Brain 首次输出与知识检索质量：`0.9.53` 的完整历史和 `0.9.60` 的生成额度不重做；在积累真实知识、客户问法和回复样本后，
   提高首次结果满足现有合同和 Guard 的概率，并评估当前关键词检索是否需要增加语义/向量检索；不删除语义复核、Guard、
   质量修复或结果验证，Brain 期间继续持有当前会话逻辑 UI 锁。
5. 首屏扫描成本：在不改变未读代次、短码/private 准入和扫描只读语义的前提下，评估 ROI、稳定区域
   复用和 OCR 调用瘦身；单独红点仍不能制造新消息事实。
6. 剩余无业务语义公共代码收口：`0.9.60` 已完成的正式包排除、前端压缩和已证明无调用代码删除不重做；
   只合并仍然存在且业务语义完全相同的截图/OCR 辅助函数、序列化和报告构建。不得改变 UI 动作顺序，
   不得新增第二套 Sidecar 生命周期或跨会话可变缓存。
7. C4 性能优化：只在获得独立真实召回链路样本后评估，不使用 C2/C3 样本推算。
8. 抖音自动接入：抖音 API、企业私信 Webhook/OAuth、小风车或巨量引擎同步统一作为一个渠道接入专题，
    复用现有线索去重、分配和审计，不在客户端增加第二套线索权威源。
9. 人工输入保护条件评估：悬浮球、键鼠 Hook 或快捷键只有在证明可独立关闭、不改变业务状态、不持有
    UI 锁且不会使任务永久 executing 后才允许重新设计；未通过独立 Windows 安全验收不得实现。
10. 客户端与后端分阶段升级兼容（已知设计缺陷）：当前客户端发布版本与 C2 合同 revision/SHA
