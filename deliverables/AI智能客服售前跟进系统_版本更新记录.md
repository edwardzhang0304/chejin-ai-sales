...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.4.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.4.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.4.puml`
...
| 本轮整改父基线 | `84349e1b7d7bbc81a710d6b9b1d662cd28cde9a5`；最终候选 HEAD 由提交及包内 manifest 动态记录 |
| 灰度版本 | `0.9.4`；`0.9.0`、`0.9.1`、`0.9.2`、`0.9.3` 已冻结，本候选不得覆盖旧包 |
| 灰度分支 | `codex/gray-release-0.9.x`；从完整候选收口后建立，禁止混入下一期功能 |
| 当前机器合同 | `contract_revision=0.9.4`；规范化合同 SHA `8d19676dc5cf5c3913d8fb8d568cf46d88c03d6d76e0493b3e68c3a94799a51f`；Sidecar、Worker、后端校验、生成 Schema、样例、manifest 和测试同步 |
| 来源治理 | 独立 OmniAuto 固定提交 `18ab469d963420f802eca14fba51327f640df658`；内嵌副本与 `.chejin-source.json` 使用同一真实提交 |
| PRD | v0.9.4 |
| 技术方案 | v0.9.4 |
| 全流程图 | v0.9.4 源文件 |
| 发布状态 | `0.9.4` 正在执行提交级门禁；通过后推送、固定标签并生成唯一 Windows 灰度 ZIP |

### 2.1 2026-08-14：AI 回复气泡的发送结果补强确认
...
### 2.2 2026-08-14：无车源时的 Brain 澄清与转人工承接收口
...
### 2.3 2026-08-14：加好友收尾与 Brain 可见回复恢复
...
### 2.4 2026-08-13：Worker 当前过程连续展示
...
- 灰度系列固定为 `0.9.x`。`0.9.0`、`0.9.1`、`0.9.2`、`0.9.3` 已冻结，当前候选为 `0.9.4`；后续每个内容不同且进入测试的
  不可变候选依次使用 `0.9.5、0.9.6……`，不得跳回旧号或覆盖同号内容。
...
- 灰度稳定分支固定为 `codex/gray-release-0.9.x`；每个不可变候选使用
  `gray-v0.9.0、gray-v0.9.1、gray-v0.9.2、gray-v0.9.3、gray-v0.9.4……` 标签。`main` 只接收完成灰度验收的确切标签提交。
