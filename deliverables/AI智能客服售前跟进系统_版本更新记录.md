...
| 车金集成提交 | `7cbe5727991650dcce5e48d799dc59635eade7f1`（0.9.68 源码及版本集成，已推送灰度分支） |
| 本次正式构建来源 | `82b5988ceeff8225f4fc282a2d3849d110a90d75`，包含 `7cbe572`、来源登记及测试夹具/升级流程修正 |
| 开发基线 | 正式构建源码 `a1faf8bd1c9fba53bbb574de0ab3e1c59364d1a2`；包含客户端工程师 `6b4a2c8` 及后续安全门禁修正 |
| 本轮版本 | 客户端 `0.9.69` 正式候选；目标为 0.9.67 点击“检查更新”升级；尚待本轮构建/发布 |
| 合同状态 | 本次已发布 `0.9.68 / 23c30e5a5cce3b2e0d5e697fe2733cf670d3c8cde04665c68997e9762977f1a3`；0.9.67 合同保留历史登记 |
| 生产 / Windows 记录 | 正式 CI `34085360247` 成功；生产后端 `0.9.68` 及更新查询、完整下载验证通过；Windows 原地升级和微信实机验收待完成 |
| 交付校验 | 使用已发布 `0.9.67` 实际公钥和旧版校验代码验签；ZIP/EXE、3285 个清单文件与运行探针通过 |
...
| 当前结论 | 0.9.69 兼容候选准备，正式包和 Windows 本机验收待完成；历史 0.9.68 发布证据不代表本轮通过 |
...
- 当前为构建前准备；正式运行号、新包哈希、生产登记与 Windows 本机验收尚未产生。既有 0.9.68 包不覆盖。
