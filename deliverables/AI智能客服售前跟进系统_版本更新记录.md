...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.6.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.6.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.6.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | 当前 `codex/gray-release-0.9.x` 本地工作区；架构复审前保持未提交、未推送、未打包 |
| 本轮整改父基线 | `c10fef9100ed810ab0292baa300c90ba4a0ba4d6`（`gray-v0.9.4`）；最终候选 HEAD 由提交及包内 manifest 动态记录 |
| 灰度版本 | `0.9.6`；`0.9.0`、`0.9.1`、`0.9.2`、`0.9.3`、`0.9.4`、`0.9.5` 已冻结，本候选不得覆盖旧包 |
| 灰度分支 | `codex/gray-release-0.9.x`；从完整候选收口后建立，禁止混入下一期功能 |
| 当前机器合同 | `contract_revision=0.9.6`；规范化合同 SHA `905ae33a2605a4c3541d1dc623298ca057bb9d0f1271a5af8e768a8131413b71`；Sidecar、Worker、后端校验、生成 Schema、样例、manifest 和测试同步 |
| 来源治理 | 当前独立 OmniAuto 固定提交为 `4260e1efcc8d78d7ba24ee9c28e28406d923832e`；`0.9.6` 生成 Schema、加好友确认后残留窗口收尾及包级测试已进入该提交，`.chejin-source.json.source_commit` 已同步登记真实哈希 |
| PRD | v0.9.6 |
| 技术方案 | v0.9.6 |
| 全流程图 | v0.9.6 源文件 |
| 发布状态 | `0.9.6` 暂停续行与 AI 回复边界候选开发中；架构复审前不推送、不打包 |

### 2.1 2026-08-14：发送焦点有限恢复与未读代次
...
- `0.9.6` 替换顺序固定为：暂停 `0.9.5` Worker -> 确认无未结算 Outbox/Journal/回执 -> 升级后端并执行 `20260814_0028` -> 替换 `0.9.6` Worker -> 恢复接单。存在未结算事实时禁止直接清库。

### 2.2 2026-08-14：AI 回复气泡的发送结果补强确认

- 实机证据确认 AI 回复已真实发送且输入框清空，但六次全屏 OCR 都漏掉绿色长文字气泡，结构图片探测器因缺少文字类型证据将其投影为图片，导致 `SEND_RESULT_UNKNOWN`。
- 发送后快照先使用全屏 OCR；若最底部右侧结构候选由同排头像确认为 `self`，则对该气泡执行一次局部放大增强 OCR。只有 OCR 全文与本次程序发送正文完全一致，才恢复为文字事实。
...
### 2.3 2026-08-14：无车源时的 Brain 澄清与转人工承接收口
...
### 2.4 2026-08-14：加好友收尾与 Brain 可见回复恢复
...
### 2.5 2026-08-13：Worker 当前过程连续展示
...
- 灰度系列固定为 `0.9.x`。`0.9.0`、`0.9.1`、`0.9.2`、`0.9.3`、`0.9.4`、`0.9.5` 已冻结，当前候选为 `0.9.6`；后续每个内容不同且进入测试的
  不可变候选依次使用 `0.9.6、0.9.7……`，不得跳回旧号或覆盖同号内容。
...
- 灰度稳定分支固定为 `codex/gray-release-0.9.x`；每个不可变候选使用
  `gray-v0.9.0、gray-v0.9.1、gray-v0.9.2、gray-v0.9.3、gray-v0.9.4、gray-v0.9.5……` 标签。`main` 只接收完成灰度验收的确切标签提交。
