...
| 灰度版本 | 目标候选 `0.9.25`；`0.9.0` 至 `0.9.24` 已冻结，旧 revision/SHA/标签/ZIP 不得覆盖 |
...
| 当前机器合同 | `contract_revision=0.9.25`，规范化 SHA `53e2ed19dbf4ee62677660689a7385066a7e660d5d70c74a0254b1e94d753bc7`；生成 Schema 已同步且 current 检查通过 |
| 来源治理 | `0.9.25` OmniAuto/Worker 功能候选提交 `15db96575a23f57beb9bab54be6a21e5de0ac748`；`.chejin-source.json` 已绑定该真实候选，最终集成提交由 Git 标签和包内 manifest 固定 |
| PRD | v0.9.10 |
...
| 发布状态 | `0.9.25` 功能修复、合同、生成 Schema、来源候选和直接受影响定向验证已完成并经架构师复审；允许提交、推送并生成仅供实机验收的新 Fast UAT ZIP。Windows 实际发送、正式发布门禁和客户可交付包仍待完成，不得将 Fast UAT ZIP 声称为正式包 |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.24` 已冻结，当前目标候选为 `0.9.25`；后续每个内容不同且进入测试的
