...
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.50.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.50.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | 原灰度分支 `codex/gray-release-0.9.x` 的隔离工作区；继承不可变 `gray-v0.9.49`，只修复 C3 checkpoint 比较凭证丢失和对应技术失败误 Handoff |
| 本轮整改父基线 | `26e149a`（`gray-v0.9.49`）；原始生产能力底座仍为 `b6ad192`（`gray-v0.9.43`） |
| 灰度版本 | 目标候选 `0.9.50`；只修改 C3 发送前连续性凭证传递和技术失败结算 |
...
| 当前机器合同 | `contract_revision=0.9.50`；本地规范化 SHA 为 `d1500d91da35f92f7b37a73317e47402123b685fddafcb2e65c95fce63b84dd5` |
| 来源治理 | `0.9.50` 独立 OmniAuto 真实提交为 `f6a7706c20c22d8cd176fefeccfdcf3027eaa753`；`.chejin-source.json` 已指向该提交并登记 Schema 与可移植比较器 scope |
| PRD | v0.9.50；产品流程不变，仅修复发送前凭证内部传递及技术失败误转人工 |
| 技术方案 | v0.9.50 是当前唯一目标口径；Worker 唯一决定连续性，Sidecar 只执行最终发送门，后端只结算 |
| 全流程图 | v0.9.50；C0—C4 主流程与 0.9.49 一致 |
| 发布状态 | 本地实现、合同、Schema 和前中后定向测试通过；待架构复审、双仓来源提交、推送、标签、灰度 ZIP、配套后端部署和 Windows UAT |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.49` 已冻结且不得同名覆盖，当前目标候选为 `0.9.50`；后续每个内容不同且进入测试的
