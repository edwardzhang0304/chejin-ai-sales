...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.42.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.42.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.42.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | `codex/gray-release-0.9.x` 的隔离工作区；`0.9.41` 已形成不可覆盖提交 `f9b16c3` 和标签 `gray-v0.9.41`；`0.9.42` 已通过代码架构复审，车金提交、推送、标签和 ZIP 待形成 |
| 本轮整改父基线 | `f9b16c3`（`gray-v0.9.41`） |
| 灰度版本 | 目标候选 `0.9.42`；`0.9.0` 至 `0.9.41` 均不得被同名覆盖；本轮只做 C3 同帧 ROI 完整回退和非首屏定位同事务复用，不放宽发送前 checkpoint 比较 |
...
| 当前机器合同 | 本地实现为 `contract_revision=0.9.42`，规范化 SHA 为 `9dcab8759b3f2a0611027cccda0044cd24e182e50418c33b88d997007a5c4305`；合同、生成 Schema、Worker、后端和打包入口已同步 |
| 来源治理 | 独立 OmniAuto 已形成真实提交 `307241810963c2e649ba04483a898687d06ba9f4`，包含本轮 Sidecar、生成 Schema、兼容测试和直接 `window_layout.input_text_detection_bounds` 依赖；`.chejin-source.json` 已绑定该 SHA |
| PRD | v0.9.42 |
| 技术方案 | v0.9.42 是当前唯一技术口径；本轮只允许两个 P0 性能热点，不改变业务状态机和发送安全门 |
| 全流程图 | v0.9.42 为当前唯一流程图；pre_send_refresh 与 S0/S1/S2 各自保持独立物理帧，单帧 ROI 不足时只对同帧整窗回退一次 |
| 发布状态 | `0.9.41` 是已冻结回退基线；`0.9.42` 功能架构复审已通过且真实 OmniAuto 来源提交已形成；车金提交、双仓推送、标签、ZIP、配套后端部署和 Windows 实机耗时/UAT 待完成 |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.41` 已冻结且不得同名覆盖，当前目标候选为 `0.9.42`；后续每个内容不同且进入测试的
