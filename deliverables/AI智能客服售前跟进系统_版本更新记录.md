...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.9.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.9.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.9.puml`
...
| 本轮整改父基线 | `ef3ce3c`（`gray-v0.9.8`） |
| 灰度版本 | `0.9.9`；`0.9.0` 至 `0.9.8` 已冻结，本候选不得覆盖旧包 |
| 灰度分支 | `codex/gray-release-0.9.x`；从完整候选收口后建立，禁止混入下一期功能 |
| 当前机器合同 | `contract_revision=0.9.9`；规范化合同 SHA `da630bbf80f5fb51c5553b30f378c7ae4a48785e5d765a3925a0081ab1a2ac65`；Sidecar、Worker、后端校验、生成 Schema、样例、manifest 和测试同步 |
| 来源治理 | 当前独立 OmniAuto 固定提交为 `392a8f09e58fc0db23f821c631d61447104df4c7`；`0.9.9` 生成 Schema 已进入该提交；`.chejin-source.json.source_commit` 已同步登记真实哈希 |
| PRD | v0.9.9 |
| 技术方案 | v0.9.9 |
| 全流程图 | v0.9.9 源文件 |
| 发布状态 | `0.9.9` 跨轮视觉换行身份兼容与观测事务隔离候选；提交级复核通过后生成唯一 ZIP 并部署匹配后端 |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.8` 已冻结，当前候选为 `0.9.9`；后续每个内容不同且进入测试的
  不可变候选依次使用 `0.9.10、0.9.11……`，不得跳回旧号或覆盖同号内容。
...
- 灰度稳定分支固定为 `codex/gray-release-0.9.x`；每个不可变候选使用
  `gray-v0.9.0、gray-v0.9.1、gray-v0.9.2、gray-v0.9.3、gray-v0.9.4、gray-v0.9.5、gray-v0.9.6、gray-v0.9.7、gray-v0.9.8、gray-v0.9.9……` 标签。`main` 只接收完成灰度验收的确切标签提交。
