...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.5.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.5.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.5.puml`
...
| 灰度版本 | `0.9.5`；`0.9.0`、`0.9.1`、`0.9.2`、`0.9.3`、`0.9.4` 已冻结，本候选不得覆盖旧包 |
| 灰度分支 | `codex/gray-release-0.9.x`；从完整候选收口后建立，禁止混入下一期功能 |
| 当前机器合同 | `contract_revision=0.9.5`；规范化合同 SHA `35c440a049973ba5aa74880c7d5207da3f3ce9cab8e86de34655f5d9a585293e`；Sidecar、Worker、后端校验、生成 Schema、样例、manifest 和测试同步 |
| 来源治理 | 独立 OmniAuto 固定提交 `7dd22f58c3c6f034387efec1c349bf6cb2d2c17d` 已形成并推送；`.chejin-source.json` 已同步真实来源、`0.9.5` Schema 和发送焦点有界恢复范围 |
| PRD | v0.9.5 |
| 技术方案 | v0.9.5 |
| 全流程图 | v0.9.5 源文件 |
| 发布状态 | `0.9.5` 功能与来源候选已完成受影响回归；经提交级快速复核后生成唯一灰度 ZIP，并协调部署同合同后端 |
...
- 替换顺序固定为：暂停 `0.9.4` Worker -> 确认无未结算 Outbox/Journal/回执 -> 升级后端并执行 `20260814_0027` -> 替换 `0.9.5` Worker -> 恢复接单。存在未结算事实时禁止直接清库。
...
- 灰度系列固定为 `0.9.x`。`0.9.0`、`0.9.1`、`0.9.2`、`0.9.3`、`0.9.4` 已冻结，当前候选为 `0.9.5`；后续每个内容不同且进入测试的
