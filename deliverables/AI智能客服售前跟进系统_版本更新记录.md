...
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.28.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.28.puml`
...
| 灰度版本 | 目标候选 `0.9.30`；`0.9.0` 至 `0.9.29` 已冻结，旧 revision/SHA/标签/ZIP 不得覆盖 |
...
| 当前机器合同 | `contract_revision=0.9.30`，规范化 SHA `5c03784ebcb561f451d78e5f0379b4455c981edae2fd093efa072f27bc350b58`；生成 Schema 已同步且 current 检查通过 |
| 来源治理 | `0.9.30` 功能候选提交待本轮代码和定向测试收口后真实形成；不得继续让 `0.9.29` 的 `abc2887…` 承担本轮能力，随后由 `.chejin-source.json` 和最终集成提交分别固定 |
| PRD | v0.9.10 |
| 技术方案 | v0.9.28 权威方案不变；`0.9.30` 补齐已规定的重启 Flow 有限恢复出口，并增加本地/后端在途登记对账，不修改业务状态机 |
| 全流程图 | v0.9.28 源文件仍为权威流程；`0.9.30` 只补全既有恢复分支的实现和组合测试 |
| 发布状态 | `0.9.29` 实机复测仍出现心跳正常但不拉单，已判定不可验收；`0.9.30` 正在收口本地旧 C2 Flow 与后端空状态的自动结算，以及其余不一致分支的显式暂停。复审、真实来源提交、标签、ZIP 和 Windows UAT 均未完成 |
...
- 定向测试必须经过生产 `tick_once()`、真实 SQLite 和真实 ActionJournal 文件，覆盖双空、仅本地 C2、仅本地 C2+Journal、同 ID 有/无凭证、仅后端、ID 冲突、非法状态、任务 Flow 单边存在、多个客户归属和当前进程新 Flow；Windows 截图、OCR、鼠标及真实服务端连接仍由实机 UAT 验收。
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.29` 已冻结，当前目标候选为 `0.9.30`；后续每个内容不同且进入测试的
