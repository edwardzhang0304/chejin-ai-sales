...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.46.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.46.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.46.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | `codex/gray-v0.9.46-english-package`；继承不可变 `gray-v0.9.45`，只补齐 C2 嵌套序列合同、旧 Outbox 有限失败终态并生成英文目录 Windows 包 |
| 本轮整改父基线 | `b6ad192`（`gray-v0.9.43`） |
| 灰度版本 | 目标候选 `0.9.46`；正常文字、语音、图片、混合、拆包、Brain 和状态机不变 |
...
| 当前机器合同 | `contract_revision=0.9.46`；本地规范化 SHA 为 `5b7cc473f16062dd668a6880379b2375108985abd779f5dcb53deb628da5a371` |
| 来源治理 | `0.9.46` 独立 OmniAuto 来源提交为 `69c69a60b7e08a31f9d773864b14b35faf86754b`；只同步生成 Schema，`.chejin-source.json` 已登记 `c2_contract_0_9_46_generated_schema` |
| PRD | v0.9.46；产品与业务流程不变 |
| 技术方案 | v0.9.46 是当前唯一目标口径；Worker HTTP 前深度门禁和后端嵌套序列验收职责已冻结 |
| 全流程图 | v0.9.46；C2 Outbox 缺证据零 HTTP 有限结算，后端精确验证位置和新增后缀 |
| 发布状态 | 架构复审通过；独立 OmniAuto 来源分支已推送，待形成车金提交、推送不可变标签、运行 GitHub Windows 打包、部署配套后端并执行 Windows 实机 UAT |
