...
| 本轮整改父基线 | `b1b73db`（`gray-v0.9.14`） |
| 灰度版本 | `0.9.15`；`0.9.0` 至 `0.9.14` 已冻结，本候选不得覆盖旧包 |
| 灰度分支 | `codex/gray-release-0.9.x`；从完整候选收口后建立，禁止混入下一期功能 |
| 当前机器合同 | `contract_revision=0.9.15`；规范化合同 SHA `0a9fcf64e41c58536dfae29f8c2dd2cc8f3867a95ef6fe79438789c68b24ac2a`；Sidecar、Worker、后端校验、生成 Schema、样例、manifest 和测试同步 |
| 来源治理 | 当前独立 OmniAuto 固定提交为 `9c45ad157a9314605333ae76d29e4a0211356656`；单调文字/语音类型仲裁与 `0.9.15` 生成 Schema 已进入该提交；知识分类静态文件登记为车金 overlay |
| PRD | v0.9.10 |
| 技术方案 | v0.9.10 |
| 全流程图 | v0.9.10 源文件 |
| 发布状态 | `0.9.15` 在完整保留 `0.9.14` 仲裁修复的基础上，新增八条经审查知识的显式幂等导入、激活和回滚能力；提交级复核通过后生成唯一 ZIP 并部署匹配后端 |
