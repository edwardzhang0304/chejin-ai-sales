...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.43.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.43.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.43.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | `codex/gray-release-0.9.x` 的隔离工作区；`0.9.42` 已形成不可覆盖提交 `ce5cbf8` 和标签 `gray-v0.9.42`，但 Windows 门禁未通过且没有 ZIP；`0.9.43` 为当前候选 |
| 本轮整改父基线 | `ce5cbf8`（`gray-v0.9.42`） |
| 灰度版本 | 目标候选 `0.9.43`；`0.9.0` 至 `0.9.42` 均不得被同名覆盖；本轮只修正一条旧测试夹具并同步版本合同，不改已复审生产逻辑 |
...
| 当前机器合同 | 本地实现为 `contract_revision=0.9.43`，规范化 SHA 为 `a87275e55d6f25aeba3185d854f4e613a9209924ba4ac5ac4f6f49a3aeb00cef`；合同、生成 Schema、Worker、后端和打包入口已同步 |
| 来源治理 | `0.9.43` 独立 OmniAuto 来源为 `27c59c8a0e9c85106a12f05f6f92e0193fefb5af`；它在已复审生产来源 `3072418…` 上只同步新版本生成 Schema，`.chejin-source.json` 已绑定 |
| PRD | v0.9.43 |
| 技术方案 | v0.9.43 是当前唯一技术口径；生产流程与已复审的 v0.9.42 完全相同 |
| 全流程图 | v0.9.43 为当前唯一流程图；pre_send_refresh 与 S0/S1/S2 各自保持独立物理帧，单帧 ROI 不足时只对同帧整窗回退一次 |
| 发布状态 | `0.9.41` 是可用回退基线；`0.9.42` 标签冻结但 Windows 门禁失败且无 ZIP；`0.9.43` 来源提交已形成，车金提交、双仓推送、标签、ZIP、配套后端部署和 Windows 实机耗时/UAT 待完成 |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.42` 已冻结且不得同名覆盖，当前目标候选为 `0.9.43`；后续每个内容不同且进入测试的
