...
| 灰度版本 | `0.9.10`；`0.9.0` 至 `0.9.9` 已冻结，本候选不得覆盖旧包 |
| 灰度分支 | `codex/gray-release-0.9.x`；从完整候选收口后建立，禁止混入下一期功能 |
| 当前机器合同 | `contract_revision=0.9.10`；规范化合同 SHA `5bddc76ddedbcb74c3172c5fb43c47e4a59a28eb1181e3ca8823dad8fc8184b6`；Sidecar、Worker、后端校验、生成 Schema、样例、manifest 和测试同步 |
| 来源治理 | 当前独立 OmniAuto 固定提交为 `639e01ef7bc5532eb3ed3948d995b2a3cfc7b33a`；`0.9.10` 生成 Schema 已进入该提交；`.chejin-source.json.source_commit` 已同步登记真实哈希 |
| PRD | v0.9.10 |
| 技术方案 | v0.9.10 |
| 全流程图 | v0.9.10 源文件 |
| 发布状态 | `0.9.10` 已确认 AI 文字全生命周期类型恢复、暂停 UI 收尾与脱敏时间窗取证候选；提交级复核通过后生成唯一 ZIP 并部署匹配后端 |
