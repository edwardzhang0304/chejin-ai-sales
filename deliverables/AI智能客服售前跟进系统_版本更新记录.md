...
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.36.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.36.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | `codex/gray-release-0.9.x`；`0.9.35` 已形成 `gray-v0.9.35` 标签和 ZIP，不得覆盖；`0.9.36` OmniAuto 真实功能提交已固定为 `d3e5993045226f68481a09a39d6bc4b38595d483` |
| 本轮整改父基线 | `4ae792b`（`gray-v0.9.15`） |
| 灰度版本 | 目标候选 `0.9.36`；`0.9.0` 至 `0.9.35` 均不得被同名覆盖；本轮只补齐 Brain 隔离子进程分阶段可观测性，不改变 C0—C4 业务流程 |
...
| 当前机器合同 | Worker、Sidecar、后端和生成 Schema 已在本地统一为 `contract_revision=0.9.36`，规范化 SHA `8154dafe703bd9685e9b96f15675a71ba8cd8e117fa57cfe317365ae5e8f377f`；禁止与 `0.9.35` 混跑 |
| 来源治理 | `.chejin-source.json` 已绑定 `0.9.36` 真实 OmniAuto 功能提交 `d3e5993045226f68481a09a39d6bc4b38595d483`；车金候选提交、推送、标签和打包仍待完成 |
| PRD | v0.9.10 |
| 技术方案 | v0.9.36 是当前唯一技术口径；保留 v0.9.35 全部业务流程，只新增 Brain 强制超时的分阶段证据 |
| 全流程图 | v0.9.36 为当前唯一流程图；Sidecar → Worker → 后端的权限、顺序和业务状态机不变 |
| 发布状态 | `0.9.35` 已推送、打标签并生成 ZIP，不得覆盖；`0.9.36` 已形成 OmniAuto 功能提交并更新来源绑定，车金候选提交、推送、标签、ZIP 和配套后端部署待完成 |
...
- 本轮不修改 Brain 生成、备用模型、语义审核、Guard、重试次数、超时时间或 C0—C4 状态机。本地合同已升为 `0.9.36 / 8154dafe…8f377f`；架构复审已通过，OmniAuto 真实功能提交已固定为 `d3e5993045226f68481a09a39d6bc4b38595d483` 并完成来源绑定；推送、打包和配套后端部署待发布流程完成。
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.35` 已冻结且不得同名覆盖，当前目标候选为 `0.9.36`；后续每个内容不同且进入测试的
