...
版本：v0.9.65（运维发布准备 v0.1；正式构建、生产切换及 Windows 实机验收待完成）
...
| 当前生产 / Windows | 后端及更新渠道仍 `0.9.64`；用户确认 Windows 安装失败后自动回滚为 `0.9.63`，保持暂停 |
...
| 当前结论 | 用户授权运维继续构建与同版部署；自动化与正式 Windows 实机验收分别记录，均不得以旧版结论代替 |
...
工程师提交说明报告定向复审 `121 tests + 2 subtests`。运维在版本同步后复跑 API、更新器、证据导出、启动健康、协调器、安全边界、打包入口/脚本和合同，结果 `163 passed, 49 subtests passed`；真实 Worker/SQLite/HTTP 自动收尾及后端更新接口测试 `15 passed, 2 warnings`（现有 httpx/anyio 弃用提示）；正式 Windows CI 待触发。
