...
版本：v0.9.70.3（正式门禁工具修正；仅验收 0.9.69 → 0.9.70，未部署或开放更新）
...
| 0.9.70 源码候选 | `30473b3423fa16f81771718c4c39b5a16058020d`，已复审、已推送；以下 0.9.69 包信息为既有发布记录 |
| 0.9.70 OmniAuto 来源 | `fd2c9d7700bac78710022b2a0c3579537395fc3e`，已推送 `codex/gray-release-0.9.x-source` |
| 0.9.68 集成基线 | `7cbe5727991650dcce5e48d799dc59635eade7f1`（0.9.68 源码及版本集成，已推送灰度分支） |
| 本次正式构建来源 | `d9264625d218edfa36a6d4b91940853d7d8db0a1`；包含工程师 `709b323`、严格旧协议兼容及 Windows 门禁修正 |
| 开发基线 | `709b3239b0072325e880ac4747b405f6f121b974`；原始 0.9.67 只作明确绑定的旧更新器兼容验收来源 |
| 本轮版本 | 客户端 `0.9.69` 有已发布记录；用户报告 0.9.67 → 0.9.69 按钮升级实机失败，该路径不通过 |
| 合同状态 | 沿用生产 `0.9.68 / 23c30e5a5cce3b2e0d5e697fe2733cf670d3c8cde04665c68997e9762977f1a3`，本次客户端修复未改 C2 合同 |
| 生产 / Windows 记录 | 正式构建/直传 `34106312819`、同提交 Fast UAT `34107799490`、生产检查 `34110260932`、发布/外网验证 `34110477182` 曾全部成功；其覆盖未包含真实旧 Worker 完整入口，不能抵消用户本次实机失败 |
| 交付校验 | 本次 EXE/清单/运行探针、原始旧 Updater 交接、服务端验签和 3285 个清单文件、HTTPS 首尾 Range 与完整下载 SHA-256 均通过 |
| OmniAuto 来源 | Brain `92648e700fba985036a5204723d950b322b40ce5` → 0.9.68 Schema `c752cbd31b476cb5e7b2cb4ccd4d500b913aec6c`，已推送用户 fork |
| 正式版本车道 | `1.0.x` 正式上线及稳定性修复；`1.1.x` 下一期优化；本轮不扩范围 |
| 当前结论 | 停止推荐 0.9.67 按钮升级，转保留数据的强制安装升级；安装方案及实机结果待验证 |
| 生产入口处置 | 本轮仅修订约束，未执行撤回/限制；实时入口状态和后续处置【待确认】，不得写“已关闭” |
