...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.47.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.47.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.47.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | 原灰度分支 `codex/gray-release-0.9.x` 的隔离工作区；继承不可变 `gray-v0.9.46`，只补齐重启媒体事实恢复、Journal 归档重试和 `faulted` 不可降级 |
| 本轮整改父基线 | `b6ad192`（`gray-v0.9.43`） |
| 灰度版本 | 目标候选 `0.9.47`；正常文字、语音、图片、混合、Brain、媒体编排和后端业务接口不变 |
...
| 当前机器合同 | `contract_revision=0.9.47`；本地规范化 SHA 为 `c3d1b2a8f436182df84d1b692f6be76cb233fb2d7716664a9db0107da8cf0b28` |
| 来源治理 | `0.9.47` 独立 OmniAuto 来源提交为 `6fc2b12a1c121ec747f9f7ef7123775474dff1d1`；只同步生成 Schema，`.chejin-source.json` 已登记 `c2_contract_0_9_47_generated_schema` |
| PRD | v0.9.47；产品与业务流程不变 |
| 技术方案 | v0.9.47 是当前唯一目标口径；已确认动作有限修复、归档重试和 `faulted` 持久状态已冻结 |
| 全流程图 | v0.9.47；旧流程安全恢复，故障状态不因断网、心跳或其他对账降级 |
| 发布状态 | 架构复审通过；独立 OmniAuto 本地真实提交已形成，待定向门禁、双仓推送、不可变标签、灰度 ZIP 解压启动校验、配套后端部署和 Windows UAT |
