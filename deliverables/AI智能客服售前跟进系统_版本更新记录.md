...
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.35.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.35.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | `codex/gray-release-0.9.x`；`0.9.34` 已打 `gray-v0.9.34` 标签并生成过 ZIP，不得覆盖；`0.9.35` 当前为本地整改候选，尚未形成真实功能提交 |
| 本轮整改父基线 | `4ae792b`（`gray-v0.9.15`） |
| 灰度版本 | 目标候选 `0.9.35`；`0.9.0` 至 `0.9.34` 均不得被同名覆盖；本轮关闭 Windows 完整门禁中的 CI、测试夹具和 SQLite 连接生命周期问题，不改变 C0—C4 业务流程 |
...
| 来源治理 | `.chejin-source.json` 仍绑定 `0.9.34` 真实功能提交 `11d43d5bb9dd83831e0bbba8ed84b5eba700cb2c`；必须先形成 `0.9.35` 真实功能提交，再更新来源记录 |
| PRD | v0.9.10 |
| 技术方案 | v0.9.35 是当前唯一技术口径；保留 v0.9.34 身份边界，只收口 Windows 发布门禁和连接释放 |
| 全流程图 | v0.9.35 为当前唯一流程图；Sidecar → Worker → 后端的权限、顺序和业务状态机不变 |
| 发布状态 | `0.9.34` 已有标签和 ZIP，本次修复不得覆盖。`0.9.35` 已完成本地代码整改和定向复测；尚未形成真实提交、推送、标签、ZIP、GitHub Windows 完整门禁或 Windows 实机 UAT |
...
- 本轮不修改 C0—C4、媒体动作顺序、Brain/Guard、Handoff、UI 锁、后端接口或发送回执流程；`0.9.35` 当前仅为本地候选，真实功能提交、来源绑定、推送、标签、ZIP、GitHub Windows 完整门禁和 Windows 实机 UAT 均未完成。
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.34` 已冻结且不得同名覆盖，当前目标候选为 `0.9.35`；后续每个内容不同且进入测试的
