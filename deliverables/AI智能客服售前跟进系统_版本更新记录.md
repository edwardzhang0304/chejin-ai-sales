...
版本：v0.9.71.2（0.9.71发布准备；应用与C2合同统一0.9.71，尚未构建或部署）
...
| 项目 | 当前值 |
|---|---|
| 发布分支 / 目标 | `codex/gray-release-0.9.x`；Windows 0.9.70、生产 Docker 后端及运营后台 |
| 应用源码 / 正式包源码 | 已复审应用 `30473b3423fa16f81771718c4c39b5a16058020d`；实际正式构建 `18d675b59007a12233e1c0b0d0ce600961619aa1`，包含门禁修正，应用代码未再修改 |
| OmniAuto 来源 | `fd2c9d7700bac78710022b2a0c3579537395fc3e`，用户 fork 已推送 |
| 支持起点 | 原始正式 0.9.69；最低 Updater 0.9.69；更早版本不在本轮兼容范围 |
| 同提交 Fast UAT / 正式构建及直传 | `34179625630` / `34180318869`，均成功 |
| Windows 完整按钮自动验收 | 原始 Worker、原始 Updater、原始信任及本次签名 ZIP；paused、faulted 两场景均通过，数据保留及新 UI 0.9.70 已核对 |
| 生产后端 / 合同 / 迁移 | `chejin-api:v0.9.70-18d675b-verified`，合同 0.9.70，迁移 `20260907_0034`，健康；运营后台同步切换 |
| 生产检查 / 发布 | `check_staged 34182754359` 成功；`publish_staged 34182936907` 已登记，外网完整下载校验通过 |
| 线上更新接口 | 0.9.69 返回可更新至 0.9.70，最低更新器 0.9.69；0.9.70 查询为已最新，哈希与验收包一致 |
| 用户 Windows 现场验收 | 【待确认】用户本机升级后的绑定、配置、暂停状态和业务表现；自动验收不能代替现场结论 |
| 数据 / 配置约束 | 业务数据、账号、配置、绑定、数据库容器与卷均保留；未新建备份，未自动生产回滚，未执行历史清理 |
