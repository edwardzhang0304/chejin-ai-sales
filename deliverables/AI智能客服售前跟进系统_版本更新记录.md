...
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.19.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.19.puml`
...
| 灰度版本 | 目标候选 `0.9.19`；`0.9.0` 至 `0.9.18` 已冻结，旧 revision/SHA 不得覆盖 |
...
| 当前机器合同 | 本地候选统一为 `contract_revision=0.9.19`；规范化 SHA `c05bf25d61a5382118daa945c2822950d4ad23b84eb2407e563ff0a2caa251f2`。`0.9.18` 已固定标签，但发布门禁随后发生测试/样例迁移，禁止覆盖原候选 |
| 来源治理 | 独立 OmniAuto 固定提交 `c8ba1c708081f20cb5f33d1ca88a7eb656bc1ccd` 已推送，`.chejin-source.json` 已回填真实提交号与 `0.9.19` scope |
| PRD | v0.9.10 |
| 技术方案 | v0.9.19；继承 v0.9.18 的帧内语音 action binding 生产语义，并同步发布门禁迁移 |
| 全流程图 | v0.9.19 源文件；正式身份提交顺序与技术方案一致 |
| 发布状态 | `0.9.18` 标签保持冻结且未形成合格 Windows ZIP；`0.9.19` 作为包含相同生产修复与完整发布门禁的唯一替代候选 |
