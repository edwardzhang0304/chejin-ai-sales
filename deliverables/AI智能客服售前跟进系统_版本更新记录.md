...
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.23.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.23.puml`
...
| 灰度版本 | 目标候选 `0.9.23`；`0.9.0` 至 `0.9.22` 已冻结，旧 revision/SHA 不得覆盖 |
...
| 当前机器合同 | `contract_revision=0.9.23`，规范化 SHA `13bad103155a207b1205696b6eebf9836a7c366288b3532c80f6d122595cd970`；代码、生成 Schema、Worker 和打包入口绑定同一合同 |
| 来源治理 | `0.9.23` 独立 OmniAuto 固定提交 `befefc6fa17be11a763fa77ee09453bb28e02432`；`.chejin-source.json` 已绑定该真实完整 SHA |
| PRD | v0.9.10 |
| 技术方案 | v0.9.23；启动按 DPI/工作区固定窗口档位并用一张真实客户区截图建立全局标定，业务过程保留 `0.9.20` 流程 |
| 全流程图 | v0.9.23 源文件；C0—C4 共用启动标定，会话行、消息、弹窗和表单继续用原有必要业务帧 |
| 发布状态 | `0.9.23` 代码、合同、PR28 哈希、定向及完整自动化已收口；待双仓推送、唯一快速 UAT ZIP 和 Windows C1—C4 实机验收 |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.22` 已冻结，当前目标候选为 `0.9.23`；后续每个内容不同且进入测试的
