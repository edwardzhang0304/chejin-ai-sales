...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.50.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.60.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.57.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | 隔离工作区灰度分支 `codex/gray-release-0.9.x`；从不可变 `gray-v0.9.59` 提交 `d460905a879f7721a4c48712222e5de7768c5793` 开发 |
| 本轮整改父基线 | 已发布 `0.9.59` 提交 `d460905a879f7721a4c48712222e5de7768c5793`；不覆盖其代码、文档、标签或安装包 |
| 灰度版本 | 本地实现候选 `0.9.60`；收口低风险代码瘦身，并将普通、商品快速、低权限快速三条 Brain 主生成路径统一为 `max_tokens=8192` |
...
| 灰度分支 | `codex/gray-release-0.9.x`；`0.9.60` 只接收本轮已复审范围的低风险代码瘦身和 Brain 主生成额度修复，禁止混入 Qt 裁剪或其他优化 |
| 当前机器合同 | `contract_revision=0.9.60`；规范化 SHA `30cdc4e69e87d3cb628ed334d2e95199837aad526a6298ab3bd5030225f51ea6`，生成 Schema 已一致；Brain 额度不属于 C2 合同字段，集成后须复跑生成器核对但不得伪造新 SHA |
| 来源治理 | 独立 OmniAuto `0.9.60` 真实来源提交为 `eae057db568b4bb8a57a0d4694aaaadfacef9464`；`.chejin-source.json` 已更新并登记本轮范围 |
| PRD | v0.9.50；本轮不改变业务需求，无需为技术互斥修复升版 |
| 技术方案 | v0.9.60；是当前唯一技术口径；v0.9.59 保持不可变历史文件 |
| 全流程图 | v0.9.57；本轮不修改业务流程，不创建第二张流程图 |
| 发布状态 | 0.9.60 为本地未提交候选；未提交、未推送、未打标签、未打包，等待架构复审 |
...
和安装包按实际影响范围同步。“客户端检查更新”和“耗时观测清理”已移出本清单，共同提前进入
灰度 `0.9.59`。仍属于 `1.1.x` 的待开发清单如下：

1. 运营后台知识管理：第一期只管理 `chejin` 正式业务知识，提供草稿、待审核、发布、归档、
...
2. 人工接管受限监听：`waiting_sales_reply` 期间只读取足以证明销售是否实际回复的会话证据，不转写
   客户语音、不复制客户图片、不调用 Vision、不调用 Brain；销售回复必须发生在 HandoffEvent 之后，
   关闭 handoff 后才恢复普通 C2。该能力不得与当前低频完整读取并行形成两套权威状态。
3. C2 定位性能：复用首屏负向结果，使 visible 到短码搜索处于同一 Flow，并评估搜索框和标题 ROI OCR；
   不得删除后端授权、private/短码确认、唯一候选或最终标题确认。
4. 语音转写性能：只减少 prepare/execute 的等价帧重复 OCR、右键菜单 ROI 和已满足条件后的多余等待；
   不减少身份跟踪帧，不改变 action ID、Journal 终态或歧义失败关闭。
5. 加好友 UI 性能：优化搜索框、资料页、邀请表单及确认后页面 ROI OCR 和有上限等待；不删除字段复核、
   唯一联系人确认、最终确认或页面收尾。
6. Brain 首次输出质量：提高首次结果满足现有合同和 Guard 的概率，减少修复链比例；不删除语义复核、
   Guard、质量修复或结果验证，Brain 期间继续持有当前会话逻辑 UI 锁。
7. 首屏扫描成本：在不改变未读代次、短码/private 准入和扫描只读语义的前提下，评估 ROI、稳定区域
   复用和 OCR 调用瘦身；单独红点仍不能制造新消息事实。
8. 无业务语义代码瘦身：合并重复截图/OCR 辅助函数、重复序列化和重复报告构建；不得改变 UI 动作顺序，
    不得新增第二套 Sidecar 生命周期或跨会话可变缓存。
9. C4 性能优化：只在获得独立真实召回链路样本后评估，不使用 C2/C3 样本推算。
10. 抖音自动接入：抖音 API、企业私信 Webhook/OAuth、小风车或巨量引擎同步统一作为一个渠道接入专题，
    复用现有线索去重、分配和审计，不在客户端增加第二套线索权威源。
11. 人工输入保护条件评估：悬浮球、键鼠 Hook 或快捷键只有在证明可独立关闭、不改变业务状态、不持有
