...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.45.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.45.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.45.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | `codex/gray-v0.9.45-test-gate`；从不可变 `gray-v0.9.44` 候选继续，仅迁移 Windows 完整门禁旧夹具并同步版本/Schema |
| 本轮整改父基线 | `b6ad192`（`gray-v0.9.43`） |
| 灰度版本 | 目标候选 `0.9.45`；生产业务代码与 `0.9.44` 一致，只修复完整 Windows 门禁的旧测试合同、Pillow 弃用告警并同步版本/Schema |
...
| 当前机器合同 | `contract_revision=0.9.45`；本地规范化 SHA 为 `8813425572dad678b86354856dad798c43a9c47192d17319dfb8e84c8877e99e`；业务语义未变，仅因不可变版本递增重新生成 Schema |
| 来源治理 | `0.9.45` 独立 OmniAuto 来源已形成真实提交 `53caedad5baece001659aafcb5d7f86d98933e27`；它继承 `0.9.44` 生产能力并同步新生成 Schema，`.chejin-source.json` 已更新 |
| PRD | v0.9.45；产品与业务流程不变 |
| 技术方案 | v0.9.45 是当前唯一目标口径；业务投影、长期身份、当前帧几何三类职责保持不变 |
| 全流程图 | v0.9.45；pre_send_refresh 与 S0/S1/S2 保持独立帧，跨帧只比较五字段业务序列 |
| 发布状态 | `gray-v0.9.44` 已形成不可变提交/标签，但 Windows 完整门禁因 8 条旧测试夹具失败，未生成 ZIP；`0.9.45` 本机完整打包门禁和来源治理已通过，待形成车金提交、推送标签、运行 GitHub Windows 打包和部署配套后端，Windows 实机 UAT 仍待执行 |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.44` 已冻结且不得同名覆盖，当前目标候选为 `0.9.45`；后续每个内容不同且进入测试的
