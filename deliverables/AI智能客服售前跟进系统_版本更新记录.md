...
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.52.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.52.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | 灰度分支 `codex/gray-release-0.9.x` 的隔离工作区；继承不可变 `gray-v0.9.51`，已完成跨轮恢复计时器、未读代次、在途 Flow 与 C4 召回互斥的本地实现，等待架构复审 |
| 本轮整改父基线 | `2335db2`（`gray-v0.9.51`）；原始生产能力底座仍为 `b6ad192`（`gray-v0.9.43`） |
| 灰度版本 | 本地候选 `0.9.52`；代码、迁移、机器合同和生成 Schema 已实现，尚未提交、推送或发布 |
...
| 当前机器合同 | 本地候选 `contract_revision=0.9.52`；规范化 SHA `053ec7587763a8360c3d134916501108af4304c38e8e3c69d4e4a80239587940` |
| 来源治理 | `0.9.52` 独立 OmniAuto 真实提交已形成为 `80b79835f181201e80cb2f58f06df2c515f1dc56`；`.chejin-source.json` 已绑定该 SHA，车金集成提交待形成 |
| PRD | v0.9.50；本轮不改变业务需求，无需为技术互斥修复升版 |
| 技术方案 | v0.9.52 是当前唯一目标口径；旧 hold、旧读取结果和 C4 到期不得中断新未读代次或在途 Flow |
| 全流程图 | v0.9.52；同步标明调度顺序、hold suspend/settle 和召回延后 |
| 发布状态 | 技术方案、代码、迁移、合同、Schema、受影响定向自动化和 OmniAuto 真实来源提交已完成；车金集成提交、推送、标签、灰度 ZIP、后端部署和 Windows UAT 均未进行 |
...
- 定向吸收 PR #13 发现的图片内 OCR 误否决问题，但不采用其“面积比直接决定图片”实现：`80%/30%` 只生成待右键验证候选；文字菜单保留同帧 OCR 文字、零剪贴板/零 Vision；图片菜单且真实新位图回执才提交图片。菜单不明时技术失败，不猜测。
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.51` 已冻结且不得同名覆盖，当前技术方案目标候选为 `0.9.52`；后续每个内容不同且进入测试的
