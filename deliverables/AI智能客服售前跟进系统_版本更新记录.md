...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.48.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.48.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.48.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | 原灰度分支 `codex/gray-release-0.9.x` 的隔离工作区；继承不可变 `gray-v0.9.47`，只修复灰度 CI 的 GitHub 上下文作用域 |
| 本轮整改父基线 | `b6ad192`（`gray-v0.9.43`） |
| 灰度版本 | 目标候选 `0.9.48`；业务代码与 `0.9.47` 完全一致 |
...
| 当前机器合同 | `contract_revision=0.9.48`；本地规范化 SHA 为 `2cdf88e61310157fc058eeeee4a5300b52c53234a964737945e46ecd6c21f41b` |
| 来源治理 | `0.9.48` 独立 OmniAuto 来源提交为 `3413c591357f6fd790f53b0f967ff6bc9c6de57f`；只同步生成 Schema，`.chejin-source.json` 已登记 `c2_contract_0_9_48_generated_schema` |
| PRD | v0.9.48；产品与业务流程不变 |
| 技术方案 | v0.9.48 是当前唯一目标口径；只修正灰度工作流上下文作用域，0.9.47 业务规则不变 |
| 全流程图 | v0.9.48；流程与 0.9.47 一致 |
| 发布状态 | `gray-v0.9.47` 因工作流表达式在创建 job 前失败且无产物；0.9.48 待定向复核、双仓推送、标签、灰度 ZIP 解压启动校验、配套后端部署和 Windows UAT |
