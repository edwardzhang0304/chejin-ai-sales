...
| 来源治理 | `0.9.30` OmniAuto/Worker 功能候选提交 `5c94a91cd956f37776be73f872b4c34ccaccd810`；`.chejin-source.json` 已绑定该真实候选，最终集成提交由 Git 标签和包内 manifest 固定 |
| PRD | v0.9.10 |
| 技术方案 | v0.9.28 权威方案不变；`0.9.30` 补齐已规定的重启 Flow 有限恢复出口，并增加本地/后端在途登记对账，不修改业务状态机 |
| 全流程图 | v0.9.28 源文件仍为权威流程；`0.9.30` 只补全既有恢复分支的实现和组合测试 |
| 发布状态 | `0.9.30` 功能与架构复审已通过，真实功能候选已形成；允许完成来源绑定、提交级快速复核、不可变标签与 Windows UAT ZIP。Windows 原故障 SQLite 重启验收仍待现场完成，不得声称自动化等于实机验收 |
