...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.2.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.2.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.2.puml`
...
| 灰度版本 | `0.9.2`；`0.9.0`、`0.9.1` 已冻结，本候选不得覆盖旧包 |
| 灰度分支 | `codex/gray-release-0.9.x`；从完整候选收口后建立，禁止混入下一期功能 |
| 当前机器合同 | `contract_revision=0.9.2`；Sidecar、Worker、后端校验、生成 Schema、样例、manifest 和测试同步，规范化合同 SHA 为 `435efe715c909b17b44b44941b9d0e6a80c8adf39d8c1c81f58f35b81e47ff1a` |
| 来源治理 | 独立 OmniAuto 固定提交为 `71a576486899e5dfc52dff6eb9e7bb0bf70285cd`；内嵌生成 Schema 与该提交一致，`.chejin-source.json` 已同步真实来源哈希 |
| PRD | v0.9.2 |
| 技术方案 | v0.9.2 |
| 全流程图 | v0.9.2 源文件 |
| 发布状态 | `0.9.2` 本地候选正在执行提交级门禁；通过后推送、固定标签并生成唯一 Windows 灰度 ZIP |

### 2.1 2026-08-14：加好友收尾与 Brain 可见回复恢复
...
### 2.2 2026-08-13：Worker 当前过程连续展示
...
- 灰度系列固定为 `0.9.x`。`0.9.0`、`0.9.1` 已冻结，当前候选为 `0.9.2`；后续每个内容不同且进入测试的
  不可变候选依次使用 `0.9.3、0.9.4……`，不得跳回旧号或覆盖同号内容。
...
- 灰度稳定分支固定为 `codex/gray-release-0.9.x`；每个不可变候选使用
  `gray-v0.9.0、gray-v0.9.1、gray-v0.9.2……` 标签。`main` 只接收完成灰度验收的确切标签提交。
