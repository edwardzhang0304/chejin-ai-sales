...
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.20.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.20.puml`
...
| 灰度版本 | 目标候选 `0.9.20`；`0.9.0` 至 `0.9.19` 已冻结，旧 revision/SHA 不得覆盖 |
...
| 当前机器合同 | 本地候选统一为 `contract_revision=0.9.20`；规范化 SHA `ca0b0dcb3362255fda103b9f3c32f947dd7f97f6991196f2a70251b5a6d0d44e`。`0.9.19` 已形成不可变候选，禁止覆盖 |
| 来源治理 | 独立 OmniAuto 固定提交 `1591942b872ef6d9db10e1922d441aff30c2c414` 已推送，`.chejin-source.json` 已回填真实提交号与 `0.9.20` scope |
| PRD | v0.9.10 |
| 技术方案 | v0.9.20；继承 0.9.19 全部生产语义，并将添加朋友页面收尾统一覆盖“邀请已发送”和“已经是好友”两条完成路径 |
| 全流程图 | v0.9.20 源文件；正式身份提交顺序与技术方案一致 |
| 发布状态 | `gray-v0.9.19` 及其 ZIP 保持冻结；`0.9.20` 是本次关窗修复后的唯一替代候选 |
