...
| 灰度版本 | 目标候选 `0.9.29`；`0.9.0` 至 `0.9.28` 已冻结，旧 revision/SHA/标签/ZIP 不得覆盖 |
...
| 当前机器合同 | `contract_revision=0.9.29`，规范化 SHA `24e0ec71bfb40ee484e67a394738b198314a7b74a213908406f9924a304a8e22`；生成 Schema 已同步且 current 检查通过 |
| 来源治理 | `0.9.29` OmniAuto/Worker 功能候选提交 `abc28876354761dc038ee4e99fa4f95e85ab342b`；`.chejin-source.json` 已绑定该真实候选，最终集成提交由 Git 标签和包内 manifest 固定 |
| PRD | v0.9.10 |
| 技术方案 | v0.9.28 权威方案不变；`0.9.29` 只补齐方案已规定的“重启旧 Flow 先恢复语音/图片 Journal，得到终态后结束旧流程并继续接单”客户端接线 |
| 全流程图 | v0.9.28 源文件仍为权威流程；`0.9.29` 不新增状态、接口或分支 |
| 发布状态 | `0.9.29` 重启旧 Flow 媒体 Journal 恢复接线、合同、生成 Schema 和来源候选已完成；两端定向门禁通过，允许推送并生成仅供实机验收的 Fast UAT ZIP。最终 ZIP SHA 和 Windows 重启恢复实机结果待本轮发布流程回填，不得将 Fast UAT ZIP 声称为正式客户包 |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.28` 已冻结，当前目标候选为 `0.9.29`；后续每个内容不同且进入测试的
