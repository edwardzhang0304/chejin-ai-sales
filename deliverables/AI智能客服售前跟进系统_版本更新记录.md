...
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.55.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.55.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | 灰度分支 `codex/gray-release-0.9.x` 的隔离工作区；完整继承 `0.9.54` 产品实现，本轮只固定 Windows 灰度门禁的 Python 控制台 UTF-8 输出 |
| 本轮整改父基线 | `0.9.54` 已冻结代码与标签，真实 OmniAuto 来源为 `b052bfa5a7333ad5651dc2de2e13ab10a74fe961`；原始生产能力底座仍为 `b6ad192`（`gray-v0.9.43`） |
| 灰度版本 | 本地候选 `0.9.55`；机器合同和生成 Schema 已同步，独立 OmniAuto 来源提交已形成，车金提交、推送和发布待本次来源治理完成 |
...
| 当前机器合同 | 本地候选 `contract_revision=0.9.55`；规范化 SHA `c290881a76f9701fac15221e48898c79101077b20bc07f937d1138a5d334e83f` |
| 来源治理 | `.chejin-source.json` 已绑定独立 OmniAuto 真实提交 `e65e363116791e43e1dd5da5a55c31c060237e09`；两仓生成 Schema 逐字一致 |
| PRD | v0.9.50；本轮不改变业务需求，无需为技术互斥修复升版 |
| 技术方案 | v0.9.55 是当前唯一目标口径；完整继承 0.9.54，只增加 Windows 灰度门禁报告 UTF-8 输出约束 |
| 全流程图 | v0.9.55；C0—C4 不变，补充 CI 控制台与产品子进程编码边界 |
| 发布状态 | 0.9.54 Windows 门禁已证明 116 条上下文链通过，但 209 项合同脚本打印中文报告时受默认代码页影响；0.9.55 仅修正门禁输出并等待推送、标签、灰度 ZIP和 Windows UAT |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.54` 已冻结且不得同名覆盖，当前技术方案目标候选为 `0.9.55`；后续每个内容不同且进入测试的
