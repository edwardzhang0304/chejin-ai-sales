...
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.56.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.56.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | 灰度分支 `codex/gray-release-0.9.x` 的隔离工作区；完整继承 `0.9.55` 产品和 CI 实现，本轮只把共享 LLM 总预算测试改为可控单调时钟 |
| 本轮整改父基线 | `0.9.55` 已冻结代码与标签，真实 OmniAuto 来源为 `e65e363116791e43e1dd5da5a55c31c060237e09`；原始生产能力底座仍为 `b6ad192`（`gray-v0.9.43`） |
| 灰度版本 | 本地候选 `0.9.56`；机器合同和生成 Schema 已同步，独立 OmniAuto 来源提交已形成，车金提交、推送和发布待本次来源治理完成 |
...
| 当前机器合同 | 本地候选 `contract_revision=0.9.56`；规范化 SHA `a94575d4dd275a59c47fb5c4be60794f6d55f5c6158176b08e3fdbcb9723577c` |
| 来源治理 | `.chejin-source.json` 已绑定独立 OmniAuto 真实提交 `7b25b0496d7fa82e7a73ea29e5caa38955a704d4`；两仓生成 Schema 逐字一致 |
| PRD | v0.9.50；本轮不改变业务需求，无需为技术互斥修复升版 |
| 技术方案 | v0.9.56 是当前唯一目标口径；完整继承 0.9.55，只增加共享总预算测试的确定性时钟约束 |
| 全流程图 | v0.9.56；C0—C4 不变，补充测试时钟与生产预算边界 |
| 发布状态 | 0.9.55 Windows 门禁中 115/116 条上下文测试通过，唯一失败为真实 `sleep` 受 Windows 调度粒度影响；0.9.56 仅修正该测试并等待推送、标签、灰度 ZIP和 Windows UAT |
