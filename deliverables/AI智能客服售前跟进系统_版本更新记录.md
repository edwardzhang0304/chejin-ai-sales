...
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.18.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.18.puml`
...
| 灰度版本 | 目标候选 `0.9.18`；`0.9.0` 至 `0.9.17` 已冻结，旧 revision/SHA 不得覆盖 |
...
| 当前机器合同 | 本地候选统一为 `contract_revision=0.9.18` / 规范化 SHA `cda58c5b0cfb53d21655f87128b35029b434d275ac929a23c09b0f8265b2919a`；`0.9.17` / SHA `a529986901e9e23e2cbfc57472229f680903487c67747668212b68930b782a5d` 已确认存在语音回归并作废，不得覆盖或打包 |
| 来源治理 | 本轮已先在独立 OmniAuto 形成本地固定提交 `80b07ef38d862ec5dccbff33646450724fd3c2c3`，再将真实提交号与 `0.9.18` 帧内 action binding scope 回填 `.chejin-source.json`；本轮不推送、不打包 |
| PRD | v0.9.10 |
| 技术方案 | v0.9.18；在 v0.9.17 MECE 模型上补齐独立帧内语音 action binding |
| 全流程图 | v0.9.18 源文件；正式身份提交顺序与技术方案一致 |
| 发布状态 | `0.9.18` 仅修复临时语音动作证据被正式消息白名单提前删除的发布阻断回归；完成定向证据和来源治理前不得推送、打包或现场替换 |
