...
版本：v0.9.68.1（文档修订；0.9.68 正式原地升级流程启动）
...
| 本轮版本 | `0.9.68` 正式发布准备；目标为现有 `0.9.67` 客户端内升级，门禁进行中 |
| 合同状态 | 当前候选 `0.9.68 / 23c30e5a5cce3b2e0d5e697fe2733cf670d3c8cde04665c68997e9762977f1a3`；已发布 0.9.67 合同保留历史登记 |
| 生产 / Windows 记录 | 正式 CI `34016248991` 成功；网站与更新下载验证通过；Windows 微信现场实机验收待完成 |
| 交付校验 | 既有可信公钥验签、ZIP/EXE/全文件清单哈希与运行探针通过 |
...
| 当前结论 | 0.9.68 代码复审及双仓推送完成，用户已授权正式升级流程；上表生产、交付校验均属于既有 0.9.67，不代表 0.9.68 验收 |
