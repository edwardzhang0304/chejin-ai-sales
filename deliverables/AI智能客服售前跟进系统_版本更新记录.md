...
版本：v0.9.67.5（正式发布重新授权；保留业务数据与配置）
...
| 本轮版本 | 目标 `0.9.67`；用户已重新授权提交、推送、正式 Windows EXE 与生产后台/后端部署；修复候选重新走门禁 |
...
| 生产 / Windows 记录 | 上次登记生产为 0.9.66；本轮公网 healthz/readyz 均 HTTP 200，未登录服务器核对版本、Worker/租约或数据；0.9.67 正式 CI 失败，Windows 实机仍待验收 |
...
| 当前结论 | 用户确认清理旧发布包、发布目录和日志，保留业务数据、账号、配置及绑定；不新增备份恢复和回滚，按新提交推进正式发布 |
