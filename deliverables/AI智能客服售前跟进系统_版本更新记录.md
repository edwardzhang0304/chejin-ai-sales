...
版本：v0.9.64（代码复审通过，双仓提交候选；构建交运维）
...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.64.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.64.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.64.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | 原灰度分支 `codex/gray-release-0.9.x`；本记录随复审通过的头像检测/分组候选提交 |
| 本轮整改父基线 | `0.9.63`，父提交 `75dc0290d411674304740a012b5666573db1b2e9`；不改写其标签或已生成包 |
| 灰度版本 | `0.9.64`，第三轮代码复审通过；继承生产默认地址 `https://jiangsuchejin.com/api` |
| 正式上线版本系列 | `1.0.x`；只接收通过灰度验收的正式上线能力和上线后的稳定性修复，不混入下一期优化 |
| 下一期优化版本系列 | `1.1.x`；待办及已完成移出项以本记录第 8 节为准，不反向覆盖 `1.0.x` 正式产物 |
| 灰度分支 | `codex/gray-release-0.9.x`；本轮仅头像检测/多行分组修复，不混入确认次数、AI 回复效果、向量语义检索、Qt 裁剪或其他优化 |
| 当前候选机器合同 | `0.9.64 / 840a770923bf6ef43c37c33086d817a0ff2d860cbb7c923c3b9c43f0bcf465bc`；正式生成器计算，Worker/后端/生成 Schema 一致 |
| 已发布父基线合同 | `0.9.63 / 18a82d17f94118c56e0f1497df6ff94a828a7db9daa3b9920d1dd9b9e136240e`；本轮未部署新合同，不改写旧标签或包 |
| 来源治理 | 独立来源 `edwardzhang0304/omniauto` 的原灰度来源分支已形成并推送 `03ddcd66cf22740c5c30537b0fb9e7873e24b50f`；父来源 `a29ba4e1278d55bc6aed617d045b0a85c10c4aff`；真实提交形成后才更新 `.chejin-source.json` |
| PRD | v0.9.64；无新产品功能，恢复多行回复正确识别与正常发送结算 |
| 技术方案 | v0.9.64；6.0.3.3.3 和 6.9 实现代码复审通过，主业务规则未改 |
| 全流程图 | v0.9.64；同步复审通过状态，主业务流程不变 |
| 发布状态 | 用户授权本轮仅双仓提交、来源登记及推送；不打包、不打标签、不部署，构建和 Windows 实机验收交运维 |
