...
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.59.md`
3. 本版本更新记录
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | 隔离工作区灰度分支 `codex/gray-release-0.9.x`；以真实 `0.9.58` 耗时观测提交 `967b429` 为底座实现唯一 `0.9.59` 候选 |
| 本轮整改父基线 | `0.9.56` 提交 `3da3dc9ebde882d83d043cfa2c58458898981fe7`；原始生产能力底座仍为 `b6ad192`（`gray-v0.9.43`） |
| 灰度版本 | 本地实现候选 `0.9.59`；耗时观测真实代码与客户端手动检查更新已合并验证，尚待架构复审和双仓提交治理 |
...
| 灰度分支 | `codex/gray-release-0.9.x`；只额外接收已批准提前的 `0.9.59` 耗时观测清理和客户端检查更新，禁止混入其他下一期功能 |
| 当前机器合同 | `contract_revision=0.9.59`；规范化 SHA `04595d8e0751b2b7ce7601ca86b7e708b6aa053830492bff4f4d19b5693e5297`，生成 Schema 已一致 |
| 来源治理 | `.chejin-source.json` 当前仍绑定 `0.9.58` 独立 OmniAuto 真实提交 `2a4a7ed9e459421c53865b0bbc9eeb9d88b3dd09`；待架构复审通过后先形成 `0.9.59` Schema 来源提交再更新 |
| PRD | v0.9.50；本轮不改变业务需求，无需为技术互斥修复升版 |
| 技术方案 | v0.9.59；是当前唯一技术口径；v0.9.57 已恢复为不可变历史文件 |
| 全流程图 | v0.9.57；本轮不修改业务流程，不创建第二张流程图 |
| 发布状态 | 0.9.59 为本地实现候选；未提交、未推送、未打标签、未打包，等待架构复审 |
