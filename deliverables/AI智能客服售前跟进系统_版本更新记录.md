...
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.17.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.17.puml`
...
| 灰度版本 | 目标候选 `0.9.17`；`0.9.0` 至 `0.9.16` 已冻结，MECE 编排不得覆盖旧 revision/SHA |
...
| 当前机器合同 | 本地候选已统一为 `contract_revision=0.9.17` / 规范化 SHA `a529986901e9e23e2cbfc57472229f680903487c67747668212b68930b782a5d`；既有 `0.9.16` / SHA `1d4ec83d52ea6ccd0d614f5578843a66aab8d6523733b95fd7f61a27060de9c5` 已冻结且明确拒绝，不得覆盖或打包 |
| 来源治理 | 上一固定 OmniAuto 提交为 `9c45ad157a9314605333ae76d29e4a0211356656`；本轮 `frame_visual_id` 通用改动必须先在独立 OmniAuto 形成新固定提交，再回填 `.chejin-source.json`，回填前禁止打包 |
| PRD | v0.9.10 |
| 技术方案 | v0.9.17；C2 身份与媒体编排已按 MECE 单一模型重写 |
| 全流程图 | v0.9.17 源文件；正式身份提交顺序与技术方案一致 |
| 发布状态 | `0.9.17` 收口 C2 编排：运行时对象统一为观察、待处理动作、正式消息、隔离记录；只有正式消息可进入 source key/Ledger/Outbox/ingest/Brain。当前代码与机器合同待按本口径复核整改，完成前不得提交发布候选、推送或打包 |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.9` 已冻结，当前候选为 `0.9.10`；后续每个内容不同且进入测试的
  不可变候选依次使用 `0.9.11、0.9.12……`，不得跳回旧号或覆盖同号内容。
