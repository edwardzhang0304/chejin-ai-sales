...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.44.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.44.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.44.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | `codex/gray-v0.9.44-minimal`；旧宽范围 `0.9.44` 工作树只保留审计证据，不得混入 |
| 本轮整改父基线 | `b6ad192`（`gray-v0.9.43`） |
| 灰度版本 | 目标候选 `0.9.44`；修复跨帧几何误判，并以最小范围统一“实际媒体动作结果后绑定正式身份”；不改 Brain、媒体优先顺序或业务 Handoff 条件 |
...
| 当前机器合同 | `contract_revision=0.9.44`；本地规范化 SHA 为 `7b951e1433d61266b3fd038c0705cf8ecb53d6e659579278d4bb019f27191bc4`；唯一连续性比较、图片临时动作槽位、技术故障零 Handoff 与生成 Schema 已同步，历次未包含最终语义的本地 SHA 均不合规 |
| 来源治理 | 父基线继续指向 `0.9.43` 的真实 OmniAuto 来源 `27c59c8a0e9c85106a12f05f6f92e0193fefb5af`；复审通过后先形成新的真实 OmniAuto 提交，再更新 `.chejin-source.json` |
| PRD | v0.9.44；补清“已确认内容处理失败可转人工，客户端动作不变量失败必须技术故障、禁止转人工”的产品终态边界 |
| 技术方案 | v0.9.44 是当前唯一目标口径；业务投影、长期身份、当前帧几何三类职责必须分离 |
| 全流程图 | v0.9.44；pre_send_refresh 与 S0/S1/S2 保持独立帧，跨帧只比较五字段业务序列 |
| 发布状态 | 权威文档、全流程图、代码、合同、生成 Schema 与定向自审已收口，架构复审通过；独立 OmniAuto 来源提交已固定为 `61f2f86b62caaf2bbb2afe51868819d7cfd10f54`，当前进入车金提交候选，尚未推送、打标签、生成 ZIP、部署或完成 Windows 实机 UAT |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.43` 已冻结且不得同名覆盖，当前目标候选为 `0.9.44`；后续每个内容不同且进入测试的
