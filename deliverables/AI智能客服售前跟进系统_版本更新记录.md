...
| 本轮版本 | 客户端 `0.9.69` 正式更新已登记；现有 0.9.67 / 0.9.68 查询返回可更新到 0.9.69 |
| 合同状态 | 沿用生产 `0.9.68 / 23c30e5a5cce3b2e0d5e697fe2733cf670d3c8cde04665c68997e9762977f1a3`，本次客户端修复未改 C2 合同 |
| 生产 / Windows 记录 | 正式构建/直传 `34106312819`、同提交 Fast UAT `34107799490`、生产检查 `34110260932`、发布/外网验证 `34110477182` 全部成功；用户 Windows 按钮和微信实机验收待完成 |
...
| 当前结论 | 运维发布与自动验收完成；Windows 本机点击升级及业务验收待用户完成 |
...
- 用户本机步骤：保持客户端暂停且无待处理任务，在原 0.9.67 设置页点击一次“检查更新”，等待下载、校验、安装与自动重启；确认版本 0.9.69、绑定/配置/业务数据，再验证受影响微信业务并恢复接单。失败时保留提示及诊断，不清数据或强行改写成功。旧程序退出后续写入仍可能触发严格拒绝；自动进程验收不代表本机按钮和业务已验收。
