...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.3.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.3.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.3.puml`
...
| 本轮整改父基线 | `abfcd957f04150024bd2788a273ccf0301456484`；最终候选 HEAD 由提交及包内 manifest 动态记录 |
| 灰度版本 | `0.9.3`；`0.9.0`、`0.9.1`、`0.9.2` 已冻结，本候选不得覆盖旧包 |
| 灰度分支 | `codex/gray-release-0.9.x`；从完整候选收口后建立，禁止混入下一期功能 |
| 当前机器合同 | `contract_revision=0.9.3`；规范化合同 SHA `558f6b21afbf6bfea58d3ba83476be780121b9adfdaf09058f657c8dac5c8d97`；Sidecar、Worker、后端校验、生成 Schema、样例、manifest 和测试同步 |
| 来源治理 | 独立 OmniAuto 固定提交 `5d316b797342684e4a1ac0616b9538ebfeb930bc`；内嵌副本与 `.chejin-source.json` 使用同一真实提交 |
| PRD | v0.9.3 |
| 技术方案 | v0.9.3 |
| 全流程图 | v0.9.3 源文件 |
| 发布状态 | `0.9.3` 正在执行“先文档、后代码”的本地整改；双仓提交级门禁通过后再推送、固定标签并生成唯一 Windows 灰度 ZIP |

### 2.1 2026-08-14：无车源时的 Brain 澄清与转人工承接收口
...
### 2.2 2026-08-14：加好友收尾与 Brain 可见回复恢复
...
### 2.3 2026-08-13：Worker 当前过程连续展示
...
- 灰度系列固定为 `0.9.x`。`0.9.0`、`0.9.1`、`0.9.2` 已冻结，当前候选为 `0.9.3`；后续每个内容不同且进入测试的
  不可变候选依次使用 `0.9.4、0.9.5……`，不得跳回旧号或覆盖同号内容。
...
- 灰度稳定分支固定为 `codex/gray-release-0.9.x`；每个不可变候选使用
  `gray-v0.9.0、gray-v0.9.1、gray-v0.9.2、gray-v0.9.3……` 标签。`main` 只接收完成灰度验收的确切标签提交。
