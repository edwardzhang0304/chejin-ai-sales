...
| 灰度版本 | 目标候选 `0.9.24`；`0.9.0` 至 `0.9.23` 已冻结，旧 revision/SHA 不得覆盖 |
...
| 当前机器合同 | `contract_revision=0.9.24`，规范化 SHA `290001afed30a8a68a1c8b48c48bf5f0af8b4d8dce1cbf87ddba666ffc6aea69`；生成 Schema 已同步且 current 检查通过 |
| 来源治理 | `0.9.24` OmniAuto 独立本地候选提交 `85205428914b4a1587d6cb21458fb001c8f1c6e3`，Worker/合同本地集成提交 `dc0e7464fe566308d493a95b20a3b5d24665eef2`；`.chejin-source.json` 已绑定新候选，尚未推送 |
| PRD | v0.9.10 |
...
| 发布状态 | `0.9.24` 代码、合同、来源候选、直接受影响定向验证及架构师复审已完成并批准推送；允许生成仅供实机验收的 Fast UAT ZIP。Windows 实机 UAT、正式发布门禁和客户可交付包仍待完成，不得将 Fast UAT ZIP 声称为正式包 |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.23` 已冻结，当前目标候选为 `0.9.24`；后续每个内容不同且进入测试的
