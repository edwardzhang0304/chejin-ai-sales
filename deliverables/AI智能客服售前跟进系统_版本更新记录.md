...
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.24.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.24.puml`
...
| 灰度版本 | 目标候选 `0.9.26`；`0.9.0` 至 `0.9.25` 已冻结，旧 revision/SHA/标签/ZIP 不得覆盖 |
...
| 当前机器合同 | `contract_revision=0.9.26`，规范化 SHA `1c939d25ea1e1ec22eb5dac985eb02632b9ed2bf5fed8df2a65a8791ef1fe486`；生成 Schema 已同步且 current 检查通过 |
| 来源治理 | `0.9.26` OmniAuto/Worker 功能候选提交 `780f3c00ee53a3c3fce26be21053fa3515a08b81`；`.chejin-source.json` 已绑定该真实候选，最终集成提交由 Git 标签和包内 manifest 固定 |
| PRD | v0.9.10 |
| 技术方案 | v0.9.24；启动按 DPI/工作区固定窗口档位并用一张真实客户区截图建立区域坐标地图；只替换 `0.9.20` 主窗口固定几何和复用无变化证据帧 |
| 全流程图 | v0.9.24 源文件；稳定外壳坐标由启动地图换算，会话行、消息、菜单、弹窗和表单继续使用 `0.9.20` 当前必要业务帧 |
| 发布状态 | `0.9.26` 语音、图片及后台状态修复、合同、生成 Schema、来源候选和直接受影响定向验证已完成并经架构师复审；允许提交、推送并生成仅供实机验收的新 Fast UAT ZIP。Windows 实际语音转写、正式发布门禁和客户可交付包仍待完成，不得将 Fast UAT ZIP 声称为正式包 |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.25` 已冻结，当前目标候选为 `0.9.26`；后续每个内容不同且进入测试的
