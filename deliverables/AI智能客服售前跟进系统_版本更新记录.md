...
| 灰度版本 | 目标候选 `0.9.28`；`0.9.0` 至 `0.9.27` 已冻结，旧 revision/SHA/标签/ZIP 不得覆盖 |
...
| 当前机器合同 | `contract_revision=0.9.28`，规范化 SHA `e0c13e862bf90a1c959d8755228800b7fb141263f807be6257ea382ddcb2d871`；生成 Schema 已同步且 current 检查通过 |
| 来源治理 | `0.9.28` OmniAuto/Worker 功能候选提交 `f66a57d51583d8225d32e9bc2cdb881798e3af49`；`.chejin-source.json` 已绑定该真实候选，最终集成提交由 Git 标签和包内 manifest 固定 |
| PRD | v0.9.10 |
| 技术方案 | v0.9.28；在既有接口、坐标、媒体和发送流程不变的前提下，只把 Worker 本地 Outbox 主键改为包含独立确定性的 `outbox_batch_key`，禁止同一外层读取 ID 覆盖不同入库事实 |
| 全流程图 | v0.9.28 源文件；明确首次读取与发送前新增消息可以在同一 Flow/read_run 内形成不同入库批次和独立 Outbox |
| 发布状态 | `0.9.28` Worker 本地 Outbox 幂等修复、“+”点击纵向约束、图片期间新文字生产链测试、合同、生成 Schema 和来源候选已完成并经架构师复审；允许推送并生成仅供实机验收的 Fast UAT ZIP。Windows 实际 OCR、鼠标点击、消息入库及发送验收仍待完成，不得将 Fast UAT ZIP 声称为正式客户包 |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.27` 已冻结，当前目标候选为 `0.9.28`；后续每个内容不同且进入测试的
