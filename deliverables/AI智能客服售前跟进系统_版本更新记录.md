...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.49.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.49.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.49.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | 原灰度分支 `codex/gray-release-0.9.x` 的隔离工作区；继承不可变 `gray-v0.9.48`，只修复旧坏 Outbox 全局门禁未释放 |
| 本轮整改父基线 | `b6ad192`（`gray-v0.9.43`） |
| 灰度版本 | 目标候选 `0.9.49`；只修改 Worker 旧 Outbox 有限隔离终态 |
...
| 当前机器合同 | `contract_revision=0.9.49`；本地规范化 SHA 为 `2302479941f86d8f854dd0169716d0eb89b1ba0f8952344728fefb86d4811289` |
| 来源治理 | `0.9.49` 独立 OmniAuto 只同步生成 Schema；真实来源提交为 `8100c2f8bd0cfcd80874d95d69b76e11c9c51a4f`，scope 为 `c2_contract_0_9_49_generated_schema` |
| PRD | v0.9.49；产品流程不变，仅修复客户端旧数据恢复终态 |
| 技术方案 | v0.9.49 是当前唯一目标口径；旧坏记录只隔离对应客户并释放全局门禁，当前版本缺陷继续故障关闭 |
| 全流程图 | v0.9.49；C0—C4 主流程与 0.9.48 一致 |
| 发布状态 | 本地实现及现场 SQLite 回放通过；待双仓来源提交、推送、标签、灰度 ZIP 解压启动校验、配套后端部署和 Windows UAT |
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.48` 已冻结且不得同名覆盖，当前目标候选为 `0.9.49`；后续每个内容不同且进入测试的
