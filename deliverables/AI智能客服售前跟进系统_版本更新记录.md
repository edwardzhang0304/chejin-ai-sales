...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.7.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.7.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.7.puml`
...
| 本轮整改父基线 | `de25d1900f247eb9c42621712224169b8243a313`（`gray-v0.9.6`） |
| 灰度版本 | `0.9.7`；`0.9.0` 至 `0.9.6` 已冻结，本候选不得覆盖旧包 |
| 灰度分支 | `codex/gray-release-0.9.x`；从完整候选收口后建立，禁止混入下一期功能 |
| 当前机器合同 | `contract_revision=0.9.7`；规范化合同 SHA `db87183f45e9b4fc8ad78484d51022e2d556091fe1e22f5639cce0af091fa38d`；Sidecar、Worker、后端校验、生成 Schema、样例、manifest 和测试同步 |
| 来源治理 | 当前独立 OmniAuto 固定提交为 `398d74c6f8ebef0d1b96890076c8ca0145979d65`；`0.9.7` 生成 Schema 已进入该提交，发送回执 OCR 分层属于明确登记的车金 overlay；`.chejin-source.json.source_commit` 已同步登记真实哈希 |
| PRD | v0.9.7 |
| 技术方案 | v0.9.7 |
| 全流程图 | v0.9.7 源文件 |
| 发布状态 | `0.9.7` AI 发送回执 OCR 整改候选；按授权完成提交、推送、审批、唯一 ZIP 和匹配后端部署 |

### 2.1 2026-08-15：AI 发送回执的文字类型与正文归属分层
...
### 2.3 2026-08-14：AI 回复气泡的发送结果补强确认
...
### 2.4 2026-08-14：无车源时的 Brain 澄清与转人工承接收口
...
### 2.5 2026-08-14：加好友收尾与 Brain 可见回复恢复
...
### 2.6 2026-08-13：Worker 当前过程连续展示
...
- 灰度系列固定为 `0.9.x`。`0.9.0` 至 `0.9.6` 已冻结，当前候选为 `0.9.7`；后续每个内容不同且进入测试的
  不可变候选依次使用 `0.9.8、0.9.9……`，不得跳回旧号或覆盖同号内容。
...
- 灰度稳定分支固定为 `codex/gray-release-0.9.x`；每个不可变候选使用
  `gray-v0.9.0、gray-v0.9.1、gray-v0.9.2、gray-v0.9.3、gray-v0.9.4、gray-v0.9.5、gray-v0.9.6……` 标签。`main` 只接收完成灰度验收的确切标签提交。
