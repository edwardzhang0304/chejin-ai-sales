...
1. `AI智能客服售前跟进系统_PRD_运营后台统一版_v0.9.61.md`
2. `AI智能客服售前跟进系统_技术方案手册_v0.9.61.md`
3. 本版本更新记录
4. `AI智能客服售前跟进系统_全流程图_v0.9.61.puml`
...
| 项目 | 当前值 |
|---|---|
| 候选工作区 | 灰度分支 `codex/gray-release-0.9.x`；知识管理为当前未提交工作区改动 |
| 本轮整改父基线 | 分支提交 `8678e888`；其中已包含 `0.9.60` 集成提交 `6e821fc7`，本轮不覆盖其标签或安装包 |
| 灰度版本 | 本地功能候选 `0.9.61`；只新增运营后台知识管理、不可变发布快照及 Brain 知识投影，不修改 Windows Worker 或 C0—C4 状态机 |
...
| 灰度分支 | `codex/gray-release-0.9.x`；`0.9.61` 只接收知识管理，不混入 AI 回复效果优化、向量语义检索、Qt 裁剪或其他优化 |
| 当前机器合同 | 已由正式生成器统一为 `contract_revision=0.9.61 / 637ba1843ac28056f88ac77b05014f65803a364426b72c30f649c7014a2d7bec` |
| 来源治理 | `.chejin-source.json` 记录独立 OmniAuto 真实提交 `a29ba4e1278d55bc6aed617d045b0a85c10c4aff`，只同步不可变知识发布投影、冻结历史检索和旧 KnowledgeRuntime 旁路隔离 |
| PRD | v0.9.61；现有 C0—C4 业务不变，本轮知识管理产品设计统一收口在该版本 |
| 技术方案 | v0.9.61；是当前知识管理唯一技术口径；v0.9.60 原始内容保留在 Git 历史，不在当前交付目录重复保留旧文件 |
| 全流程图 | v0.9.61；在唯一全流程图中补充知识草稿、发布、版本冻结和 Provider 使用链，不创建第二张流程图 |
| 发布状态 | 0.9.61 功能 P0、受影响测试、合同和双仓来源治理已收口；正式提交、推送、Windows EXE ZIP 和服务端部署由本次发布流程继续完成 |
