...
当前唯一架构口径继承 `0.9.57` 全部业务边界。`0.9.59` 合并两项已批准提前进入灰度的能力：15.6 重复耗时统计清理，以及 15.8 客户端“检查更新”。耗时观测仍是纯旁路；客户端更新只在运营人员点击按钮后启动，不修改 C0—C4、媒体编排、Brain、Handoff 或发送规则，不删除本地业务数据。当前隔离候选已以真实 `0.9.58` 耗时观测提交 `967b429` 为底座完成客户端、后端、机器合同、Schema、manifest 和构建脚本的 `0.9.59` 实现，规范化合同 SHA 为 `04595d8e0751b2b7ce7601ca86b7e708b6aa053830492bff4f4d19b5693e5297`；尚未形成新的独立 OmniAuto 来源提交或车金提交，未推送、未打标签、未打包，必须先通过架构复审。
...
6. 对外 HTTP 接口的唯一正式名称固定为“接口编号 + HTTP 方法 + 从 `/api` 开始的
   ...
   同义名称。
7. 灰度版本使用唯一 `0.9.x` 序列：`0.9.0` 至 `0.9.57` 已冻结，当前目标候选为 `0.9.59`；
   ...
   灰度发布版本；当前目标为 `contract_revision=0.9.59`，规范化 SHA 必须在客户端更新、后端发布清单、
   Schema 和生成物全部完成后由正式生成器真实计算，不得预填。本地实现候选已实算为 `04595d8e0751b2b7ce7601ca86b7e708b6aa053830492bff4f4d19b5693e5297`；独立 OmniAuto 来源、车金候选提交、标签和安装包仍须按发布流程真实形成。
...
8. 灰度稳定分支固定为 `codex/gray-release-0.9.x`，只接收当前灰度版本的缺陷修复、合同同步、
   ...
   15.8 客户端手动检查更新，共同目标为 `0.9.59`，
   不得借此带入其他下一期功能。每个不可变灰度候选使用精确标签
