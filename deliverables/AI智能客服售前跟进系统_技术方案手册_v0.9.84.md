...
文档版本：v0.9.84；合同基线：0.9.84；文档修订：r1（2026-09-16，正式强升候选，尚未发布）。

本次r11按用户“提交推送灰度分支”授权交付11号已通过独立源码和补证复审的客户插话续接修复。产品与已审测试逐字不变，只同步来源、复审证据与交付记录；不新建分支，不升版本/合同，不打包或部署。后端与客户端联合版本、Windows实际输入/插话/发送仍待验收，10号候选不包含在本结论内。双仓提交、分支与证据见版本记录r11。

## 0.9.84 正式发布准备

已审车金来源 `774c8f7909aa27531e8aa2479fded57cd22f8895`；共享来源 `f418b5dc2ab8650f2a19debd09f4fb55d43f479e`。复用 246 条去重正向源码证据，负向控制另记，0 项业务回归重跑；不把旧同步 HTTP 夹具当成正式异步续接证明。先核对发布工具凭证，正式包只构建一次，再验同包的实际起点保留数据安装，GitHub 直接传服务器，部署匹配 Docker 后端及生产 `/api` 后台。

保留账号、知识、销售、Worker 绑定／配置与全部业务数据；历史已转人工记录不批量恢复。沿用用户免新增备份、免自动回滚授权，失败时保留包和证据、停止对应阶段；未通过的场景不开放。Fast UAT 不变。以下旧版本源码复审及发布记录按发生当时保留。

## 当前版本与阅读说明

当前正式候选为 **0.9.84**，应用、合同和四份文档同步；生产仍为 **0.9.83**，本条不代表已上线。按用户要求保留数据安装（强升），不开放“检查更新”。本轮累计包含动态输入框、裁剪历史核对，以及客户插话后确认清稿、取消旧回复并继续读取新消息的已审修复。正式包和实际 Windows 输入／插话／发送仍待验收。
