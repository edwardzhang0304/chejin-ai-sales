...
文档版本：v0.9.84；合同基线：0.9.84；文档修订：r5（2026-09-16，加好友备注末尾光标修复按用户授权提交灰度分支，未发包）。
...
当前已发布 **0.9.84**：Windows 正式包、生产 Docker 后端与机器合同一致，运营后台接入生产 `/api`。按用户要求采用保留数据安装（强升），本轮不开放“检查更新”。本轮累计包含动态输入框、裁剪历史核对，以及客户插话后确认清稿、取消旧回复并继续读取新消息的已审修复。正式包及原装 0.9.83 保留数据安装通过；客户 Windows 实际输入／插话／发送仍待现场验收，不能据此承诺现场故障全部解决。
