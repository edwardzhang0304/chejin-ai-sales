...
当前版本继承不可变 `gray-v0.9.45`（`5680629`），其生产能力仍以 `gray-v0.9.43`（`b6ad192`）为原始稳定底座。历史版本的提交、合同 SHA、发布状态和变更原因只允许记录在《版本更新记录》，不在本文继续保留可能被误读为现行规则的旧实现说明。本文及配套全流程图是 `0.9.46` 唯一有效技术口径。
...
**实现状态：** `0.9.46` 已完成 C2 嵌套序列合同和旧 Outbox 有限失败终态的本地实现并通过架构复审。Worker 在 HTTP 前深度校验所有嵌套序列证据；存在消息或 Ledger 时，证据缺失或格式错误必须保留原始数据、隔离 Outbox、零 HTTP、将旧 Flow 结算为 `technical_failed` 并解除 `draining`。后端逐项验证 observation 的 `post_index` 实际位置和新增后缀连续性，拒绝错位、断裂和伪造载荷。规范化合同为 `0.9.46 / 5b7cc473f16062dd668a6880379b2375108985abd779f5dcb53deb628da5a371`；独立 OmniAuto 来源提交为 `69c69a60b7e08a31f9d773864b14b35faf86754b`，只同步生成合同 Schema。正常文字、语音、图片、混合消息、拆包、重启续传、Brain、媒体顺序和状态机未改变。车金提交、推送、标签、ZIP、配套后端部署和 Windows 实机 UAT 尚未完成，不得把自动化写成实机验收。
