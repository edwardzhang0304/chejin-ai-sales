...
| E2 | 销售人工回复后客户暂时不回。 | 状态sales_replied_waiting_user。 | 已覆盖 | 普通AI回复停止；到期可进入召回前检查。 |
...
| G4 | 等待销售接管时客户继续发消息。 | 保持waiting_sales_reply，AI不自由回复，由销售继续处理。 | 已覆盖 | 是否再次提醒销售当前不做。 |
...
| 编号 | 现实场景 | 当前方案处理 | 是否覆盖 | 备注/遗漏 |
|---|---|---|---|---|
| H1 | 销售回复后客户不回。 | sales_replied_waiting_user，到期可召回。 | 已覆盖 | 召回前仍须重新读取微信确认。 |
| H2 | 销售回复后客户继续问。 | 客户新消息入库，状态从sales_replied_waiting_user回到ai_active，由Brain继续判断回复、转人工或不动作。 | 已覆盖 | 销售回复本身不永久关闭AI。 |
| H3 | 销售希望完全停止AI与召回。 | 人工关闭自动化硬开关，或移除短码使会话closed。 | 已覆盖 | 普通销售回复不等于关闭硬开关。 |
