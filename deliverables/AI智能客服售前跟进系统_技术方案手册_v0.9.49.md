...
当前版本继承不可变 `gray-v0.9.48`（`5da224c`）；`0.9.48` 已通过的业务、灰度门禁和故障状态规则完整保留。本轮只修正旧版本不可修复 C2 Outbox 已结束旧 Flow、但仍以 `capability_paused` 留在全局重试门禁，导致人工开始后再次自动暂停的问题。其生产能力仍以 `gray-v0.9.43`（`b6ad192`）为原始稳定底座。历史版本的提交、合同 SHA、发布状态和变更原因只允许记录在《版本更新记录》，不在本文继续保留可能被误读为现行规则的旧实现说明。本文及配套全流程图是 `0.9.49` 唯一有效技术口径。
...
**实现状态：** `0.9.49` 本地实现已经完成，规范化合同为 `0.9.49 / 2302479941f86d8f854dd0169716d0eb89b1ba0f8952344728fefb86d4811289`。生产代码只修改 Worker 对旧版本不可修复 Outbox 的本地有限终态：原始 Outbox 原样保留、仅匹配的等待 Ledger 在同一 SQLite 事务内结算为 `failed + not_required`、当前客户进入既有 `identity_quarantined`，全局自动重试门禁释放；不请求后端、不操作微信、不清理整库。当前版本若自行产生同类坏证据，仍保持 `capability_paused` 和故障关闭，禁止用兼容逻辑掩盖新缺陷。独立 OmniAuto 只同步 `0.9.49` 生成合同 Schema，真实来源提交为 `8100c2f8bd0cfcd80874d95d69b76e11c9c51a4f`。正常文字、语音、图片、混合消息、Brain、媒体编排和后端业务接口未改变；自动化通过不代表 Windows 实机 UAT。
...
6. 对外 HTTP 接口的唯一正式名称固定为“接口编号 + HTTP 方法 + 从 `/api` 开始的
   ...
   同义名称。
7. 灰度版本使用唯一 `0.9.x` 序列：`0.9.0` 至 `0.9.48` 已冻结，当前目标候选为 `0.9.49`；
   ...
   灰度发布版本。`0.9.41` 至 `0.9.48` 已形成不可覆盖的代码、合同、标签或发布证据；当前目标为 `0.9.49`。客户端、后端、OmniAuto 生成 Schema、manifest 与打包入口最终必须统一使用 `contract_revision=0.9.49`；本地实现已真实计算规范化合同 SHA `2302479941f86d8f854dd0169716d0eb89b1ba0f8952344728fefb86d4811289`。OmniAuto 来源提交已固定；车金候选提交、GitHub Windows 门禁、Windows 实机 UAT 和正式包仍待完成。
...
目标机器合同 revision 为 `0.9.49`。代码、Worker 旧版本不可修复 Outbox 的本地有限隔离终态及生成 Schema 已按本文统一实现；规范化 SHA 已真实计算为 `2302479941f86d8f854dd0169716d0eb89b1ba0f8952344728fefb86d4811289`，独立 OmniAuto 来源提交为 `8100c2f8bd0cfcd80874d95d69b76e11c9c51a4f`。车金提交、打包、部署和 Windows UAT 尚未完成，不得把当前工作树写成已发布或已通过 Windows UAT：
...
**实现状态补充：** `0.9.43` 仍是生产能力的父基线，其规范化合同 SHA 和 OmniAuto 来源提交只用于校验基线真实性。`0.9.44` 关闭跨帧连续性与媒体动作结果绑定问题，`0.9.45` 完成 Windows 旧测试合同和 Pillow 兼容收口，`0.9.46` 至 `0.9.48` 完成嵌套合同、重启恢复、故障状态持久化和灰度门禁收口；它们均为不可覆盖历史版本。当前 `0.9.49` 只补齐旧版本不可修复 Outbox 的本地有限隔离终态，新合同 SHA 为 `2302479941f86d8f854dd0169716d0eb89b1ba0f8952344728fefb86d4811289`；独立 OmniAuto 来源提交为 `8100c2f8bd0cfcd80874d95d69b76e11c9c51a4f`，车金提交和发布产物仍须按来源治理顺序真实形成。
