...
文档版本：v0.9.86；合同基线：0.9.86；文档修订：r8（2026-09-17，意图修复源码复审通过，灰度提交，未发布）。
...
| 后端 | 沿用任务、claim-send、sent_ack、幂等、Flow和原Handoff/恢复规则；通过真实HTTP验证结算未退化 | 本轮不改技术失败转人工名单、会话状态机、数据库结构或HTTP接口；不把不转人工/自动恢复夹带进修复 |
...
正常业务的无新消息、转人工、销售回复继续使用原业务规则，不因本节重新归类为Worker技术故障。判定确属技术故障后，不借“临时重试”解除故障或继续接新单。
...
- A的旧事务只按A的原身份和事实收尾；微信当前停在B不能证明A已结清，也不能成为重发或丢弃A记录的理由。A已安全结束时，不仅因当前在B就永久禁止恢复；后续新任务仍走原目标定位与发送前校验。A原回复失败/取消后自动补发仍是1.1.x待办，不纳入本轮。
...
- 恢复成功不补发历史失败/取消回复，不转移客户所有权，不解除转人工。当前微信在B且列表重排时，A旧事只按原身份结清；重新接单才按当前画面和正常授权定位/读取/发送。1.1.x漏回复补偿、分句发送及一般混合版本运行仍不在本轮。
...
对应PRD 6.6及版本更新记录第8节第7项。按用户2026-09-09决定只登记，实际遇到后依据现场证据讨论方案；本节不是当前已实现能力，也不授权改代码或扩大本轮发布范围。
