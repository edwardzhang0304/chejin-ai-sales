...
当前唯一架构口径完整继承 `0.9.61` 的运营后台知识管理及既有 C0—C4 能力。`0.9.63` 只将 Windows 客户端默认 API 地址改为 `https://jiangsuchejin.com/api`，并将客户端、后端主合同、C2 生成 Schema、manifest 和权威文档的精确版本统一为 `0.9.63`。不修改 C2 字段语义、微信操作、媒体处理、Brain、Handoff 或 C0—C4 状态机；后端不接受旧合同号，发布必须按停止旧 Worker、结算本地事实、同步替换后端和客户端的顺序执行。
...
6. 对外 HTTP 接口的唯一正式名称固定为“接口编号 + HTTP 方法 + 从 `/api` 开始的
   ...
   同义名称。
7. 灰度版本使用唯一 `0.9.x` 序列：`0.9.0` 至 `0.9.62` 已冻结，当前目标候选为 `0.9.63`；
   ...
   灰度发布版本；当前目标为 `contract_revision=0.9.63`，规范化 SHA 必须在客户端、后端发布清单、
...
8. 灰度稳定分支固定为 `codex/gray-release-0.9.x`，只接收当前灰度版本的缺陷修复、合同同步、
   ...
   `0.9.63` 只额外接收生产地址与发布合同统一，不得借此带入 AI 回复效果优化、经验池、Prompt 管理或其他下一期功能。每个不可变灰度候选使用精确标签
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `contract_version` | integer | 是 | 协议结构代号固定为 `3`，不是发布版本；目标灰度版本由 `contract_revision=0.9.63` 表达，并同时校验最终重新生成的 `contract_sha256 / observation_schema_version`。 |
