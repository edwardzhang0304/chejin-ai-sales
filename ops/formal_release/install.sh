...
versions=[p.name for p in (root/'baseline').iterdir() if p.is_dir()]
assert versions and all(re.fullmatch(r'\d+\.\d+\.\d+', v) for v in versions)
...
config={'staging_root':'/var/lib/chejin-formal-staging','container':'chejin-leads-api',
        ...
        'client_baselines':{v:'/opt/chejin-formal-release/baseline/'+v for v in sorted(p.name for p in Path('/opt/chejin-formal-release/baseline').iterdir() if p.is_dir())},
        'manual_download_origin':'https://update.jiangsuchejin.com',
