"""Shared viewport projection plus the sole Worker continuity decision.

The imported OmniAuto helpers describe business facts visible in one frame.
Only this Worker module is allowed to decide whether two frames are
continuous.  Geometry remains available to Sidecar for a current-frame click,
but it is deliberately absent from every comparison below.
"""
...
def compare_business_viewport_continuity(
    ...
    *,
    ...
    """The only public Worker continuity comparator.

    When a caller consumes its one expansion opportunity, the expanded
