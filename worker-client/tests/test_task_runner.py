...
def committed_voice_recovery_observation(
    ...
    *,
    ...
    observation = {
        ...
        "parent_voice_anchor_key": f"anchor:{action_id}",
    ...
    observation["source_message"]["source_message_key"] = (
        committed.source_message_key
    )
...
def initialize_confirmed_voice_recovery_journal(
    *,
    ...
    initialize_action_journal(
        ...
        prepare_evidence={
            "pre_frame_id": pre_frame_id,
            "selected_pre_observation_id": pre_observation_id,
            "selected_action_token": f"token:{action_id}",
            "selected_target_fingerprint": f"fingerprint:{action_id}",
            "message_viewport_change_digest": "a" * 64,
        },
        items=[
            {
                "journal_item_id": action_id,
                "physical_anchor_keys": [f"anchor:{action_id}"],
            }
        ],
    )
    record_action_sequence_alignment(
        path,
        {
            ...
            "pre_frame_id": pre_frame_id,
            "post_frame_id": f"frame:{observation_id}",
...
def initialize_confirmed_image_recovery_journal(
    *,
    conversation_id: str,
    flow_id: str,
    action_id: str,
    ...
    initialize_action_journal(
        ...
        canonical_action_id=action_id,
        reserved_worker_stable_id=stable_id,
        pre_frame_id=f"frame:{observation_id}",
        pre_action_identity_sequence=[
            {
                "identity_state": "selected_action",
                "canonical_action_id": action_id,
...
class FakeBridge:
    ...
    def _contractual_message_payload(self, payload: dict) -> dict:
        ...
        if "observations" in payload:
            ...
            self._attach_image_frame_action_bindings(payload)
            return payload
    ...
    @staticmethod
    def _attach_image_frame_action_bindings(payload: dict) -> None:
        ...
        if not isinstance(payload.get("send_context_guard"), dict):
        ...
        payload.setdefault(
            ...
            {
                "schema_version": 2,
                "sequence": list(guard.get("sequence") or []),
                "message_count": int(guard.get("message_count") or 0),
    ...
    def execute_voice_action(self, *, display_name: str, rpa_session_key: str, **kwargs):
        ...
        payload.setdefault(
            "message_viewport_change_digest",
            str(kwargs.get("message_viewport_change_digest") or ""),
        )
        action_id = str(
        ...
        if (
            ...
            payload["send_context_guard"] = dict(
                future_payload.get("send_context_guard") or {}
            )
...
class TaskRunnerTest(unittest.TestCase):
    def test_v0932_new_suffix_system_errors_remain_specific(self):
        alignment = {
            "alignment_status": "unique",
            "new_suffix_observation_ids": ["system-new"],
        }
        common = {
            "observation_id": "system-new",
            "row_kind": "system_message",
            "sender_role": "system",
            "sender_role_source": "system",
            "message_type": "system",
        }
        cases = [
            (
                {**common, "content_clean": "", "system_classification": "unreadable"},
                "C2_PRE_SEND_SYSTEM_CONTENT_UNREADABLE",
            ),
            (
                {**common, "content_clean": "可读系统消息", "system_classification": "unresolved"},
            ...
            (
                {
                    **common,
                    ...
                    "sender_role_source": "",
                    "content_clean": "角色不明",
                    "system_classification": "ordinary",
                },
                "C2_PRE_SEND_MESSAGE_ROLE_UNCONFIRMED",
            ),
        ]
        for observation, error_code in cases:
            with self.subTest(error_code=error_code):
                result = pre_send_new_suffix_validation(
    ...
    def test_faulted_is_persisted_locally_before_backend_sync(self):
        ...
        with patch(
            "chejin_worker_client.task_runner.threading.Thread"
        ) as thread_factory:
            thread_factory.return_value.is_alive.return_value = False
            restarted.start(load_binding())

        self.assertEqual(
            restarted._pending_run_status_sync,
            "faulted",
        )
        api.heartbeat_run_status = "running"
        restarted.tick_once()
        self.assertEqual(restarted.binding.run_status, "faulted")
        self.assertNotIn("pull", api.events)
    ...
    def test_restart_without_finish_receipt_keeps_draining_flow(self):
        ...
        self.assertTrue(
            any(
                item.get("error_code")
                == "RUNTIME_INFLIGHT_FINISH_RECEIPT_MISSING"
                for item in read_logs(limit=20)
            )
        )

    def test_live_flow_created_after_start_is_not_adopted_by_restart_recovery(
        self,
    ):
        api = FakeApi(None)
        runner, _ = self.make_runner(
        ...
        binding = Binding(
            worker_id="worker-live-flow",
            worker_token="token",
            client_instance_id="client-live-flow",
            run_status="running",
        )
        runner.binding = binding
        flow_id = "read-live-current-process"
        begin_runtime_flow(flow_id, "c2_read")
        api.inflight_flow_id = flow_id
        api.inflight_flow_state = {
            "status": "active",
    ...
    def test_restart_local_flow_backend_empty_settles_then_pulls(self):
        api = FakeApi(None)
        runner, _ = self.make_runner(
            api,
            FakeBridge(
        ...
        binding = Binding(
            worker_id="worker-restart-local-only",
            worker_token="token",
            client_instance_id="client-restart-local-only",
            run_status="running",
        )
        runner.binding = binding
        flow_id = "read-restart-local-only"
        begin_runtime_flow(flow_id, "c2_read")
        runner._restart_recovery_flow_id = flow_id
    ...
    def test_startup_loop_reconciles_local_only_flow_before_first_pull(self):
        api = FakeApi(None)
        runner, _ = self.make_runner(
            api,
            FakeBridge(
        ...
        binding = Binding(
            worker_id="worker-startup-local-only",
            worker_token="token",
            client_instance_id="client-startup-local-only",
            run_status="running",
        )
        flow_id = "read-startup-local-only"
        begin_runtime_flow(flow_id, "c2_read")
        ...
        try:
            with patch(
                ...
                while "pull" not in api.events and time.monotonic() < deadline:
                    time.sleep(0.01)
        finally:
            runner.stop()
        ...
        self.assertIsNone(load_runtime_control()["inflight_flow_id"])
        self.assertIsNone(runner._restart_recovery_flow_id)
        first_pull_index = api.events.index("pull")
        self.assertTrue(
            any(
                event.startswith("heartbeat:")
                for event in api.events[:first_pull_index]
            ),
    ...
    def test_restart_local_flow_missing_media_sequence_pauses_without_pull(
        ...
        api = FakeApi(None)
        target = WechatReadTarget(
            conversation_id="conv-restart-orphan-journal",
            rpa_session_key="wx:rpa:v1:restart-orphan-journal",
            display_name="CJORPHAN",
            remark_code="CJORPHAN",
            read_reason="recoverable_hold",
            authorization_revision="revision-restart-orphan-journal",
            raw={"identity_checkpoint": identity_checkpoint()},
        )
        api.read_targets = [target]
        bridge = FakeBridge(
        ...
        runner, _ = self.make_runner(api, bridge)
        binding = Binding(
            worker_id="worker-restart-orphan-journal",
            worker_token="token",
            client_instance_id="client-restart-orphan-journal",
            run_status="running",
        )
        runner.binding = binding
        flow_id = "read-restart-orphan-journal"
        action_id = "voice-restart-orphan-journal"
        begin_runtime_flow(flow_id, "c2_read")
        runner._restart_recovery_flow_id = flow_id
        journal_path = action_journal_path("voice", action_id)
        initialize_action_journal(
            ...
            origin_read_run_id=flow_id,
            canonical_action_id=action_id,
            reserved_worker_stable_id="worker-message-orphan",
            pre_frame_id="voice-frame-orphan",
            pre_action_identity_sequence=[
                {
                    "identity_state": "selected_action",
                    "canonical_action_id": action_id,
                    "reserved_worker_stable_id": "worker-message-orphan",
                    "pre_observation_id": "voice-before-orphan",
                    "pre_sequence_index": 0,
                    "sender_role": "customer",
                    "message_type": "voice",
                }
            ...
            prepare_evidence={
                "pre_frame_id": "voice-frame-orphan",
                "selected_pre_observation_id": "voice-before-orphan",
                "selected_action_token": "voice-token-orphan",
                "selected_target_fingerprint": "voice-fingerprint-orphan",
                "message_viewport_change_digest": "a" * 64,
                "candidate_group_count": 1,
            },
            items=[
                {
                    "journal_item_id": action_id,
                    "physical_anchor_keys": ["voice-anchor-orphan"],
                }
            ],
        )
        update_action_journal_item(
        ...
        self.assertEqual(
            action_journal_phase(journal_path),
            "trigger_attempted",
        )
        self.assertEqual(binding.run_status, "paused")
        self.assertEqual(
        ...
        self.assertEqual(runner._restart_recovery_flow_id, flow_id)
        self.assertEqual(api.inflight_flow_events, [])
        self.assertNotIn("pull", api.events)
        self.assertTrue(
            any(
                item.get("error_code")
                == "C2_MEDIA_FACT_RECOVERY_SEQUENCE_MISSING"
                for item in read_logs(limit=30)
            )
        )
        self.assertEqual(bridge.locate_payloads, [])
    ...
    def test_restart_local_flow_backend_empty_replays_outbox_then_pulls(
        self,
    ):
        api = FakeApi(None)
        runner, _ = self.make_runner(
        ...
        binding = Binding(
            worker_id="worker-restart-orphan-outbox",
            worker_token="token",
            client_instance_id="client-restart-orphan-outbox",
            run_status="running",
        )
        runner.binding = binding
        flow_id = "read-restart-orphan-outbox"
        conversation_id = "conv-restart-orphan-outbox"
        begin_runtime_flow(flow_id, "c2_read")
        runner._restart_recovery_flow_id = flow_id
        outbox_id = enqueue_c2_outbox(
            {
                "read_run_id": flow_id,
                "conversation_id": conversation_id,
                "authorization_revision": "revision-restart-orphan-outbox",
                "messages": [],
                "evidence": {"test_case": "restart_orphan_outbox"},
            }
        )
        ...
        self.assertEqual(
            load_runtime_control()["inflight_flow_id"],
            flow_id,
            "the restart reconciler must not clear the flow before replay settles",
        )
        self.assertNotIn("pull", api.events)
        self.assertEqual(len(api.message_payloads), 1)
    ...
    def test_restart_local_flow_backend_empty_keeps_failed_outbox_and_blocks_pull(
        self,
    ):
        api = FakeApi(None)
        api.message_ingest_error = ConnectionError("backend offline")
        ...
        binding = Binding(
            worker_id="worker-restart-orphan-outbox-retry",
            worker_token="token",
            client_instance_id="client-restart-orphan-outbox-retry",
            run_status="running",
        )
        runner.binding = binding
        flow_id = "read-restart-orphan-outbox-retry"
        begin_runtime_flow(flow_id, "c2_read")
        runner._restart_recovery_flow_id = flow_id
        outbox_id = enqueue_c2_outbox(
            {
                "read_run_id": flow_id,
                "conversation_id": "conv-restart-orphan-outbox-retry",
                "authorization_revision": (
                    "revision-restart-orphan-outbox-retry"
                ),
                "messages": [],
                "evidence": {"test_case": "restart_orphan_outbox_retry"},
            }
        )
        ...
        self.assertNotIn("pull", api.events)
        self.assertIsNotNone(runner._restart_recovery_flow_id)

    def test_restart_sqlite_action_journal_builds_ledger_outbox_then_pulls(
        self,
    ):
        api = FakeApi(None)
        conversation_id = "conv-restart-sqlite-action"
        target = WechatReadTarget(
            conversation_id=conversation_id,
            rpa_session_key="wx:rpa:v1:restart-sqlite-action",
            display_name="CJSQL001",
            remark_code="CJSQL001",
            read_reason="fact_settlement",
            authorization_revision="revision-restart-sqlite-action",
        )
        api.read_targets = [target]
        bridge = FakeBridge(
            RpaResult(ok=True, result_code="unused", message="unused")
        ...
        runner, _ = self.make_runner(api, bridge)
        binding = Binding(
            worker_id="worker-restart-sqlite-action",
            worker_token="token",
            client_instance_id="client-restart-sqlite-action",
            run_status="running",
        )
        runner.binding = binding
        flow_id = "read-restart-sqlite-action"
        action_id = "voice-restart-sqlite-action"
        observation, source_key = committed_voice_recovery_observation(
            conversation_id,
            stable_id="worker-message-301",
            observation_id="voice-post-restart-sqlite-action",
            action_id=action_id,
            content="这是结果日志恢复的语音正文",
        )
        begin_runtime_flow(flow_id, "c2_read")
        runner._restart_recovery_flow_id = flow_id
        checkpoint_c2_action_outcomes(
            flow_id="c2-action-restart-sqlite",
            conversation_id=conversation_id,
            origin_read_run_id=flow_id,
            outcomes=[
                {
                    ...
                    "terminal_payload": {
                        "state": "completed",
                        "transcribed_message": {
                            "content_clean": observation["content_clean"],
                            "sender_role": "customer",
                            "parent_voice_anchor_key": observation[
    ...
    def test_restart_waiting_voice_ledger_builds_outbox_then_pulls(self):
        api = FakeApi(None)
        conversation_id = "conv-restart-waiting-voice"
        target = WechatReadTarget(
            conversation_id=conversation_id,
            rpa_session_key="wx:rpa:v1:restart-waiting-voice",
            display_name="CJWAIT01",
            remark_code="CJWAIT01",
            read_reason="fact_settlement",
            authorization_revision="revision-restart-waiting-voice",
        )
        api.read_targets = [target]
        bridge = FakeBridge(
            RpaResult(ok=True, result_code="unused", message="unused")
        ...
        runner, _ = self.make_runner(api, bridge)
        binding = Binding(
            worker_id="worker-restart-waiting-voice",
            worker_token="token",
            client_instance_id="client-restart-waiting-voice",
            run_status="running",
        )
        runner.binding = binding
        flow_id = "read-restart-waiting-voice"
        observation, source_key = committed_voice_recovery_observation(
            conversation_id,
            stable_id="worker-message-302",
            observation_id="voice-post-restart-waiting",
            action_id="voice-restart-waiting",
            content="这是 waiting Ledger 恢复的语音正文",
        )
        begin_runtime_flow(flow_id, "c2_read")
        runner._restart_recovery_flow_id = flow_id
        save_c2_ledger_terminal(
            ...
            source_message_key=source_key,
            origin_read_run_id=flow_id,
            dedupe_key="dedupe-restart-waiting-voice",
            message_type="voice",
            terminal_state="completed",
            ingest_state="waiting",
            result={
    ...
    def test_restart_waiting_voice_transport_retry_does_not_pause(self):
        api = FakeApi(None)
        api.message_ingest_error = ConnectionError("backend offline")
        conversation_id = "conv-restart-waiting-voice-retry"
        target = WechatReadTarget(
            conversation_id=conversation_id,
            rpa_session_key="wx:rpa:v1:restart-waiting-voice-retry",
            display_name="CJWAIT02",
            remark_code="CJWAIT02",
            read_reason="fact_settlement",
            authorization_revision="revision-restart-waiting-voice-retry",
        )
        api.read_targets = [target]
        bridge = FakeBridge(
            RpaResult(ok=True, result_code="unused", message="unused")
        ...
        runner, _ = self.make_runner(api, bridge)
        binding = Binding(
            worker_id="worker-restart-waiting-voice-retry",
            worker_token="token",
            client_instance_id="client-restart-waiting-voice-retry",
            run_status="running",
        )
        runner.binding = binding
        flow_id = "read-restart-waiting-voice-retry"
        observation, source_key = committed_voice_recovery_observation(
            conversation_id,
            stable_id="worker-message-305",
            observation_id="voice-post-restart-waiting-retry",
            action_id="voice-restart-waiting-retry",
            content="这是等待后端重试的语音正文",
        )
        begin_runtime_flow(flow_id, "c2_read")
        runner._restart_recovery_flow_id = flow_id
        save_c2_ledger_terminal(
            ...
            source_message_key=source_key,
            origin_read_run_id=flow_id,
            dedupe_key="dedupe-restart-waiting-voice-retry",
            message_type="voice",
            terminal_state="completed",
            ingest_state="waiting",
            result={
        ...
        with db_connection() as conn:
            waiting = [
                ...
                for row in conn.execute(
                    """
                    SELECT outbox_id, read_run_id, status
        ...
        self.assertEqual(waiting[0]["status"], "retry_waiting")
        self.assertNotIn("pull", api.events)
        self.assertEqual(bridge.locate_chats, [])
        self.assertEqual(bridge.message_reads, [])
        self.assertEqual(bridge.voice_transcribes, [])

    def test_restart_recovery_authorization_retries_then_pulls(self):
        api = FakeApi(None)
        conversation_id = "conv-restart-authorization-retry"
        target = WechatReadTarget(
            conversation_id=conversation_id,
            rpa_session_key="wx:rpa:v1:restart-authorization-retry",
            display_name="CJAUTH01",
            remark_code="CJAUTH01",
            read_reason="fact_settlement",
            authorization_revision="revision-restart-authorization-retry",
        )
        api.read_targets = [target]
        original_authorization = api.get_wechat_read_authorization
        authorization_attempts = 0

        def flaky_authorization(*args, **kwargs):
            nonlocal authorization_attempts
            authorization_attempts += 1
            if authorization_attempts == 1:
                raise ConnectionError("temporary authorization outage")
            return original_authorization(*args, **kwargs)

        api.get_wechat_read_authorization = flaky_authorization  # type: ignore[method-assign]
        bridge = FakeBridge(
            RpaResult(ok=True, result_code="unused", message="unused")
        )
        runner, _ = self.make_runner(api, bridge)
        binding = Binding(
            worker_id="worker-restart-authorization-retry",
            worker_token="token",
            client_instance_id="client-restart-authorization-retry",
            run_status="running",
        )
        runner.binding = binding
        flow_id = "read-restart-authorization-retry"
        observation, source_key = committed_voice_recovery_observation(
            conversation_id,
            stable_id="worker-message-306",
            observation_id="voice-post-authorization-retry",
            action_id="voice-authorization-retry",
            content="授权恢复后继续处理",
        )
        begin_runtime_flow(flow_id, "c2_read")
        runner._restart_recovery_flow_id = flow_id
        save_c2_ledger_terminal(
            ...
            source_message_key=source_key,
            origin_read_run_id=flow_id,
            dedupe_key="dedupe-restart-authorization-retry",
            message_type="voice",
            terminal_state="completed",
            ingest_state="waiting",
            result={
        ...
        self.assertGreaterEqual(authorization_attempts, 2)
        self.assertEqual(binding.run_status, "running")
        self.assertEqual(len(api.message_payloads), 1)
        self.assertIsNone(load_runtime_control()["inflight_flow_id"])
        self.assertIn("pull", api.events)
    ...
    def test_restart_waiting_image_ledger_builds_outbox_then_pulls(self):
        api = FakeApi(None)
        conversation_id = "conv-restart-waiting-image"
        target = WechatReadTarget(
            conversation_id=conversation_id,
            rpa_session_key="wx:rpa:v1:restart-waiting-image",
            display_name="CJIMG001",
            remark_code="CJIMG001",
            read_reason="fact_settlement",
            authorization_revision="revision-restart-waiting-image",
        )
        api.read_targets = [target]
        bridge = FakeBridge(
            RpaResult(ok=True, result_code="unused", message="unused")
        ...
        runner, _ = self.make_runner(api, bridge)
        binding = Binding(
            worker_id="worker-restart-waiting-image",
            worker_token="token",
            client_instance_id="client-restart-waiting-image",
            run_status="running",
        )
        runner.binding = binding
    ...
    def test_restart_confirmed_voice_journal_ingests_once_without_ui_then_pulls(
        self,
    ):
        api = FakeApi(None)
        conversation_id = "conv-restart-confirmed-voice"
        target = WechatReadTarget(
            conversation_id=conversation_id,
            rpa_session_key="wx:rpa:v1:restart-confirmed-voice",
            display_name="CJVOICE9",
            remark_code="CJVOICE9",
            read_reason="fact_settlement",
            authorization_revision="revision-restart-confirmed-voice",
        )
        api.read_targets = [target]
        bridge = FakeBridge(
            RpaResult(ok=True, result_code="unused", message="unused")
        ...
        runner, _ = self.make_runner(api, bridge)
        binding = Binding(
            worker_id="worker-restart-confirmed-voice",
            worker_token="token",
            client_instance_id="client-restart-confirmed-voice",
            run_status="running",
        )
        runner.binding = binding
        flow_id = "read-restart-confirmed-voice"
        action_id = "voice-restart-confirmed-action"
        reserved_id = "worker-message-303"
        begin_runtime_flow(flow_id, "c2_read")
        runner._restart_recovery_flow_id = flow_id
        journal_path = action_journal_path("voice", action_id)
        initialize_action_journal(
            ...
            canonical_action_id=action_id,
            reserved_worker_stable_id=reserved_id,
            pre_frame_id="voice-restart-confirmed-pre",
            pre_action_identity_sequence=[
                {
                    "identity_state": "selected_action",
                    "canonical_action_id": action_id,
                    "reserved_worker_stable_id": reserved_id,
                    "pre_observation_id": "voice-restart-confirmed-pre",
                    "pre_sequence_index": 0,
                    "sender_role": "customer",
                    "message_type": "voice",
                }
            ...
            prepare_evidence={
                ...
                "selected_action_token": "voice-restart-confirmed-token",
                "selected_target_fingerprint": (
                    "voice-restart-confirmed-fingerprint"
                ),
                "message_viewport_change_digest": "a" * 64,
            },
            items=[
                {
                    "journal_item_id": action_id,
                    "physical_anchor_keys": [
                        "voice-restart-confirmed-anchor"
                    ],
                }
            ],
        )
        record_action_sequence_alignment(
            journal_path,
            {
                "pre_sequence_source": "action_frame",
                "pre_frame_id": "voice-restart-confirmed-pre",
                "post_frame_id": "voice-restart-confirmed-post-frame",
                "alignment_status": "unique",
                "candidate_alignment_count": 1,
                "matched_pairs": [
                    {
    ...
    def test_restart_two_voice_journals_use_worker_sequence_not_filename(self):
        api = FakeApi(None)
        conversation_id = "conv-restart-two-voice-order"
        target = WechatReadTarget(
            conversation_id=conversation_id,
            rpa_session_key="wx:rpa:v1:restart-two-voice-order",
            display_name="CJORDER1",
            remark_code="CJORDER1",
            read_reason="fact_settlement",
            authorization_revision="revision-restart-two-voice-order",
        )
        api.read_targets = [target]
        bridge = FakeBridge(
            RpaResult(ok=True, result_code="unused", message="unused")
        ...
        runner, _ = self.make_runner(api, bridge)
        binding = Binding(
            worker_id="worker-restart-two-voice-order",
            worker_token="token",
            client_instance_id="client-restart-two-voice-order",
            run_status="running",
        )
        runner.binding = binding
        flow_id = "read-restart-two-voice-order"
        begin_runtime_flow(flow_id, "c2_read")
        runner._restart_recovery_flow_id = flow_id
        first_path = initialize_confirmed_voice_recovery_journal(
            conversation_id=conversation_id,
            flow_id=flow_id,
            action_id="voice-order-4",
            stable_id="worker-message-401",
            observation_id="voice-order-post-1",
            content="第一条语音",
        )
        second_path = initialize_confirmed_voice_recovery_journal(
            conversation_id=conversation_id,
            flow_id=flow_id,
            action_id="voice-order-5",
            stable_id="worker-message-402",
            observation_id="voice-order-post-2",
            content="第二条语音",
        )
        self.assertGreater(first_path.name, second_path.name)
        ...
        self.assertEqual(
            [
                message["content"]
                for message in api.message_payloads[0]["messages"]
            ],
            ["第一条语音", "第二条语音"],
        )
    ...
    def test_restart_voice_image_journals_share_worker_sequence_order(self):
        api = FakeApi(None)
        conversation_id = "conv-restart-mixed-media-order"
        target = WechatReadTarget(
            conversation_id=conversation_id,
            rpa_session_key="wx:rpa:v1:restart-mixed-media-order",
            display_name="CJORDER2",
            remark_code="CJORDER2",
            read_reason="fact_settlement",
            authorization_revision="revision-restart-mixed-media-order",
        )
        api.read_targets = [target]
        bridge = FakeBridge(
            RpaResult(ok=True, result_code="unused", message="unused")
        ...
        runner, _ = self.make_runner(api, bridge)
        binding = Binding(
            worker_id="worker-restart-mixed-media-order",
            worker_token="token",
            client_instance_id="client-restart-mixed-media-order",
            run_status="running",
        )
        runner.binding = binding
        flow_id = "read-restart-mixed-media-order"
        begin_runtime_flow(flow_id, "c2_read")
        runner._restart_recovery_flow_id = flow_id
        voice_path = initialize_confirmed_voice_recovery_journal(
            conversation_id=conversation_id,
            flow_id=flow_id,
            action_id="voice-mixed-2",
            stable_id="worker-message-501",
            observation_id="voice-mixed-post-1",
            content="先收到的语音",
        )
        image_path = initialize_confirmed_image_recovery_journal(
            conversation_id=conversation_id,
            flow_id=flow_id,
            action_id="image-mixed-4",
            stable_id="worker-message-502",
            observation_id="image-mixed-post-2",
        )
        self.assertGreater(voice_path.name, image_path.name)
    ...
    def test_restart_duplicate_media_sequence_pauses_without_guessing(self):
        api = FakeApi(None)
        conversation_id = "conv-restart-media-sequence-conflict"
        api.read_targets = [
            WechatReadTarget(
                conversation_id=conversation_id,
                rpa_session_key="wx:rpa:v1:restart-media-sequence-conflict",
                display_name="CJORDER3",
                remark_code="CJORDER3",
                read_reason="fact_settlement",
                authorization_revision=(
                    "revision-restart-media-sequence-conflict"
                ),
            )
        ]
        bridge = FakeBridge(
        ...
        runner, _ = self.make_runner(api, bridge)
        binding = Binding(
            worker_id="worker-restart-media-sequence-conflict",
            worker_token="token",
            client_instance_id="client-restart-media-sequence-conflict",
            run_status="running",
        )
        runner.binding = binding
        flow_id = "read-restart-media-sequence-conflict"
        begin_runtime_flow(flow_id, "c2_read")
        runner._restart_recovery_flow_id = flow_id
        initialize_confirmed_voice_recovery_journal(
            conversation_id=conversation_id,
            flow_id=flow_id,
            action_id="voice-sequence-conflict-1",
            stable_id="worker-message-601",
            observation_id="voice-sequence-conflict-post-1",
            content="冲突语音一",
        )
        initialize_confirmed_voice_recovery_journal(
            conversation_id=conversation_id,
            flow_id=flow_id,
            action_id="voice-sequence-conflict-2",
            stable_id="worker-message-601",
            observation_id="voice-sequence-conflict-post-2",
            content="冲突语音二",
        )
    ...
    def test_restart_waiting_voice_without_replay_evidence_pauses_explicitly(
        self,
    ):
        api = FakeApi(None)
        runner, _ = self.make_runner(
        ...
        binding = Binding(
            worker_id="worker-restart-voice-evidence-missing",
            worker_token="token",
            client_instance_id="client-restart-voice-evidence-missing",
            run_status="running",
        )
        runner.binding = binding
        flow_id = "read-restart-voice-evidence-missing"
        conversation_id = "conv-restart-voice-evidence-missing"
        begin_runtime_flow(flow_id, "c2_read")
        runner._restart_recovery_flow_id = flow_id
        save_c2_ledger_terminal(
            conversation_id=conversation_id,
            source_message_key="source-restart-voice-evidence-missing",
            origin_read_run_id=flow_id,
            dedupe_key="dedupe-restart-voice-evidence-missing",
            message_type="voice",
            terminal_state="completed",
            ingest_state="waiting",
    ...
    def test_restart_backend_flow_local_empty_pauses_without_pull(self):
        api = FakeApi(None)
        api.inflight_flow_state = {
            "status": "active",
            "flow_id": "read-backend-only",
            "flow_kind": "c2_read",
        }
        runner, seen = self.make_runner(
            api,
        ...
        binding = Binding(
            worker_id="worker-restart-backend-only",
            worker_token="token",
            client_instance_id="client-restart-backend-only",
            run_status="running",
        )
        runner.binding = binding
        ...
        self.assertTrue(
            any(
                item.get("error_code")
                == "RUNTIME_INFLIGHT_LOCAL_STATE_MISSING"
                for item in read_logs(limit=30)
            )
        )

    def test_restart_probe_with_both_flow_states_empty_allows_pull(self):
        api = FakeApi(None)
        runner, _ = self.make_runner(
            api,
            FakeBridge(
        ...
        binding = Binding(
            worker_id="worker-restart-both-empty",
            worker_token="token",
            client_instance_id="client-restart-both-empty",
            run_status="running",
        )
        runner.binding = binding
        runner._restart_backend_probe_pending = True
        ...
        self.assertFalse(runner._restart_backend_probe_pending)
        self.assertEqual(binding.run_status, "running")
        self.assertIn("pull", api.events)

    def test_restart_memory_pointer_without_sqlite_flow_pauses(self):
        api = FakeApi(None)
        runner, _ = self.make_runner(
            api,
            FakeBridge(
        ...
        binding = Binding(
            worker_id="worker-restart-pointer-missing",
            worker_token="token",
            client_instance_id="client-restart-pointer-missing",
            run_status="running",
        )
        runner.binding = binding
        runner._restart_recovery_flow_id = "read-restart-pointer-missing"
        ...
        self.assertTrue(
            any(
                item.get("error_code")
                == "RUNTIME_INFLIGHT_LOCAL_POINTER_MISSING"
                for item in read_logs(limit=30)
            )
        )
    ...
    def test_restart_memory_and_sqlite_flow_id_mismatch_pauses(self):
        ...
        binding = Binding(
            worker_id="worker-restart-pointer-mismatch",
            worker_token="token",
            client_instance_id="client-restart-pointer-mismatch",
            run_status="running",
        )
        runner.binding = binding
        begin_runtime_flow("read-restart-sqlite", "c2_read")
        runner._restart_recovery_flow_id = "read-restart-memory"
        ...
        self.assertTrue(
            any(
                item.get("error_code")
                == "RUNTIME_INFLIGHT_LOCAL_POINTER_MISMATCH"
                for item in read_logs(limit=30)
            )
        )

    def test_restart_local_task_flow_backend_empty_pauses_and_preserves_local(
        self,
    ):
        api = FakeApi(None)
        runner, _ = self.make_runner(
        ...
        binding = Binding(
            worker_id="worker-restart-task-local-only",
            worker_token="token",
            client_instance_id="client-restart-task-local-only",
            run_status="running",
        )
        runner.binding = binding
        begin_runtime_flow("task-restart-local-only", "task")
        runner._restart_recovery_flow_id = "task-restart-local-only"
        ...
        self.assertEqual(
            load_runtime_control()["inflight_flow_id"],
            "task-restart-local-only",
        )
        self.assertNotIn("pull", api.events)
        self.assertTrue(
            any(
                item.get("error_code")
                == "RUNTIME_INFLIGHT_TASK_BACKEND_STATE_MISSING"
                for item in read_logs(limit=30)
            )
        )

    def test_restart_local_backend_flow_id_mismatch_pauses_and_preserves_local(
        self,
    ):
        api = FakeApi(None)
        api.inflight_flow_state = {
            "status": "active",
            "flow_id": "read-backend-different",
            "flow_kind": "c2_read",
        }
        runner, _ = self.make_runner(
            api,
        ...
        binding = Binding(
            worker_id="worker-restart-mismatch",
            worker_token="token",
            client_instance_id="client-restart-mismatch",
            run_status="running",
        )
        runner.binding = binding
        begin_runtime_flow("read-local-old", "c2_read")
        runner._restart_recovery_flow_id = "read-local-old"
        ...
        self.assertTrue(
            any(
                item.get("error_code")
                == "RUNTIME_INFLIGHT_FLOW_ID_MISMATCH"
                for item in read_logs(limit=30)
            )
        )

    def test_restart_matching_flow_with_unknown_backend_status_pauses(self):
        api = FakeApi(None)
        runner, _ = self.make_runner(
            api,
            FakeBridge(
        ...
        binding = Binding(
            worker_id="worker-restart-status-unknown",
            worker_token="token",
            client_instance_id="client-restart-status-unknown",
            run_status="running",
        )
        runner.binding = binding
        flow_id = "read-restart-status-unknown"
        begin_runtime_flow(flow_id, "c2_read")
        runner._restart_recovery_flow_id = flow_id
        api.inflight_flow_state = {
            "status": "unknown",
        ...
        self.assertTrue(
            any(
                item.get("error_code")
                == "RUNTIME_INFLIGHT_BACKEND_STATE_INVALID"
                for item in read_logs(limit=30)
            )
        )

    def test_restart_local_flow_with_multiple_conversations_pauses(self):
        api = FakeApi(None)
        runner, _ = self.make_runner(
            api,
            FakeBridge(
        ...
        binding = Binding(
            worker_id="worker-restart-scope-conflict",
            worker_token="token",
            client_instance_id="client-restart-scope-conflict",
            run_status="running",
        )
        runner.binding = binding
        flow_id = "read-restart-scope-conflict"
        begin_runtime_flow(flow_id, "c2_read")
        runner._restart_recovery_flow_id = flow_id
        for index in (1, 2):
            save_c2_ledger_terminal(
                conversation_id=f"conv-restart-scope-{index}",
                source_message_key=f"source-restart-scope-{index}",
                origin_read_run_id=flow_id,
                dedupe_key=f"dedupe-restart-scope-{index}",
                message_type="text",
                terminal_state="completed",
        ...
        self.assertTrue(
            any(
                item.get("error_code")
                == "RUNTIME_INFLIGHT_LOCAL_FLOW_SCOPE_CONFLICT"
                for item in read_logs(limit=30)
            )
        )

    def test_restart_matching_flow_without_receipt_uses_durable_owner(self):
        api = FakeApi(None)
        runner, _ = self.make_runner(
            api,
            FakeBridge(
        ...
        binding = Binding(
            worker_id="worker-restart-owner",
            worker_token="token",
            client_instance_id="client-restart-owner",
            run_status="running",
        )
        runner.binding = binding
        flow_id = "read-restart-durable-owner"
        conversation_id = "conv-restart-durable-owner"
        begin_runtime_flow(flow_id, "c2_read")
        runner._restart_recovery_flow_id = flow_id
        api.inflight_flow_id = flow_id
        api.inflight_flow_state = {
        ...
        save_c2_ledger_terminal(
            conversation_id=conversation_id,
            source_message_key="source-restart-durable-owner",
            origin_read_run_id=flow_id,
            dedupe_key="dedupe-restart-durable-owner",
            message_type="text",
            terminal_state="completed",
            ingest_state="confirmed",
            result={"state": "completed"},
        ...
        self.assertIsNone(load_runtime_control()["inflight_flow_id"])
        self.assertIn(
            f"finish:{flow_id}:read_confirmed:{conversation_id}:",
            api.inflight_flow_events,
        )
        self.assertIn("pull", api.events)
    ...
    def test_layout_invalid_settlement_faults_worker_and_preserves_error(self):
        task = self.make_chat_reply_task(task_id="task-layout-invalid")
        api = FakeApi(task)
        runner, _ = self.make_runner(
            api,
            FakeBridge(
    ...
    def test_layout_invalid_without_reply_task_still_faults_worker(self):
        api = FakeApi(None)
        runner, _ = self.make_runner(
            api,
            FakeBridge(
    ...
    def test_specific_reidentification_error_is_not_rewritten(self):
        task = self.make_chat_reply_task(task_id="task-role-unconfirmed")
        api = FakeApi(task)
        runner, _ = self.make_runner(
            api,
            FakeBridge(
        ...
        self.assertEqual(
            api.events.count(
                "fail:C2_PRE_SEND_MESSAGE_ROLE_UNCONFIRMED:pre_send_refresh"
            ),
            1,
        )

    def test_pre_send_layout_failure_uploads_nested_durable_evidence(self):
        task = self.make_chat_reply_task(
            task_id="task-layout-evidence-upload"
        )
        api = FakeApi(task)
        runner, _ = self.make_runner(
            api,
        ...
        evidence = {
            ...
            "layout_evidence": {
                ...
                "ocr_evidence": [{"text": "CJTEST01"}],
                "screenshot_path": "C:/artifacts/layout-invalid.png",
            },
        }
        ...
        self.assertTrue(settled)
        self.assertEqual(len(api.evidence_payloads), 1)
        uploaded = api.evidence_payloads[0]
        self.assertEqual(
            uploaded["evidence_path"],
            "C:/artifacts/layout-invalid.png",
        )
        self.assertEqual(
            uploaded["metadata"]["pre_send_failure_evidence"],
            evidence,
        )
                    ...
                    },
                    {
                        ...
                        "image_physical_anchor": {
                            ...
                            "preceding_stable_message": (
                                "stable-text-before-media"
                            ),
                            "following_stable_message": (
                                "voice-customer-failed"
                            ),
                            "occurrence_index": 0,
                            "bubble_visual_fingerprint": (
            ...
            payload = bridge._contractual_message_payload(
                {
                    ...
                    "message_viewport_change_digest": str(
                        kwargs["message_viewport_change_digest"]
                    ),
            ...
            sidecar_observations = [
                {
                    ...
                    if not str(key).startswith("_worker_")
                }
                for observation in initial["observations"]
            ]
            historical = dict(sidecar_observations[0])
            return bridge._contractual_message_payload(
                {
                    **initial,
    ...
    def test_pre_send_first_win32_voice_without_native_id_executes_once(self):
        api = FakeApi(None)
        target = WechatReadTarget(
            conversation_id="conv-pre-send-first-win32-voice",
            rpa_session_key="wx:pre-send-first-win32-voice",
            display_name="CJWINV33",
            remark_code="CJWINV33",
            authorization_revision="revision-pre-send-first-win32-voice",
            raw={"identity_checkpoint": identity_checkpoint()},
        )
        bridge = FakeBridge(
            RpaResult(ok=True, result_code="unused", message="unused")
                ...
                "messages": [
                    {
                        ...
                        "content": '[语音] 4"',
                        "voice_duration": 4,
                        "voice_anchor_stable_key": "win32-voice-anchor",
                    }
                ]
            }
        )
        ...
        bridge.voice_payload = {
            "ok": True,
            "state": "voice_transcribe_completed",
            "processed_voice_anchor_keys": ["win32-voice-anchor"],
            "failed_voice_anchor_keys": [],
            "transcribed_messages": [
                {
                    "content": "我想买一辆二手车",
            ...
            "item_action_outcomes": [
                {
                    "physical_anchor_keys": ["win32-voice-anchor"],
                    "action_phase": "confirmed",
                    "business_state": "completed",
                    "business_result_confirmed": True,
                }
        ...
        result = runner._finish_new_visible_voices_in_current_chat(
            ...
            target=target,
            target_label="CJWINV33",
            sidecar_payload=initial,
            lease=unittest.mock.Mock(),
            action_cancel_requested=lambda: False,
            enforce_read_targets=False,
            read_run_id="read-pre-send-first-win32-voice",
            excluded_voice_anchor_keys=set(),
            flow_outcomes=FlowOutcomeAccumulator(
                origin_read_run_id="read-pre-send-first-win32-voice"
            ),
            operation_phase="pre_send_refresh",
        )
    ...
    def test_pre_send_same_duration_voice_replacement_is_zero_click_ambiguous(
        ...
        api = FakeApi(None)
        target = WechatReadTarget(
            conversation_id="conv-fixed-capacity-voice-replacement",
            rpa_session_key="wx:fixed-capacity-voice-replacement",
            display_name="CJREPL32",
            remark_code="CJREPL32",
            authorization_revision="revision-fixed-capacity-replacement",
            raw={"identity_checkpoint": identity_checkpoint()},
        )
        bridge = FakeBridge(RpaResult(ok=True, result_code="unused"))
        old_voice = {
            "schema_version": 3,
            "observation_id": "win32_ocr:same-frame-local-voice-id",
            "row_kind": "voice_bubble",
            "sender_role": "customer",
            "sender_role_source": "same_row_avatar",
            "message_type": "voice",
            ...
            "voice_anchor_key": "old-voice-anchor",
            "bubble_rect": [480, 280, 540, 310],
            "source_message": {
                "id": "win32_ocr:same-frame-local-voice-id",
                "source_adapter": "win32_ocr",
                "type": "voice",
                "sender_role": "customer",
            },
        ...
        initial = bridge._contractual_message_payload(
            {"observations": [old_voice]}
        )
        initial["observations"][0]["_worker_stable_id"] = (
            "worker-message-8"
        )
        initial["observations"][0]["_worker_identity_scope"] = (
        ...
        def replacement_prepare(**_kwargs):
            ...
            return {
                **prepared,
                ...
                "state": "voice_action_prepared",
                "voice_action_stage": "prepare",
                "pre_frame_id": "replacement-frame",
                "selected_pre_observation_id": (
                    "win32_ocr:same-frame-local-voice-id"
                ),
                "selected_action_token": "replacement-token",
                "selected_target_fingerprint": (
                    "same-duration-position-fingerprint"
                ),
                "message_viewport_change_digest": str(
                ...
                "selected_voice_observation": new_voice,
                "selected_physical_anchor_keys": [
                    "new-voice-anchor"
                ],
                "candidate_group_count": 1,
        ...
        result = runner._finish_new_visible_voices_in_current_chat(
            ...
            action_cancel_requested=lambda: False,
            enforce_read_targets=False,
            read_run_id="read-fixed-capacity-replacement",
            excluded_voice_anchor_keys=set(),
            flow_outcomes=FlowOutcomeAccumulator(
                origin_read_run_id=(
                    "read-fixed-capacity-replacement"
                )
            ),
            operation_phase="pre_send_refresh",
        )
    ...
    def test_pre_send_fixed_capacity_target_and_neighbor_replacements_are_zero_click(
        ...
        for replacement_type in ("text", "voice", "image"):
            with self.subTest(replacement_type=replacement_type):
                ...
                target = WechatReadTarget(
                    conversation_id=(
                        "conv-fixed-neighbor-" + replacement_type
                    ),
                    rpa_session_key=(
                        "wx:fixed-neighbor-" + replacement_type
                    ),
                    display_name="CJREPLN2",
                    remark_code="CJREPLN2",
                    authorization_revision=(
                        "revision-fixed-neighbor-" + replacement_type
                    ),
                    raw={"identity_checkpoint": identity_checkpoint()},
                )
                bridge = FakeBridge(
                ...
                def row(
                    ...
                    *,
                    ...
                    item = {
                        ...
                        "voice_duration": (
                            4 if message_type == "voice" else None
                        ),
                        "source_message": {
                            "id": (
                                native_id
                                or "win32_ocr:same-weak-neighbor"
                            ),
                            "source_adapter": (
                                "native_fixture"
                                if native_id
                ...
                before = [
                    row("old-neighbor", replacement_type),
                    row("before", "text", native_id="native-before"),
                    row(
                        "target-voice",
                        "voice",
                        native_id="native-target-voice",
                    ),
                    row("after", "text", native_id="native-after"),
                ]
                after = [
                    row(
                        "before-next",
                        "text",
                        native_id="native-before",
                    ),
                    row(
                        "target-voice-next",
                        "voice",
                ...
                for index, item in enumerate(before, start=1):
                    ...
                    item["_worker_identity_scope"] = (
                        "current_read_provisional"
                        if item["message_type"] in {"voice", "image"}
                        else "committed"
                    )
                initial = bridge._contractual_message_payload(
                    {"observations": before}
                ...
                def replacement_prepare(**_kwargs):
                    ...
                    return {
                        **prepared,
                        ...
                        "voice_action_stage": "prepare",
                        "pre_frame_id": (
                            "fixed-neighbor-frame-" + replacement_type
                        ),
                        "selected_pre_observation_id": (
                            "target-voice-next"
                        ),
                        "selected_action_token": (
                            "fixed-neighbor-token-" + replacement_type
                        ),
                        "selected_target_fingerprint": (
                            "fixed-neighbor-fingerprint-"
                            + replacement_type
                        ),
                        "message_viewport_change_digest": str(
                            initial["send_context_guard"][
                        ...
                        "selected_voice_observation": after[1],
                        "selected_physical_anchor_keys": [
                            "fixed-neighbor-target-anchor"
                        ],
                        "candidate_group_count": 1,
                        "ui_action_performed": False,
                    }
                ...
                result = (
                    runner._finish_new_visible_voices_in_current_chat(
                        ...
                        target=target,
                        target_label="CJREPLN2",
                        sidecar_payload=initial,
                        lease=unittest.mock.Mock(),
                        action_cancel_requested=lambda: False,
                        enforce_read_targets=False,
                        read_run_id=(
                            "read-fixed-neighbor-" + replacement_type
                        ),
                        excluded_voice_anchor_keys=set(),
                        flow_outcomes=FlowOutcomeAccumulator(
                            origin_read_run_id=(
    ...
    def test_first_viewport_change_consumes_only_reidentification_budget(self):
        api = FakeApi(None)
        target = WechatReadTarget(
            conversation_id="conv-v0932-one-reidentification",
            rpa_session_key="wx:v0932-one-reidentification",
            display_name="CJREID32",
            remark_code="CJREID32",
            authorization_revision="revision-v0932-one-reidentification",
            raw={"identity_checkpoint": identity_checkpoint()},
        )
        bridge = FakeBridge(
            RpaResult(ok=True, result_code="unused")
        ...
        existing_text = {
            "schema_version": 3,
            "observation_id": "existing-text-v0932",
            "row_kind": "text_bubble",
            "sender_role": "customer",
            "sender_role_source": "same_row_avatar",
            "message_type": "text",
            "voice_state": "not_voice",
            "content_clean": "原有文字",
            "native_source_message_id": "native-existing-v0932",
            "_worker_stable_id": "worker-message-1",
            "source_message": {
                "id": "existing-text-v0932",
                "type": "text",
                "sender_role": "customer",
                "content": "原有文字",
            },
            "bubble_rect": [480, 210, 650, 250],
        }
        voice = {
            "schema_version": 3,
            "observation_id": "voice-v0932",
            "row_kind": "voice_bubble",
            "sender_role": "customer",
            "sender_role_source": "same_row_avatar",
            "message_type": "voice",
            "voice_state": "untranscribed",
            "voice_anchor_key": "voice-anchor-v0932",
            "voice_duration": 4,
            "native_source_message_id": "native-voice-v0932",
            "source_message": {
                "id": "voice-v0932",
                "type": "voice",
                "sender_role": "customer",
        ...
        new_text = {
            "schema_version": 3,
            "observation_id": "new-text-v0932",
            "row_kind": "text_bubble",
            "sender_role": "customer",
            "sender_role_source": "same_row_avatar",
            "message_type": "text",
            "voice_state": "not_voice",
            "content_clean": "语音期间新增文字",
            "source_message": {
                "id": "new-text-v0932",
                "type": "text",
                "sender_role": "customer",
            ...
            "bubble_rect": [480, 350, 720, 395],
        }
        prepare_calls = 0
        execute_calls = 0

        def changed_prepare(**_kwargs):
            nonlocal prepare_calls
            prepare_calls += 1
            prepared = bridge._contractual_message_payload(
                {"observations": [existing_text, voice, new_text]}
            )
            bridge.last_message_payload = dict(prepared)
            viewport_digest = str(
                prepared["send_context_guard"][
                    "message_viewport_change_digest"
                ]
            ...
            return {
                **prepared,
                ...
                "state": "voice_action_prepared",
                "voice_action_stage": "prepare",
                "pre_frame_id": "v0932-reidentified-frame",
                "selected_pre_observation_id": "voice-v0932",
                "selected_action_token": "v0932-action-token",
                "selected_target_fingerprint": "v0932-target-fingerprint",
                "message_viewport_change_digest": viewport_digest,
                "selected_voice_observation": dict(voice),
                "selected_physical_anchor_keys": ["voice-anchor-v0932"],
                "candidate_group_count": 1,
                "ui_action_performed": False,
            }

        def changed_again_execute(**kwargs):
            nonlocal execute_calls
            execute_calls += 1
            base = bridge._contractual_message_payload(
                {"observations": [existing_text, voice, new_text]}
            )
            return {
                **base,
                "ok": False,
                "state": "voice_action_cancelled_before_trigger",
                "error_code": "C2_VOICE_ACTION_CANCELLED",
                "voice_action_stage": "execute",
                "action_phase": "cancelled_before_trigger",
                "business_state": "not_attempted",
                "business_result_confirmed": False,
                "ui_action_performed": False,
                "canonical_voice_action_id": kwargs[
                    "canonical_voice_action_id"
                ],
                "reserved_worker_stable_id": kwargs[
                    "reserved_worker_stable_id"
                ],
                "pre_frame_id": kwargs["pre_frame_id"],
                "selected_pre_observation_id": kwargs[
                    "selected_pre_observation_id"
                ],
                "selected_action_token": kwargs["selected_action_token"],
                "selected_target_fingerprint": kwargs[
                    "selected_target_fingerprint"
                ],
                "message_viewport_change_digest": kwargs[
                    "message_viewport_change_digest"
                ],
                "candidate_count": 1,
                "matched_candidate_count": 1,
                "post_frame_id": "v0932-changed-again-frame",
            }

        bridge.prepare_voice_action = changed_prepare  # type: ignore[method-assign]
        bridge.execute_voice_action = changed_again_execute  # type: ignore[method-assign]
        runner, _ = self.make_runner(api, bridge)
        result = runner._finish_new_visible_voices_in_current_chat(
            binding=Binding(
                worker_id="worker-1",
            ...
            target=target,
            target_label="CJREID32",
            sidecar_payload=initial,
            lease=unittest.mock.Mock(),
            action_cancel_requested=lambda: False,
            enforce_read_targets=False,
            read_run_id="read-v0932-one-reidentification",
            excluded_voice_anchor_keys=set(),
            flow_outcomes=FlowOutcomeAccumulator(
                origin_read_run_id="read-v0932-one-reidentification"
            ),
            operation_phase="pre_send_refresh",
        )
    ...
    def test_adapter_exception_after_trigger_writes_terminal_before_recovery(self):
        ...
        action_payload["send_context_guard"] = (
            FakeBridge._send_context_guard([observation])
        )
    ...
    def test_sidecar_message_identity_claim_is_rejected_before_media_or_ingest(self):
        api = FakeApi(None)
        target = WechatReadTarget(
            conversation_id="conv-sidecar-identity-overreach",
            rpa_session_key="wx:rpa:v1:sidecar-identity-overreach",
            display_name="CJSIDE01",
            remark_code="CJSIDE01",
            row_fingerprint={"title_text": "CJSIDE01"},
            read_reason="waiting_user_reply",
            authorization_revision="revision-sidecar-identity-overreach",
        )
        api.read_targets = [target]
        bridge = FakeBridge(
            RpaResult(ok=True, result_code="unused", message="unused")
                ...
                "observations": [
                    {
                        ...
                        "message_type": "voice",
                        "voice_state": "untranscribed",
                        "worker_stable_id": "worker-message-forged",
                        "source_message": {
        ...
        voice_actions.assert_not_called()
        image_actions.assert_not_called()
        self.assertEqual(api.message_payloads, [])
        self.assertEqual(bridge.voice_transcribes, [])
    ...
    def test_completed_image_survives_final_read_failure_and_restart(self):
        ...
        def vision_completes_with_identity_receipt(**kwargs):
            ...
            journal = read_action_journal(journal_path)
            frame_binding = kwargs["frame_action_binding"]
            self.assertEqual(
                frame_binding,
                journal["prepare_evidence"]["frame_action_binding"],
            )
            selected = [
                item
            ...
            update_action_journal_item(
                ...
                terminal_payload={
                    ...
                    "confirmed_action_mapping": {
                        "canonical_action_id": action_id,
                        "reserved_worker_stable_id": reserved_id,
                        "selected_action_token": frame_binding[
                            "selected_action_token"
                        ],
    ...
    def test_action_journal_vertical_c2_image_reaches_ingest_and_ledger(self):
        ...
        def vision_boundary(**kwargs):
            ...
            self.assertTrue(selected_action_token)
    ...
    def test_scheduler_recovers_confirmed_image_journal_after_process_exit(
        ...
        action_payload["send_context_guard"] = (
            FakeBridge._send_context_guard(reconciled)
        )
        FakeBridge._attach_image_frame_action_bindings(action_payload)
    ...
    def test_cached_failed_and_new_completed_image_requires_final_refresh(self):
        ...
        FakeBridge._attach_image_frame_action_bindings(mixed_payload)
        with patch(
            "chejin_worker_client.omniauto_vision."
            "vision_configuration_status",
            return_value={
    ...
    def test_c2_history_gap_blocks_brain_but_still_terminalizes_image(self):
        ...
        payload.pop("message_viewport_change_evidence", None)
        FakeBridge._attach_image_frame_action_bindings(payload)
    ...
    def test_real_image_terminal_path_captures_each_unattended_failure(self):
        ...
        with patch(
            ...
            for observation in payload["observations"]:
                ...
                frame_payload.pop("image_frame_action_bindings", None)
                frame_payload.pop("send_context_guard", None)
                frame_payload.pop(
                    "message_viewport_change_evidence", None
                )
    ...
    def test_c2_image_method_fails_closed_if_global_preflight_is_bypassed(self):
        ...
        with patch(
            ...
            _, first_stats = runner._process_final_image_slots(
                ...
                allowed_new_observation_ids={
                    str(
                        sidecar_payload["observations"][0][
                            "observation_id"
                        ]
                    )
                },
    ...
    def test_new_media_with_native_id_stays_unreserved_until_selected(self):
        runner, _ = self.make_runner(
            FakeApi(None),
            FakeBridge(RpaResult(ok=True, result_code="ok", message="unused")),
        )
        target = WechatReadTarget(
            conversation_id=f"conv-native-media-{time.time_ns()}",
            rpa_session_key="wx:rpa:v1:native-media",
            display_name="CJNATIVE1",
            remark_code="CJNATIVE1",
            authorization_revision="revision-native-media",
            raw={"identity_checkpoint": identity_checkpoint(next_sequence_floor=31)},
        )
        observations = [
            {
                "observation_id": "native-voice-candidate",
                "row_kind": "voice_bubble",
                "message_type": "voice",
                "sender_role": "customer",
                "sender_role_source": "same_row_avatar",
            ...
            {
                "observation_id": "native-image-candidate",
                "row_kind": "image_bubble",
                "message_type": "image",
                "sender_role": "customer",
                "sender_role_source": "same_row_avatar",
        ...
        assigned = runner._assign_sequence_new_suffix_identities(
            ...
            evidence={
                ...
                "old_tail_fully_consumed": True,
                "new_suffix_observation_ids": [
                    "native-voice-candidate",
                    "native-image-candidate",
                ],
            },
            read_run_id="read-native-media",
        )

        for item in assigned:
            self.assertNotIn("_worker_stable_id", item)
            self.assertNotIn("_worker_identity_scope", item)
            self.assertNotIn("_worker_committed_message", item)
        first_selected_id = runner._reserve_worker_sequence(
            target,
            reservation_key="selected-action:voice-native-media",
        )
        self.assertEqual(first_selected_id, "worker-message-31")

    def test_new_suffix_rejects_pre_reserved_unselected_media(self):
        runner, _ = self.make_runner(
            FakeApi(None),
            FakeBridge(RpaResult(ok=True, result_code="ok", message="unused")),
        )
        target = WechatReadTarget(
            conversation_id=f"conv-pre-reserved-media-{time.time_ns()}",
            rpa_session_key="wx:rpa:v1:pre-reserved-media",
            display_name="CJRESV33",
            remark_code="CJRESV33",
            authorization_revision="revision-pre-reserved-media",
        )
        ...
        with self.assertRaisesRegex(
            ...
            runner._assign_sequence_new_suffix_identities(
                ...
                observations=[
                    {
                        "observation_id": "unselected-voice",
                        "row_kind": "voice_bubble",
                        "message_type": "voice",
                        "sender_role": "customer",
                        "sender_role_source": "same_row_avatar",
                        "_worker_stable_id": "worker-message-41",
                ...
                evidence={
                    "alignment_status": "unique",
                    "old_tail_fully_consumed": True,
                    "new_suffix_observation_ids": ["unselected-voice"],
                },
                read_run_id="read-pre-reserved-media",
            )
    ...
    def test_image_processor_executes_only_one_candidate_per_authoritative_frame(self):
        ...
        def image(observation_id: str, top: int) -> dict:
            return {
                "schema_version": 3,
                "observation_id": observation_id,
                "row_kind": "image_bubble",
        ...
        first = image("image-first", 100)
        second = image("image-second", 220)
        allowed = {
    ...
    def test_zero_ui_image_cancellation_defers_next_image_to_fresh_frame(self):
        ...
        first = image("image-zero-ui", 100)
        second = image("image-with-ui", 220)
        allowed = {
            str(first["observation_id"]),
            str(second["observation_id"]),
        }
